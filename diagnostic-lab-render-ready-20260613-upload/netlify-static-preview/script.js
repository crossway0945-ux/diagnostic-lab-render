...
function handleCompletedAnalysis(result, payload) {
  ...
  showToast("Personalized diagnostic analysis is ready.");
}
...
function updateUserPanel(user) {
  currentUser = {
    ...user,
    quota: Number(user.totalQuota ?? user.quota ?? 0),
    used: Number(user.usedQuota ?? user.used ?? 0),
    remaining: Math.max(0, Number(user.remainingQuota ?? user.remaining ?? (user.quota || 0) - (user.used || 0))),
...
function updateAnalyzeAvailability() {
  ...
  const quotaUsed = currentUser.remaining <= 0;
  analyzeButton.disabled = accessExpired || quotaUsed;
  if (accessExpired) {
...
async function collectPayload() {
  ...
  const minimumWords = isTask1 ? 40 : 80;
  const wordCount = countWords(payload.writing);
  if (wordCount < minimumWords) {
    throw new Error(`${activeTask} writing is too short for a useful diagnostic. Please paste the full response.`);
  }
  ...
  printReport.innerHTML = `
    <article class="print-page print-cover">
      ...
      <div class="print-summary-grid">
        ${renderPrintInfo("Student", currentUser?.displayName || currentUser?.username || "-")}
    ...
    <article class="print-page">
      <section class="print-section">
        ...
        <div class="print-issue-list">
          ${topIssues.length ? topIssues.map((issue, index) => renderPrintTopIssue(issue, feedbackCards[index], index)).join("") : "<p>No evidence-based issues found yet.</p>"}
        </div>
      </section>
    </article>
...
function renderPrintTopIssue(issue, card = {}, index) {
  ...
  return `<div class="print-issue">
    ...
    <p><strong>Paragraph Location:</strong> ${escapeHtml(card.paragraphLocation || issue.paragraphLocation || "-")}</p>
    <blockquote>${escapeHtml(card.exactSentence || issue.exactSentence || "Exact sentence is available in the detailed feedback section.")}</blockquote>
...
function renderPrintProgressSummary(taskType, records) {
  if (records.length <= 1) {
    return `<p class="print-note">Progress tracking for this task type will appear after multiple submissions.</p>`;
  }
  ...
  return `
    <div class="progress-summary-grid">
      ...
      <article class="progress-stat">
        ...
        <strong>${escapeHtml(getTopRepeatedIssue(records) || "-")}</strong>
      </article>
    </div>
...
function setLoading(isLoading) {
  ...
  const quotaUsed = currentUser && currentUser.remaining <= 0;
  analyzeButton.disabled = isLoading || accessExpired || quotaUsed;
