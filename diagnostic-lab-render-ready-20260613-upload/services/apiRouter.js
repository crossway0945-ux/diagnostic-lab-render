...
async function handleAnalyze(request, storage, jobStore) {
  ...
  if (existingRecord) {
    ...
    return jsonResponse(200, {
      ...
      analysis: existingRecord.report || existingRecord.analysis,
      user: withExpiryFlags(sanitizeUserForClient(latestUser || user)),
  ...
  const progressRecord = buildSubmissionHistoryRecord(user.username, payload, analysis);
...
export async function processAnalyzeJob({ jobId, rootDir = process.cwd() }) {
  ...
  try {
    ...
    const progressRecord = buildSubmissionHistoryRecord(user.username, job.payload, analysis);
...
function buildSubmissionHistoryRecord(username, payload, analysis) {
