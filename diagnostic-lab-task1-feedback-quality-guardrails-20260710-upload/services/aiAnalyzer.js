...
function buildTask1IntroTargetedRevision(payload) {
  ...
  return buildBarIntroRevision(promptText);
}
...
function buildLineIntroRevision(prompt) {
  ...
  return `The line graph illustrates changes in ${subject}${periodSuffix}.`;
}

function buildBarIntroRevision(prompt) {
  const subject = compactTask1Prompt(prompt);
  return `The bar chart compares figures for ${subject}.`;
}

function buildPieIntroRevision(prompt) {
  const subject = compactTask1Prompt(prompt);
  return `The pie chart illustrates the proportions of ${subject}.`;
}
...
function buildMapIntroRevision(prompt) {
  ...
  if (place && years.length >= 2) {
    return `The maps compare the layout of ${place} in ${years[0]} and ${years[1]}.`;
  }
  if (place) return `The maps compare the layout of ${place} at the given stages.`;
  return "The maps compare the layout of the area at the given stages.";
}
...
function compactTask1Prompt(prompt) {
  const cleaned = String(prompt || "")
    ...
    .replace(/^the\s+(?:line graph|bar chart|pie chart|pie charts|table|maps?|plans?|diagrams?)\s+(?:below\s+)?(?:shows?|show|illustrates?|illustrate|compares?|compare|gives?|give|presents?|present)\s+/i, "")
    .replace(/^how\s+/i, "")
    .replace(/\s+/g, " ")
    .replace(/[. ]+$/g, "")
    .trim();
...
function repairTask1FeedbackCardRevision(card, payload) {
  ...
  const cardText = [
    card.issueType,
    card.paragraphLocation,
    card.sentenceFunction,
    card.whyItLimitsBand,
    card.kruPomDiagnosis
  ].filter(Boolean).join(" ");
  const isIntro = isTask1IntroductionCard(card, payload);
  ...
  if (isIntro && shouldRepairTask1IntroRevision(payload, revision)) {
    ...
    reason = "This revision is safer because it names the visual type, measurement, subject, categories or groups, and avoids copying the task prompt's clause order.";
    action = "For Task 1 introductions, identify the visual type first, then state what is measured or compared; keep technical nouns but change the reporting structure naturally.";
  ...
  return {
    ...
    whyRevisionIsStronger: appendGuidance(card.whyRevisionIsStronger, reason, /names the visual type|visual-type-appropriate|process diagrams need stages/i),
    studentAction: appendGuidance(card.studentAction, action, /visual type first|Task 1 overviews|process diagrams/i)
  };
}
...
function enrichFeedbackStrategies(cards = [], payload = {}) {
  return cards.map((card) => {
    ...
    if (!strategy) return card;

    return {
      ...card,
      kruPomDiagnosis: appendGuidance(card.kruPomDiagnosis, strategy.diagnosis, strategy.marker),
      whyRevisionIsStronger: appendGuidance(card.whyRevisionIsStronger, strategy.why, strategy.marker),
      studentAction: appendGuidance(card.studentAction, strategy.action, strategy.marker)
    };
  });
}
...
function enrichParagraphStrategies(items = [], payload = {}) {
  ...
  return items.map((item) => ({
    ...item,
    action: appendGuidance(item.action, strategy.action, strategy.marker)
  }));
}
