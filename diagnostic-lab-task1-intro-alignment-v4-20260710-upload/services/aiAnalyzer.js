...
function buildBarIntroRevision(prompt) {
  ...
  return dynamic
    ? `The bar chart compares changes in ${subject}.`
    : `The bar chart compares figures for ${subject}.`;
}
...
function repairTask1FeedbackCardRevision(card, payload) {
  ...
  return {
    ...
    kruPomDiagnosis: isIntro
      ...
      : baseCard.kruPomDiagnosis,
    whyRevisionIsStronger: appendGuidance(baseCard.whyRevisionIsStronger, reason, /changes-over-time frame|visual-type-appropriate|process diagrams need stages|exact metric\/unit/i),
