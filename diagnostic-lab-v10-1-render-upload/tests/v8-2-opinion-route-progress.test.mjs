...
function testLowConfidenceConfirmation() {
  ...
  assert.throws(
    ...
    (error) => error.errorCode === "ESSAY_TYPE_CONFIRMATION_REQUIRED" && error.promptClassificationConfidence === "low"
  );
  ...
  assert.equal(confirmed.promptClassificationConfidence, "low");
}
