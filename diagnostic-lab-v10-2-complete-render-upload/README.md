...
Upload the complete folder `diagnostic-lab-v10-2-complete-render-upload` to the repository root. Verify that GitHub shows `diagnostic-lab-v10-2-complete-render-upload/package.json`, then set Render **Root Directory** to `diagnostic-lab-v10-2-complete-render-upload`. Build command remains `npm install`; start command remains `npm start`.
