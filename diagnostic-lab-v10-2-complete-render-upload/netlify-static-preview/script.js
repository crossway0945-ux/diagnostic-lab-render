...
const progressStudentControls = document.querySelector("#progress-student-controls");
const progressStudentSearch = document.querySelector("#progress-student-search");
const progressStudentStatus = document.querySelector("#progress-student-status");
...
let progressSelectedStudentProfileToken = "";
...
studentProfileSelect.addEventListener("change", () => {
  ...
  if (value === "__add__") {
    ...
    progressSelectedStudentProfileToken = value;
...
progressStudentSearch?.addEventListener("input", renderProgressStudentOptions);
progressStudentStatus?.addEventListener("change", renderProgressStudentOptions);
progressStudentSelect?.addEventListener("change", async () => {
  progressSelectedStudentProfileToken = progressStudentSelect.value;
...
document.addEventListener("click", (event) => {
  ...
  if (historyReanalyzeButton) {
    reanalyzeSubmission(historyReanalyzeButton.dataset.historyReanalyze);
    return;
  }
...
logoutButton.addEventListener("click", async () => {
  ...
  progressSelectedStudentProfileToken = "";
  selectedStudentProfileToken = "";
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (!response.ok && ["ESSAY_TYPE_MISMATCH", "VISUAL_TYPE_MISMATCH"].includes(result.errorCode)) {
      const detected = result.detectedEssayType || result.detectedVisualType || "";
      const selector = result.errorCode === "ESSAY_TYPE_MISMATCH"
        ? document.querySelector("#essay-type")
        : document.querySelector("#visual-type");
      if (detected && [...selector.options].some((option) => option.value === detected)) selector.value = detected;
      throw Object.assign(new Error(result.error), { errorCode: result.errorCode });
    }
    if (!response.ok && ["ESSAY_TYPE_CONFIRMATION_REQUIRED", "VISUAL_TYPE_CONFIRMATION_REQUIRED"].includes(result.errorCode)) {
      const detected = result.detectedEssayType || result.detectedVisualType || "";
      if (!detected || /Not Sure|Auto-detect/i.test(detected)) {
        throw Object.assign(new Error(result.error), { errorCode: result.errorCode });
      }
      const confirmed = window.confirm(`${result.error}\n\nContinue using ${detected}?`);
      if (!confirmed) return;
      payload.options = {
        ...(payload.options || {}),
        ...(result.errorCode === "ESSAY_TYPE_CONFIRMATION_REQUIRED" ? { essayTypeConfirmed: true } : { visualTypeConfirmed: true })
      };
      ...
      result = await readJsonResponse(response, "Analysis could not be completed after task-type confirmation.");
    }
...
function handleCompletedAnalysis(result, payload) {
  if (selectedStudentProfileToken) progressSelectedStudentProfileToken = selectedStudentProfileToken;
  ...
  loadProgressHistory();
  showToast(result.duplicateSubmission ? result.message || "This exact submission has already been analysed. Opening the existing validated report." : "Personalized diagnostic analysis is ready.");
}
...
function updateAnalyzeAvailability() {
  ...
  analyzeButton.disabled = accessExpired || studentMissing;
  if (accessExpired) {
    analyzeButton.textContent = "Access Expired";
    showError(ACCESS_EXPIRED_MESSAGE);
  } else if (quotaUsed) {
    analyzeButton.textContent = "Open Existing / Check Submission";
...
async function loadStudentProfiles(preferredToken = "") {
  ...
  try {
    ...
    progressSelectedStudentProfileToken = selectedStudentProfileToken;
...
function renderProgressStudentOptions() {
  if (!progressStudentControls || !progressStudentSelect) return;
  const teacher = isTeacherAccount(currentUser);
  progressStudentControls.hidden = !teacher;
  if (!teacher) return;
  const query = String(progressStudentSearch?.value || "").normalize("NFKC").trim().toLowerCase();
  const status = progressStudentStatus?.value || "active";
  const source = status === "archived"
    ? archivedStudentProfiles
    : status === "all"
      ? [...studentProfiles, ...archivedStudentProfiles]
      : studentProfiles;
  const visible = source.filter((profile) => !query || profile.displayName.toLowerCase().includes(query));
  progressStudentSelect.innerHTML = [
    '<option value="">Select a student</option>',
    ...visible.map((profile) => {
      const suffix = profile.active === false ? " — archived" : "";
      return `<option value="${escapeHtml(profile.profileToken)}">${escapeHtml(profile.displayName)}${escapeHtml(suffix)}</option>`;
    })
  ].join("");
  progressStudentSelect.value = visible.some((profile) => profile.profileToken === progressSelectedStudentProfileToken)
    ? progressSelectedStudentProfileToken
    : "";
}
...
async function loadProgressHistory() {
  if (!currentUser) return;
  renderProgressStudentOptions();
  if (isTeacherAccount(currentUser) && !progressSelectedStudentProfileToken) {
    progressRecords = [];
    currentProgressSummary = null;
    renderProgressTracker();
    return;
  ...
  try {
    const query = progressSelectedStudentProfileToken ? `?student=${encodeURIComponent(progressSelectedStudentProfileToken)}` : "";
...
function renderProgressTracker() {
  ...
  const teacherNoSelection = isTeacherAccount(currentUser) && !progressSelectedStudentProfileToken;
  const profile = [...studentProfiles, ...archivedStudentProfiles]
    .find((item) => item.profileToken === progressSelectedStudentProfileToken);
  const displayName = profile?.displayName || currentUser?.displayName || currentUser?.username || "this student";
  const hasRecords = !teacherNoSelection && validRecords.length > 0;
  progressEmpty.classList.toggle("hidden", hasRecords);
  progressContent.classList.toggle("visible", hasRecords);

  if (!hasRecords) {
    if (teacherNoSelection) {
      progressEmptyEyebrow.textContent = "Student Progress";
      progressEmptyTitle.textContent = "Select a student to view progress.";
      progressEmptyText.textContent = "Choose a student above to load Task 1 and Task 2 history separately.";
    } else {
      progressEmptyEyebrow.textContent = "No valid reports yet";
      progressEmptyTitle.textContent = `No progress data yet for ${displayName}.`;
      progressEmptyText.textContent = "Start a new analysis. Progress appears only after a validated report is saved.";
    }
    return;
  }
...
function renderTaskProgress(taskType, records) {
  ...
  const previousMidpoint = previous ? getBandMidpoint(previous) : null;
  const latestMidpoint = getBandMidpoint(latest);
  const change = !Number.isFinite(previousMidpoint) || !Number.isFinite(latestMidpoint)
    ? "First valid report"
    : latestMidpoint > previousMidpoint ? `Improved by ${(latestMidpoint - previousMidpoint).toFixed(1)}`
      : latestMidpoint < previousMidpoint ? `Lower by ${(previousMidpoint - latestMidpoint).toFixed(1)}`
  ...
  return `
    <div class="progress-summary-grid">
      ...
      <article class="progress-stat">
        <span>Previous ${escapeHtml(taskType)} Estimated Range</span>
        <strong>${escapeHtml(previous?.estimatedBandRange || "No previous submission")}</strong>
      </article>
      <article class="progress-stat">
        <span>Change</span>
...
function renderHistoryRow(record, index) {
  ...
  return `<tr${invalid ? ' class="invalid-history-row"' : ""}>
    ...
    <td>${escapeHtml(type)}${isTeacherAccount(currentUser) && record.canonicalEngineVersion ? `<br><small>Engine ${escapeHtml(record.canonicalEngineVersion)} · ${escapeHtml(record.analysisReason || "first-analysis")}</small>` : ""}</td>
    ...
    <td>
      ...
      ${!invalid ? `<button class="text-button" type="button" data-history-reanalyze="${escapeHtml(record.submissionId)}">Re-analyze</button>` : ""}
...
async function reanalyzeSubmission(submissionId) {
  const confirmed = window.confirm("Re-analyze with the current engine? If this report already uses the current engine, the existing validated report will open and no credit will be used.");
  if (!confirmed) return;
  clearError();
  setLoading(true);
  try {
    const response = await fetch(`/api/submissions/${encodeURIComponent(submissionId)}/reanalyze`, { method: "POST" });
    const result = await readJsonResponse(response, "The report could not be re-analyzed.");
    if (!response.ok || !result.ok) throw new Error(result.error || "The report could not be re-analyzed.");
    handleCompletedAnalysis(result, { taskType: result.analysis?.taskType || activeTask });
    showToast(result.message || (result.reanalysisStatus === "engine-upgrade-created"
      ? "A new engine-version report was saved; the earlier report was preserved."
      : "The current-version report already exists and was opened without using credit."));
  } catch (error) {
    showError(error.message);
  } finally {
    setLoading(false);
...
function getTopRepeatedIssue(records) {
  ...
  for (const record of records) {
    const issues = new Set((record.top3Issues || [])
      .map((issue) => issue?.issueType || issue?.title || issue?.issue || "")
      .filter(Boolean));
    if (!issues.size) {
      const fallback = getMainIssue(record);
      if (fallback && fallback !== "-") issues.add(fallback);
    }
...
function setLoading(isLoading) {
  ...
  analyzeButton.disabled = isLoading || accessExpired || studentMissing;
  ...
  if (!isLoading && quotaUsed && !accessExpired) analyzeButton.textContent = "Open Existing / Check Submission";
  if (!isLoading && studentMissing && !accessExpired) analyzeButton.textContent = "Select a Student";
