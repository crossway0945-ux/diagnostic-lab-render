...
import { normalizeTask1PublicVisualType } from "./task1Safety.js";
...
export function buildCanonicalAnalysis({
  ...
  if (taskType === "Task 2" && analysis.canonicalTask2Analysis) {
    ...
    return {
      ...
      metadata: {
        ...
        canonicalEngineVersion: String(payload.canonicalEngineVersion || base.metadata?.canonicalEngineVersion || ""),
        rubricVersion: String(payload.rubricVersion || base.metadata?.rubricVersion || ""),
        promptVersion: String(payload.promptVersion || base.metadata?.promptVersion || ""),
        reportSchemaVersion: String(payload.reportSchemaVersion || base.metadata?.reportSchemaVersion || ""),
        ...
        generatedAt: String(payload.generatedAt || base.metadata?.generatedAt || "")
      },
      executiveSummary: {
  ...
  return {
    ...
    metadata: {
      ...
      publicVisualType: normalizeTask1PublicVisualType(payload.publicVisualType || analysis.visualType || payload.visualType),
    ...
    consistency: {
      analysisSource: "canonical-analysis-v10",
      routeSource: "canonical-analysis-v10",
...
export function projectCanonicalAnalysis(canonical, legacy = {}) {
  ...
  return {
    ...legacy,
    essayType: canonical.metadata?.publicEssayType || legacy.essayType,
    visualType: canonical.metadata?.publicVisualType || canonical.metadata?.visualType || legacy.visualType,
