...
async function testCanonicalRendererAndPdfSerializationPath() {
  ...
  assert.match(index, /Mixed \/ Combination Visuals/);
  assert.doesNotMatch(index, /<option>Structural Diagram<\/option>/);
  assert.doesNotMatch(index, /<option>Advantages Outweigh Disadvantages<\/option>/);
  assert.match(index, /script\.js\?v=diagnostic-v10-2-frontend-recovery/);
