...
- Canonical Task 1/Task 2 analysis and one displayed scoring source
- Six public Task 2 choices with prompt-driven internal obligations for causes/effects, causes/solutions, outweigh, positive/negative and mixed direct questions
- Eight public Task 1 choices with internal process vs structural/mechanism diagram detection
- Exact-input/version fingerprinting and validated-report reuse for reproducible duplicate submissions
- Teacher Progress student search, active/archived filter, separate Task tabs and report version history
- Criterion arithmetic with explicit, visible cap metadata
- Task-aware framework, feedback-card and PDF wording
- Minimal-correction and route-preserving revision fidelity checks
- One-to-one Top Issue to Detailed Feedback evidence mapping
...
- Student profile archive/restore plus confirmed permanent deletion scoped to that student's reports and cache; account credits remain unchanged
...
Upload the complete folder `diagnostic-lab-v10-final-commercial-sale-readiness-upload` to the repository root. Verify that GitHub shows `diagnostic-lab-v10-final-commercial-sale-readiness-upload/package.json`, then set Render **Root Directory** to `diagnostic-lab-v10-final-commercial-sale-readiness-upload`. Build command remains `npm install`; start command remains `npm start`.
