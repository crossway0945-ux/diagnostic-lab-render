...
const TASK2_TYPE_LABELS = Object.freeze({
  ...
  [TASK2_CANONICAL_TYPES.CAUSES_SOLUTIONS]: "Causes & Solutions",
  [TASK2_CANONICAL_TYPES.CAUSES_EFFECTS]: "Causes & Effects",
  [TASK2_CANONICAL_TYPES.POSITIVE_NEGATIVE]: "Positive / Negative Development",
  [TASK2_CANONICAL_TYPES.DIRECT_QUESTION]: "Direct Question",
  [TASK2_CANONICAL_TYPES.HYBRID]: "Hybrid Direct Question",
  [TASK2_CANONICAL_TYPES.UNRESOLVED]: "Unresolved Task Type"
});

export const TASK2_PUBLIC_ESSAY_TYPES = Object.freeze([
  "Opinion Essay",
  "Discuss Both Views",
  "Problem & Solution",
...
export function publicTask2TypeForInternal(essayType) {
  if (essayType === TASK2_CANONICAL_TYPES.OPINION) return "Opinion Essay";
  if (essayType === TASK2_CANONICAL_TYPES.DISCUSS_BOTH_VIEWS) return "Discuss Both Views";
  if ([TASK2_CANONICAL_TYPES.PROBLEM_SOLUTION, TASK2_CANONICAL_TYPES.CAUSES_SOLUTIONS].includes(essayType)) return "Problem & Solution";
  if ([TASK2_CANONICAL_TYPES.ADVANTAGES_DISADVANTAGES, TASK2_CANONICAL_TYPES.OUTWEIGH].includes(essayType)) return "Advantages & Disadvantages";
  if ([TASK2_CANONICAL_TYPES.CAUSES_EFFECTS, TASK2_CANONICAL_TYPES.POSITIVE_NEGATIVE, TASK2_CANONICAL_TYPES.DIRECT_QUESTION, TASK2_CANONICAL_TYPES.HYBRID].includes(essayType)) return "Direct Question";
  return "Not Sure / Auto-detect";
}

export function normalizeTask2PublicEssayType(value) {
  const text = String(value || "").trim();
  if (!text || /not sure|auto.?detect/i.test(text)) return "Not Sure / Auto-detect";
  const internal = selectedTaskType(text);
  return internal ? publicTask2TypeForInternal(internal) : "";
}
...
export function analyzeTask2Safety(payload = {}) {
  ...
  const directQuestionMissingPart = [TASK2_CANONICAL_TYPES.DIRECT_QUESTION, TASK2_CANONICAL_TYPES.HYBRID].includes(essayRoute) && routeAssessment.missingRequirements.length > 0;
  ...
  if (essayRoute === TASK2_CANONICAL_TYPES.OUTWEIGH && isControlledRouteStatus(routeAssessment.status) && (languageAccuracyRisk.blocksSecureBand7 || developmentRisk.unevenDevelopment)) {
  ...
  const canonicalAnalysis = {
    ...
    metadata: {
      ...
      essayTypeLabel: publicTask2TypeForInternal(essayRoute),
      publicEssayType: publicTask2TypeForInternal(essayRoute),
      internalEssaySubtype: essayRoute,
      internalEssaySubtypeLabel: TASK2_TYPE_LABELS[essayRoute],
...
export function classifyTask2Prompt(payload = {}) {
  ...
  add(TASK2_CANONICAL_TYPES.DISCUSS_BOTH_VIEWS, "high", /discuss both views|discuss (?:these|the) views/, "two views");
  add(TASK2_CANONICAL_TYPES.OUTWEIGH, "high", /(?:do|does) the advantages? outweigh|(?:do|does) the disadvantages? outweigh|outweigh the (?:advantages?|disadvantages?|drawbacks?)/, "comparative outweigh judgement");
  add(TASK2_CANONICAL_TYPES.POSITIVE_NEGATIVE, "high", /positive or negative development|negative or positive development|is this (?:a )?positive or negative/, "evaluative development judgement");
  add(TASK2_CANONICAL_TYPES.CAUSES_EFFECTS, "high", /(?:what|which) (?:are )?(?:the )?causes?[^?]*(?:effects?|consequences?|impacts?)|causes?\s*(?:and|&)\s*(?:effects?|consequences?|impacts?)/, "cause-effect prompt pair");
  add(TASK2_CANONICAL_TYPES.CAUSES_SOLUTIONS, "high", /(?:what|which) (?:are )?(?:the )?causes?[^?]*(?:solutions?|measures?|steps?)|causes?\s*(?:and|&)\s*(?:solutions?|measures?)/, "cause-solution prompt pair");
  add(TASK2_CANONICAL_TYPES.PROBLEM_SOLUTION, "high", /problems?[^?]*(?:solutions?|measures?|steps?)|problem\s*[&/]\s*solution/, "problem-solution prompt pair");
  add(TASK2_CANONICAL_TYPES.ADVANTAGES_DISADVANTAGES, "high", /advantages?\s*(?:and|&)\s*disadvantages?|benefits?\s*(?:and|&)\s*drawbacks?|what are the advantages?[^?]*disadvantages?/, "two-sided advantages/disadvantages");
  add(TASK2_CANONICAL_TYPES.OPINION, "high", /to what extent do you agree or disagree|do you agree or disagree/, "agree/disagree judgement");
  ...
  if (!winner && directQuestionCount >= 2) {
    winner = {
      essayType: TASK2_CANONICAL_TYPES.DIRECT_QUESTION,
      confidence: "high",
  ...
  if (!winner && directQuestionCount === 1) {
    winner = {
      essayType: explicitOpinionQuestion ? TASK2_CANONICAL_TYPES.OPINION : TASK2_CANONICAL_TYPES.DIRECT_QUESTION,
  ...
  if (!winner) {
    winner = {
      essayType: TASK2_CANONICAL_TYPES.UNRESOLVED,
      confidence: "low",
  ...
  const selectedType = selectedTaskType(selected);
  const selectedPublicType = publicTask2TypeForInternal(selectedType);
  const detectedPublicType = publicTask2TypeForInternal(winner.essayType);
  const autoSelected = /not sure|auto.?detect/i.test(selected) || !selectedType;
  const classificationMatch = autoSelected || selectedType === winner.essayType || selectedPublicType === detectedPublicType;
  const stanceRequired = isStanceRequired({ ...payload, essayType: TASK2_TYPE_LABELS[winner.essayType] }, winner.essayType);
  const requiredRoutes = requiredRoutesForType(winner.essayType, stanceRequired, promptParts.length);
  return {
    essayType: winner.essayType,
    essayTypeLabel: TASK2_TYPE_LABELS[winner.essayType],
    publicEssayType: detectedPublicType,
    internalEssaySubtype: winner.essayType,
    confidence: winner.confidence,
    exactPromptSignals: Array.from(new Set(matches
      .filter((item) => item.essayType === winner.essayType)
      .map((item) => item.signal)
      .concat(winner.signal)
      .filter(Boolean))),
    signalReason: winner.signalLabel,
    selectedEssayType: selectedType,
    selectedEssayTypeLabel: autoSelected ? "Not Sure / Auto-detect" : (selectedPublicType || selected),
    classificationMatch,
    autoSelected,
    stanceRequired,
    requiredRoutes,
    internalObligations: requiredRoutes,
    promptParts
  };
}
...
function isStanceRequired(payload, essayType) {
  ...
  if (essayType === TASK2_CANONICAL_TYPES.POSITIVE_NEGATIVE) return true;
  ...
  if ([TASK2_CANONICAL_TYPES.DIRECT_QUESTION, TASK2_CANONICAL_TYPES.HYBRID].includes(essayType)) {
...
function selectedTaskType(value) {
  ...
  if (/hybrid/.test(text)) return TASK2_CANONICAL_TYPES.HYBRID;
  if (/discuss both views|discussion/.test(text)) return TASK2_CANONICAL_TYPES.DISCUSS_BOTH_VIEWS;
  if (/outweigh/.test(text)) return TASK2_CANONICAL_TYPES.OUTWEIGH;
  if (/positive.*negative|negative.*positive/.test(text)) return TASK2_CANONICAL_TYPES.POSITIVE_NEGATIVE;
  if (/causes?.*effects?|effects?.*causes?/.test(text)) return TASK2_CANONICAL_TYPES.CAUSES_EFFECTS;
  if (/causes?.*solutions?|cause\s*[&/]\s*solution/.test(text)) return TASK2_CANONICAL_TYPES.CAUSES_SOLUTIONS;
  if (/problem.*solution/.test(text)) return TASK2_CANONICAL_TYPES.PROBLEM_SOLUTION;
  if (/advantages?.*disadvantages?|advantage\s*[&/]\s*disadvantage/.test(text)) return TASK2_CANONICAL_TYPES.ADVANTAGES_DISADVANTAGES;
  if (/direct|two-part|two part/.test(text)) return TASK2_CANONICAL_TYPES.DIRECT_QUESTION;
  if (/opinion|agree.*disagree/.test(text)) return TASK2_CANONICAL_TYPES.OPINION;
  return "";
}
...
function requiredRoutesForType(essayType, stanceRequired, promptPartCount) {
  const routes = {
...
function buildTaskTypeRouteAssessment(context) {
  const builders = {
    ...
    [TASK2_CANONICAL_TYPES.CAUSES_SOLUTIONS]: buildProblemSolutionRouteAssessment,
    [TASK2_CANONICAL_TYPES.CAUSES_EFFECTS]: buildCausesEffectsRouteAssessment,
    [TASK2_CANONICAL_TYPES.POSITIVE_NEGATIVE]: buildPositiveNegativeRouteAssessment,
    [TASK2_CANONICAL_TYPES.DIRECT_QUESTION]: buildDirectQuestionRouteAssessment,
    [TASK2_CANONICAL_TYPES.HYBRID]: buildDirectQuestionRouteAssessment,
...
function finalizeRouteAssessment(context, assessment) {
  ...
  const bodyRoutes = assessment.bodyRoutes.map((item) => ({
    ...item,
    status: normalizeRouteCoverage(item.status)
  }));
  ...
  return {
    ...
    recommendedRouteRationale: buildTaskTypeRouteRationale(context.essayRoute, overallRouteStatus),
...
function buildTaskTypeRouteRationale(essayType, status) {
  return `${TASK2_TYPE_LABELS[essayType]} requires each prompt function to remain visible from the introduction through the conclusion. The current response route is ${routeCoverageLabel(status)}, so the revision should preserve developed ideas and repair only the missing or partial functions.`;
}
...
function buildCanonicalFrameworkAssessment(routeAssessment) {
  const developmentStatus = routeAssessment.bodyRoutes.some((item) => isPartialRouteStatus(item.status)) ? "Moderate" : "Strong";
