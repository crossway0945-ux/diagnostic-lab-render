  ...
  return async function handleApiRequest(request) {
    ...
    try {
      ...
      if (method === "GET" && apiPath === "/api/progress") {
        ...
        const validRecords = records.filter(isProgressEligibleRecord);
        return jsonResponse(200, {
          ok: true,
          ...
          summariesByTask: {
            "Task 1": buildServerProgressSummary(validRecords, null, "Task 1"),
            "Task 2": buildServerProgressSummary(validRecords, null, "Task 2")
          }
        });
      }
...
export function buildServerProgressSummary(records = [], currentRecord = null, taskType = "") {
  const eligible = stableProgressSort(records.filter((record) =>
    isProgressEligibleRecord(record) && (!taskType || record.taskType === taskType)
  ));
