...
assert.deepEqual(report.top3Issues.map((issue) => issue.issueType), [
  ...
  "Introduction and Conclusion Precision"
]);
assert.match(report.top3Issues[0].summary, /Both body paragraphs|both examples/i);
assert.match(report.top3Issues[4].summary, /introduction and conclusion/i);
...
const expectedCards = new Map([
  ...
  ["Lexical Precision and Word Formation", ["Route-Preserving Revision", "First of all, concentrating similar facilities in one zone could create serious commuting difficulties."]],
  ["Body 1 Explanation and Sentence Control", ["Route-Preserving Revision", "Families live in different parts of a city, so concentrating essential facilities in one zone could force many residents to travel long distances to reach them."]],
  ...
  ["Paragraph Closure and Link-Back", ["Route-Preserving Revision", "Therefore, concentrating similar facilities in one area could cause some residents to face significant travel difficulties."]],
  ...
  ["Conclusion Precision and Sentence Control", ["Route-Preserving Revision", "In conclusion, I firmly believe that urban areas should not be divided into zones according to facility type, because concentrating similar facilities in separate districts would reduce accessibility and worsen traffic congestion."]]
]);

assert.deepEqual(report.feedbackCards.map((card) => card.issueType), [...expectedCards.keys()]);
assert.equal(report.feedbackCards.length, 7);
...
assert.match(css, /font-family:\s*"Segoe UI", Tahoma, Arial, sans-serif/);
assert.match(css, /\.print-route-section\s*\{\s*margin-top:\s*0/);

console.log("V11.3 report completeness hotfix: seven evidence cards, five priorities, revision fidelity, PDF layout/text and report-version grouping checks passed.");
