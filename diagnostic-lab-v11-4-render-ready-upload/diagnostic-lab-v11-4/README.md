...
The V11.4 ready-upload ZIP is flat internally so Windows **Extract All** creates one project folder without a duplicated inner folder. Upload that single extracted folder as `diagnostic-lab-v11-4-final-ready-upload`, confirm `diagnostic-lab-v11-4-final-ready-upload/package.json` exists in GitHub, and set Render Root Directory to `diagnostic-lab-v11-4-final-ready-upload`. Do not upload two same-named folders nested inside one another, and do not include `.zip` in Root Directory.
