...
function renderPrintFeedbackCard(card) {
  ...
  return `<div class="print-feedback-card">
    <h3>${escapePrintHtml(card.issueType || "Diagnostic Issue")} <em class="print-badge ${statusClass(severity)}">${escapePrintHtml(severity)}</em></h3>
    <p><strong>Paragraph Location:</strong> ${escapePrintHtml(card.paragraphLocation || "-")}</p>
    <p><strong>Exact Sentence Found</strong></p>
    <blockquote>${escapePrintHtml(card.exactSentence || "-")}</blockquote>
    <p><strong>What This Sentence Is Trying To Do:</strong> ${escapePrintHtml(card.sentenceFunction || "-")}</p>
    <p><strong>Why This Limits the Band:</strong> ${escapePrintHtml(card.whyItLimitsBand || "-")}</p>
    <p><strong>Kru Pom Diagnosis:</strong> ${escapePrintHtml(card.kruPomDiagnosis || "-")}</p>
    ${card.revisionType ? `<p><strong>Revision Type:</strong> ${escapePrintHtml(card.revisionType)}</p>` : ""}
    <p><strong>Targeted Revision</strong></p>
    <div class="print-revision">${escapePrintHtml(card.targetedRevision || "-")}</div>
    <p><strong>Why This Revision Is Stronger:</strong> ${escapePrintHtml(card.whyRevisionIsStronger || "-")}</p>
    <p><strong>Student Action:</strong> ${escapePrintHtml(card.studentAction || "-")}</p>
  </div>`;
}
...
function renderPrintProgressSummary(taskType, records, analysis, serverSummary) {
  ...
  if (!summary.previousSubmissionCount) {
    return `<div class="print-summary-grid">
      ...
      ${renderPrintInfo("Report versions for latest essay", String(summary.reportVersionCount || 1))}
    </div>${renderPrintProgressProof(summary)}`;
  }
  return `<div class="print-summary-grid">
...
function renderPrintProgressProof(summary = {}) {
  const versions = Array.isArray(summary.reportVersions) ? summary.reportVersions : [];
  if (!versions.length && !summary.currentSubmissionGroupId) return "";
  return `<div class="print-progress-proof">
    <h3>Report-Version and Progress Proof</h3>
    <p><strong>Latest essay submissionGroupId:</strong> ${escapePrintHtml(summary.currentSubmissionGroupId || "-")}</p>
    <p><strong>Previous distinct response:</strong> ${escapePrintHtml(summary.previousSubmissionId || "none")} ${summary.previousSubmissionGroupId ? `(group ${escapePrintHtml(summary.previousSubmissionGroupId)})` : ""}</p>
    <p><strong>Distinct submissions in trend:</strong> ${escapePrintHtml(String(summary.distinctSubmissionCount || 0))}</p>
    <p><strong>Progress policy:</strong> ${summary.sameWorkRerunsExcluded ? "Same-work engine reruns are excluded from submission counts and student-progress trends." : "Progress grouping could not be verified."} ${summary.repeatedIssuesUseDistinctSubmissions ? "Repeated issues are counted across distinct submissions only." : ""}</p>
    ${versions.length ? `<div class="print-version-list">${versions.map((version) => `<p><strong>Version ${escapePrintHtml(String(version.version || "-"))}</strong> - report ${escapePrintHtml(version.submissionId || "-")} - group ${escapePrintHtml(version.submissionGroupId || summary.currentSubmissionGroupId || "-")} - ${version.progressEligible ? "progress representative" : "engine/report rerun excluded from progress"}</p>`).join("")}</div>` : ""}
  </div>`;
}
...
function normalizeProgressSummary(records, taskType, analysis, serverSummary) {
  if (serverSummary && (!serverSummary.taskType || serverSummary.taskType === taskType)) {
    return {
      ...
      reportVersionCount: Number(serverSummary.reportVersionCount || 1),
      reportVersions: Array.isArray(serverSummary.reportVersions) ? serverSummary.reportVersions : [],
