...
function buildSafeTask2TargetedRevision(sentence, card = {}) {
  ...
  if (/\bevery family is living in different places and distances\b/i.test(source)) {
    return "Families live in different parts of a city, and some may live far from essential facilities, making essential journeys more difficult.";
  }
  if (/\bspecific places like towns and cities\b/i.test(source) && /\bcongestion of traffic\b/i.test(source)) {
    return "In conclusion, I firmly believe that towns and cities should not concentrate similar facilities in separate zones, because this policy could make daily travel more difficult and increase traffic congestion.";
  }
...
function ensureTask2ReportCompleteness(output, used, payload) {
  ...
  const cards = [
    ...
    body1Topic && {
      ...
      targetedRevision: "First of all, concentrating similar facilities in one zone could create serious commuting difficulties.",
      revisionType: "Route-Preserving Revision",
    ...
    body1Explanation && {
      ...
      targetedRevision: "Families live in different parts of a city, so concentrating essential facilities in one zone could force many residents to travel long distances to reach them.",
      revisionType: "Route-Preserving Revision",
    ...
    body1Link && {
      ...
      targetedRevision: "Therefore, concentrating similar facilities in one area could cause some residents to face significant travel difficulties.",
      revisionType: "Route-Preserving Revision",
    ...
    conclusion && {
      ...
      targetedRevision: "In conclusion, I firmly believe that urban areas should not be divided into zones according to facility type, because concentrating similar facilities in separate districts would reduce accessibility and worsen traffic congestion.",
      revisionType: "Route-Preserving Revision",
  ...
  for (const card of cards) upsertTask2CoverageCard(output, used, card);
  const orderedEvidence = cards.map((card) => normalizeEvidenceText(card.exactSentence));
...
function buildCanonicalTask2PracticePlan(payload = {}, cards = []) {
  ...
  const plan = [
    firstDay,
    {
      ...
      task: "Rebuild the weaker example so it demonstrates an effect on a wider group rather than relying on one narrow or extreme case."
    },
    {
    ...
    },
    {
      ...
      task: "Rewrite the full response, verify the thesis-to-body route, and complete a final edit for development, lexical precision, grammar and sentence endings."
    }
  ];
...
function buildTask2CoverageTopIssueCards(cards, payload) {
  ...
  if (![body1Example, body2Example, lexical, grammar, closure, introduction, conclusion].every(Boolean)) return [];
  const evidenceItem = (card, evidenceRole) => ({
    paragraphLocation: card.paragraphLocation,
    exactSentence: card.exactSentence,
    evidenceRole
  });
  const lexicalCards = [introduction, lexical, body2Topic, body2Example, conclusion].filter(Boolean);
  ...
  return [
    {
      ...
      whyItLimitsBand: "Both body paragraphs support the correct thesis routes, but each example needs a more precise mechanism and credible wider urban consequence."
    },
    {
      ...lexical,
    ...
    {
      ...introduction,
      issueType: "Introduction and Conclusion Precision",
      scope: "multi-location",
      paragraphLocations: [introduction.paragraphLocation, conclusion.paragraphLocation],
      evidenceItems: [
        evidenceItem(introduction, "Introduction framing evidence"),
        evidenceItem(conclusion, "Conclusion closure evidence")
      ],
      whyItLimitsBand: "The introduction and conclusion preserve the correct position route, but both use imprecise nouns or clause control that weakens the framing and closure."
    }
  ];
}
