...
The V11.6 upload ZIP contains exactly one project folder named `diagnostic-lab-v11-6-final-upload`. Upload that folder directly to the GitHub repository root, confirm `diagnostic-lab-v11-6-final-upload/package.json` exists, and set Render Root Directory to `diagnostic-lab-v11-6-final-upload`. Do not upload an extra wrapper folder and do not include `.zip` in Root Directory.
