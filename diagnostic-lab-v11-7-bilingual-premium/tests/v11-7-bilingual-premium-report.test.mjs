...
assert.match(englishReport.practicePlan[1].task, /Rebuild both examples/i);

const thesisCard = englishReport.feedbackCards.find((card) => card.issueType === "Thesis Language Precision");
assert.ok(thesisCard);
assert.equal(
  thesisCard.targetedRevision,
  "However, I strongly disagree because this arrangement could make travel less accessible and worsen traffic congestion."
);
const conclusionCard = englishReport.feedbackCards.find((card) => card.issueType === "Conclusion Precision and Sentence Control");
assert.ok(conclusionCard);
assert.equal(
  conclusionCard.targetedRevision,
  "In conclusion, I firmly believe that urban areas should not be divided into separate zones for different types of facilities, because this would make travel less accessible and worsen traffic congestion."
);
...
for (const html of [rootHtml, previewHtml]) {
  ...
  assert.match(html, /diagnostic-v11-7-bilingual-premium/);
}
