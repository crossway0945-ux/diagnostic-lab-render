...
export function buildCanonicalAnalysis({
  ...
  if (taskType === "Task 2" && analysis.canonicalTask2Analysis) {
    ...
    const display = projectCanonicalTask2Framework(base, payload.reportLanguage || base.metadata?.reportLanguage);
    return {
      ...base,
      metadata: {
        ...base.metadata,
...
export function projectCanonicalTask2Framework(canonical = {}, reportLanguage = "") {
  ...
  const thesisDiagnosis = positionClear
    ? (thai
        ? `Introduction ระบุจุดยืน ${route.position} ชัด และวาง route ของ Body ตามหน้าที่ของโจทย์ โดยประเมินความชัดของ route แยกจากความแม่นยำของภาษา`
        : `The introduction states ${route.position} and establishes the body route required by the task; route clarity is assessed separately from language precision.`)
    : (thai ? "Thesis ยังวางจุดยืนหรือ route ที่โจทย์ต้องการไม่ชัดพอ" : "The thesis does not yet establish the required position or task route clearly.");
  const bodyRouteDiagnosis = (route.bodyRoutes || []).length
    ? (route.bodyRoutes || []).map((item) => `Body ${item.index}: ${item.label} [${String(item.developmentStatus || item.status || "").replaceAll("_", " ")}]`).join(" | ")
    : (thai ? "ยังไม่พบ Body route ที่ตรวจสอบได้" : "No verifiable body route was detected.");
  const generated = {
    "Essay Type Recognition": {
      status: "Strong",
      diagnosis: thai
        ? `ระบบจำแนกโจทย์เป็น ${canonical.metadata?.essayTypeLabel || "Task 2"} จากถ้อยคำในโจทย์โดยตรง`
        : `The prompt is classified as ${canonical.metadata?.essayTypeLabel || "Task 2"} from its own wording.`
    },
    "Prompt Coverage": {
      status: routeStatus,
      diagnosis: route.missingRequirements?.length
        ...
        : /Moderate/i.test(routeStatus)
          ? (thai ? "มี route ที่โจทย์กำหนดครบ แต่หนึ่ง route ยังพัฒนาเพียงบางส่วน" : "All required routes are present, but one route is only partially developed.")
          : (thai ? "ครอบคลุม route ที่โจทย์กำหนดครบ" : "All required routes are covered.")
    },
    "Position Clarity": canonical.taskRequirements?.stanceRequired
      ? {
          status: positionClear ? "Strong" : "Needs Work",
          diagnosis: positionClear
            ? (thai ? "ผู้เขียนสื่อ judgement ที่โจทย์ต้องการได้ชัดและสม่ำเสมอ" : "The writer clearly and consistently communicates the required judgement.")
    ...
    "Explanation Depth": {
      ...
      diagnosis: partialConcession
        ? (thai ? "Concession เกี่ยวข้องกับโจทย์ แต่ยังอธิบายความสัมพันธ์กับ judgement หลักของผู้เขียนไม่ครบ" : "The concession is relevant, but its relationship to the writer's main judgement is not fully explained.")
        : (thai ? "Body route เกี่ยวข้องกับโจทย์ แต่ causal mechanism และผลลัพธ์ยังต้องเชื่อมให้แม่นและครบตามหลักฐาน" : "The body routes are relevant, but the causal mechanisms and consequences need more precise evidence-based development.")
    },
    "SAR Example Quality": {
      status: framework.sarExampleQuality?.status || routeStatus,
      diagnosis: thai ? "ตัวอย่างต้องพิสูจน์ claim ของแต่ละ Body ด้วย situation, mechanism และ result ที่สมเหตุสมผล ไม่ใช่เพียงเพิ่มรายละเอียดให้ยาวขึ้น" : "Examples should prove each body claim through a credible situation, mechanism and result rather than merely adding length."
    },
    "Link Back Control": {
      status: framework.linkBackControl?.status || routeStatus,
      diagnosis: partialConcession
        ? (thai ? "Concession ต้องพากลับไปหา judgement หลักของผู้เขียนอย่างชัดเจน" : "The concession needs an explicit return to the writer's main judgement.")
        : (thai ? "Body 1 พยายาม Link-Back แล้วแต่จบประโยคไม่สมบูรณ์และใช้คำไม่แม่น ส่วน Body 2 จบด้วย result แต่ยังต้องปิดย่อหน้าให้กลับไปหาเหตุผลหลักชัดขึ้น" : "Body 1 attempts a link-back but has an incomplete sentence ending and imprecise wording. Body 2 ends with a result but needs a cleaner paragraph-level return to the reason.")
    },
    "Conclusion Closure": {
      ...
      diagnosis: conclusionStatus === "Strong"
        ...
        : conclusionStatus === "Moderate"
          ? (thai ? "Conclusion ยังรักษา route หลักได้ แต่ policy reference, clause control หรือการ match กับ Body ยังต้องซ่อม" : "The conclusion broadly preserves the route, but policy reference, clause control or body-route matching still needs repair.")
          : (thai ? "Conclusion ขาด ไม่สมบูรณ์ ขัดกับจุดยืน หรือยังปิด route ที่โจทย์ต้องการไม่ได้" : "The conclusion is missing, unfinished, contradictory, or does not close the required route.")
    },
    "LFC CPC Control": {
      status: framework.lfcCpcControl?.status || routeStatus,
      diagnosis: thai ? "มองเห็น paragraph route ได้ แต่ sentence-level language และ paragraph closure ยังควบคุมไม่สม่ำเสมอ" : "The paragraph routes are visible, but sentence-level language and paragraph closure remain inconsistent."
    }
  };
  const calibratedDisplay = Object.fromEntries(
    Object.keys(generated)
      .filter((name) => framework.display?.[name])
      .map((name) => [name, framework.display[name]])
  );
  return {
    ...generated,
    ...calibratedDisplay
