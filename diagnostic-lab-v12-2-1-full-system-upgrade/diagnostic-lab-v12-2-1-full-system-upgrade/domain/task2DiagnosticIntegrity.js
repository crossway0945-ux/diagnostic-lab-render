...
export function applyTask2FullSystemUpgrade({ payload = {}, analysis = {}, feedbackCards = [], paragraphFeedback = [], practicePlan = [] } = {}) {
  ...
  cards = ensureHighBandRefinementCoverage(cards, records, safety, payload);
  ...
  const executive = buildExecutiveSummary({ analysis, safety, conclusionIntegrity, payload, cards });
  const canonicalTask2Analysis = upgradeCanonicalTask2(analysis.canonicalTask2Analysis, {
    ...
    paragraphFeedback: buildParagraphFeedback(records, cards, payload.reportLanguage, safety),
  ...
  const alignedParagraphFeedback = buildParagraphFeedback(records, cards, payload.reportLanguage, safety);
...
export function assessConclusionIntegrity({ payload = {}, safety = null, records = null } = {}) {
  ...
  const conclusionParagraphIndex = map.paragraphs.length - 1;
  const conclusionRecords = map.sentences.filter((item) => item.paragraphIndex === conclusionParagraphIndex);
  if (!conclusionRecords.length) return { status: "Needs Work", present: false, complete: false, stanceConsistent: false, routeOverlap: 0, policyContradiction: false, reasons: ["missing conclusion"] };
  const conclusionText = conclusionRecords.map((item) => item.sentence).join(" ").trim();
  const finalRecord = conclusionRecords.at(-1);
  const positionRecord = conclusionRecords.find((item) => /\b(?:agree|disagree|i believe|in my (?:view|opinion)|valuable|worthwhile|outweigh)\b/i.test(item.sentence)) || conclusionRecords[0];
  const complete = /[.!?]["')\]]?$/.test(String(finalRecord?.sentence || "").trim()) && !/[,;:]\s*$/.test(String(finalRecord?.sentence || "").trim());
  ...
  const routeOverlap = tokenOverlap(routeText, conclusionText);
  ...
  const policyContradiction = detectNegationContradiction(conclusionText, payload.prompt);
  const conclusionLanguageIssues = (effectiveSafety.languageProfile?.validatedIssues || []).filter((item) => item.paragraphLocation === "Conclusion" || Number(item.paragraphIndex) === conclusionParagraphIndex).length;
  const reasons = [];
  ...
  return {
    ...
    policyContradiction,
    conclusionLanguageIssues,
    reasons,
    exactSentence: positionRecord?.sentence || finalRecord?.sentence || conclusionText,
    finalSentence: finalRecord?.sentence || "",
    paragraphText: conclusionText,
    paragraphLocation: positionRecord?.location || "Conclusion"
  };
}

function upgradeCanonicalTask2(canonical, { criteriaScores, kruPomScores, estimatedBandRange, conclusionIntegrity, executive, cards, topIssues, paragraphFeedback, practicePlan }) {
  ...
  next.frameworkAssessment = {
    ...
    display: {
      ...
      ...(kruPomScores || {}),
      "Conclusion Closure": kruPomScores?.["Conclusion Closure"] || {
        status: conclusionIntegrity.status,
        diagnosis: conclusionIntegrity.reasons.join("; ")
      }
    }
  };
...
function rejectStaleProviderCards(cards, safety, payload) {
  ...
  const secureHighBand = Boolean(safety.languageProfile?.secureBand7Profile) &&
    !safety.developmentRisk?.hasModerateDevelopment &&
    !safety.languageAccuracyRisk?.blocksSecureBand7;
  return cards.filter((card) => {
    const issueType = String(card.issueType || "");
    const exact = String(card.exactSentence || "").trim();
    if (/topic sentence route alignment|body paragraph route alignment/i.test(issueType)) {
      ...
      if (routeControlled && !safety.routeConflict) return false;
    }
    if (secureHighBand && /SAR Example Failure|weak example|example development/i.test(issueType)) {
      const concreteList = (exact.match(/,/g) || []).length >= 2 || /\b(?:GPS|climate|weather forecasts?|communication|hospitals?|schools?|satellites?|poverty|healthcare|education)\b/i.test(exact);
      const explicitExample = /^(?:for example|for instance|such as)\b/i.test(exact);
      const vagueOnly = /\b(?:many people|people use|students use|good things|useful information|some things)\b/i.test(exact);
      if (explicitExample && exact.length >= 85 && concreteList && !vagueOnly) return false;
    }
    return true;
  });
}
...
function normalizeCardLanguage(card, payload) {
  ...
  const learnerFields = [card.sentenceFunction, card.whyItLimitsBand, card.kruPomDiagnosis, card.whyRevisionIsStronger, card.studentAction];
  if (thaiReport || !learnerFields.some(hasThai)) return card;
  const label = String(card.issueType || "this control point");
  const location = String(card.paragraphLocation || "the paragraph");
  return {
    ...card,
    sentenceFunction: hasThai(card.sentenceFunction) ? `This sentence is being evaluated for ${label.toLowerCase()} within the task route.` : card.sentenceFunction,
    whyItLimitsBand: hasThai(card.whyItLimitsBand) ? `At ${location}, the sentence does not yet express its function with enough precision, development or language control for a secure higher-band response.` : card.whyItLimitsBand,
    kruPomDiagnosis: hasThai(card.kruPomDiagnosis) ? `Keep the valid paragraph route at ${location}, but repair the specific ${label.toLowerCase()} weakness without replacing the student's central claim.` : card.kruPomDiagnosis,
    whyRevisionIsStronger: hasThai(card.whyRevisionIsStronger) ? "The revision preserves the student's route while making the diagnosed control point clearer and more precise." : card.whyRevisionIsStronger,
    studentAction: hasThai(card.studentAction) ? `Revise the ${label.toLowerCase()} issue at ${location}, then check the same control point throughout the response.` : card.studentAction
  };
}
...
function ensureHighBandRefinementCoverage(cards, records, safety, payload) {
  const secureHighBand = Boolean(safety.languageProfile?.secureBand7Profile) &&
    !safety.developmentRisk?.hasModerateDevelopment &&
    !safety.languageAccuracyRisk?.blocksSecureBand7 &&
    !safety.routeConflict;
  if (!secureHighBand) return cards;
  const output = [...cards];
  const thai = String(payload.reportLanguage || "").toLowerCase() === "th";
  const alreadyHas = (record, pattern) => output.some((card) =>
    normalize(card.exactSentence) === normalize(record?.sentence) || pattern.test(String(card.issueType || ""))
  );
  const introRecords = records.sentences.filter((item) => item.paragraphIndex === 0);
  const thesis = introRecords.find((item) => /\b(?:agree|disagree|i believe|in my (?:view|opinion)|valuable|worthwhile)\b/i.test(item.sentence)) || introRecords.at(-1);
  const broadRoadmap = thesis && /\b(?:new technology|useful knowledge|future opportunities|useful results|many benefits|good things|important benefits)\b/i.test(thesis.sentence);
  if (broadRoadmap && !alreadyHas(thesis, /thesis.*precision|vocabulary precision/i)) {
    let revision = String(thesis.sentence)
      .replace(/new technology/gi, "practical technologies")
      .replace(/useful knowledge/gi, /\bclimate\b/i.test(payload.writing || "") ? "climate knowledge" : "scientific knowledge")
      .replace(/future opportunities/gi, "long-term scientific innovation")
      .replace(/useful results/gi, "practical benefits")
      .replace(/many benefits/gi, "specific long-term benefits")
      .replace(/good things/gi, "practical benefits")
      .replace(/important benefits/gi, "specific benefits");
    output.push({
      _integrityGenerated: true,
      issueType: thai ? "ความแม่นยำของ Vocabulary ใน Thesis" : "Thesis Vocabulary Precision",
      severity: "High-Band Refinement",
      criteria: ["Task Response", "Lexical Resource"],
      framework: ["Thesis Route Clarity", "Vocabulary Precision", "Clear / Precise"],
      paragraphLocation: thesis.location,
      exactSentence: thesis.sentence,
      sentenceFunction: thai ? "Thesis นี้ระบุจุดยืนและ preview เหตุผลหลักของ Body" : "This thesis states the writer's position and previews the main body reasons.",
      whyItLimitsBand: thai ? "Route ชัดแล้ว แต่บาง noun phrase ยังกว้างกว่าหมวดหลักฐานที่พัฒนาใน Body" : "The route is clear, but several noun phrases are broader than the specific evidence categories developed in the body paragraphs.",
      kruPomDiagnosis: thai ? "Golden Thread แข็งแรงแล้ว จุดที่เหลือคือทำ roadmap ใน Thesis ให้ใช้หมวด concept ที่ตรงกับ Body มากขึ้น" : "The Golden Thread is controlled; the remaining refinement is to make the thesis roadmap name the same concept categories developed in the body paragraphs.",
      targetedRevision: revision,
      revisionType: "High-Band Refinement",
      whyRevisionIsStronger: thai ? "เวอร์ชันนี้รักษาจุดยืนและ route เดิม พร้อมแทนคำนามกว้างด้วยหมวดเหตุผลที่เฉพาะขึ้น" : "The revision preserves the position and route while replacing broad roadmap nouns with more specific academic categories.",
      studentAction: thai ? "ขีดเส้นใต้คำนามใน Thesis แล้วตรวจว่าทุกคำมีหลักฐานตรงกันใน Body" : "Underline the roadmap nouns in the thesis and verify that each one has matching evidence in the body paragraphs."
    });
  }

  const conclusionIndex = records.paragraphs.length - 1;
  const conclusionRecords = records.sentences.filter((item) => item.paragraphIndex === conclusionIndex);
  const finalSentence = conclusionRecords.at(-1);
  const earlierText = records.sentences.filter((item) => item.paragraphIndex < conclusionIndex).map((item) => item.sentence).join(" ");
  const timeFrameMismatch = finalSentence && /\bnear future\b/i.test(finalSentence.sentence) && /\b(?:long run|long term|long-term|future generations?)\b/i.test(earlierText);
  if (timeFrameMismatch && !alreadyHas(finalSentence, /conclusion.*(?:time|closure|consistency)/i)) {
    output.push({
      _integrityGenerated: true,
      issueType: thai ? "ความสอดคล้องของช่วงเวลาใน Conclusion" : "Conclusion Time-Frame Consistency",
      severity: "High-Band Refinement",
      criteria: ["Task Response", "Coherence & Cohesion", "Lexical Resource"],
      framework: ["Conclusion Closure", "Vocabulary Precision", "LFC CPC Control"],
      paragraphLocation: finalSentence.location,
      exactSentence: finalSentence.sentence,
      sentenceFunction: thai ? "ประโยคนี้ปิดงานด้วย balanced spending approach" : "This sentence closes the essay by restating the balanced spending approach.",
      whyItLimitsBand: thai ? "Conclusion สอดคล้องกับจุดยืน แต่คำว่า near future แคบกว่า long-term route ที่วางไว้ก่อนหน้า" : "The conclusion is consistent, but 'near future' slightly narrows the long-term route established earlier in the essay.",
      kruPomDiagnosis: thai ? "Closure สำเร็จแล้ว เหลือเพียงทำ time frame ให้ตรงกับ Thesis และ Body" : "Closure is successful; the final time frame should match the thesis and developed body route more closely.",
      targetedRevision: finalSentence.sentence.replace(/\bnear future\b/gi, "long run"),
      revisionType: "High-Band Refinement",
      whyRevisionIsStronger: thai ? "เวอร์ชันนี้รักษา balanced conclusion เดิมและทำช่วงเวลาให้ตรงกับ route ก่อนหน้า" : "The revision preserves the balanced conclusion and aligns its time frame with the established route.",
      studentAction: thai ? "ตรวจว่าคำบอกช่วงเวลาใน Conclusion ตรงกับ Thesis และ Body" : "Check that the conclusion uses the same time frame as the thesis and body paragraphs."
    });
  }
  return output;
}
...
function calibrateFramework(input, safety, conclusionIntegrity, language) {
  ...
  const bodyRoutes = safety.routeAssessment?.bodyRoutes || [];
  const positionClear = !safety.stanceRequired || !["unclear", "contradictory", ""].includes(String(safety.detectedPosition || ""));
  output["Position Clarity"] = {
    ...(output["Position Clarity"] || {}),
    status: !safety.stanceRequired ? "Not Applicable" : positionClear && ["high", "medium"].includes(safety.positionConfidence) ? "Strong" : "Needs Work",
    diagnosis: !safety.stanceRequired
      ? (thai ? "โจทย์ประเภทนี้ไม่บังคับให้ระบุจุดยืน" : "This task family does not require an opinion position.")
      : positionClear
        ? (thai ? `จุดยืน ${safety.detectedPosition} มองเห็นได้และสอดคล้องกับ route` : `The ${safety.detectedPosition} position is identifiable and consistent with the essay route.`)
        : (output["Position Clarity"]?.diagnosis || "The position is not controlled consistently.")
  };
  const thesisRouteStrong = /adequately_developed|fully_extended/.test(String(safety.routeAssessment?.thesisRouteStatus || ""));
  output["Thesis Route Clarity"] = {
    ...(output["Thesis Route Clarity"] || {}),
    status: thesisRouteStrong ? "Strong" : "Moderate",
    diagnosis: thesisRouteStrong
      ? (thai ? `Introduction ระบุจุดยืน ${safety.detectedPosition || "ชัดเจน"} และวาง route ของ Body ได้สอดคล้องกับโจทย์` : `The introduction states ${safety.detectedPosition || "a clear position"} and establishes a body route consistent with the task.`)
      : (thai ? "Thesis ระบุจุดยืนได้ แต่ route ของ Body ยังต้องทำให้แม่นหรือครบขึ้น" : "The thesis communicates a position, but the body route still needs greater precision or completeness.")
  };
  ...
  output["Explanation Depth"] = {
    ...(output["Explanation Depth"] || {}),
    status: safety.developmentRisk?.hasModerateDevelopment ? "Moderate" : "Strong",
    diagnosis: safety.developmentRisk?.hasModerateDevelopment
      ? (thai ? "อย่างน้อยหนึ่ง Body ยังต้องทำ causal mechanism หรือผลที่ตามมาให้แม่นขึ้น" : "At least one body paragraph still needs a more precise causal mechanism or consequence.")
      : (thai ? "แต่ละ Body อธิบายเหตุผลด้วยกลไกและผลที่เกี่ยวข้องอย่างเพียงพอ" : "The body paragraphs explain their controlling reasons with relevant mechanisms and consequences.")
  };
  output["SAR Example Quality"] = {
    ...(output["SAR Example Quality"] || {}),
    status: safety.developmentRisk?.hasModerateDevelopment ? "Moderate" : "Strong",
    diagnosis: safety.developmentRisk?.hasModerateDevelopment
      ? (thai ? "อย่างน้อยหนึ่งตัวอย่างยังต้องทำ situation, mechanism และ result ให้ชัดขึ้น" : "At least one example still needs a clearer situation, mechanism and result.")
      : (thai ? "ตัวอย่างมีความเฉพาะเจาะจง เกี่ยวข้องกับ claim และช่วยพิสูจน์ย่อหน้าได้" : "The examples are specific, relevant to their claims and provide credible support for the paragraph routes.")
  };
  const linkBackWeak = Boolean(safety.languageProfile?.sentenceCompletionErrors?.length) || bodyRoutes.some((item) => /partial|mentioned|conflict|unclear/.test(String(item.status || item.developmentStatus || "")));
  output["Link Back Control"] = {
    ...(output["Link Back Control"] || {}),
    status: linkBackWeak ? "Moderate" : "Strong",
    diagnosis: linkBackWeak
      ? (thai ? "อย่างน้อยหนึ่งย่อหน้ายังปิดหรือเชื่อมกลับไปหา controlling idea ได้ไม่เต็มที่" : "At least one paragraph does not yet close or return to its controlling idea securely.")
      : (thai ? "แต่ละ Body รักษา controlling idea ได้ตลอดย่อหน้า แม้ไม่จำเป็นต้องมีประโยค Link-Back แยกทุกครั้ง" : "Each body paragraph maintains its controlling idea throughout; a separate formulaic link-back sentence is not required.")
  };
  ...
  output["LFC CPC Control"] = {
    ...(output["LFC CPC Control"] || {}),
    status: safety.languageAccuracyRisk?.blocksSecureBand7 ? "Moderate" : "Strong",
    diagnosis: safety.languageAccuracyRisk?.blocksSecureBand7
      ? (thai ? "มี language pattern ที่เกิดซ้ำและยังลดความชัดหรือความแม่นยำของงาน" : "Recurring language patterns still reduce clarity or precision across the response.")
      : (thai ? "งานรักษา Link, Flow, Clarity, Concision, Precision และ Completeness ได้ดีโดยรวม" : "The response maintains strong overall control of link, flow, clarity, concision, precision and completeness.")
  };
  return output;
}

function buildExecutiveSummary({ analysis, safety, conclusionIntegrity, payload, cards = [] }) {
  const thai = String(payload.reportLanguage || "").toLowerCase() === "th";
  ...
  const substantiveCards = cards.filter((card) => !/high-band refinement/i.test(`${card.severity || ""} ${card.revisionType || ""}`));
  const parsedCriteria = ["Task Response", "Coherence & Cohesion", "Lexical Resource", "Grammatical Range & Accuracy"]
    .map((name) => parseRange(analysis.criteriaScores?.[name]?.range || analysis.criteriaScores?.[name]))
    .filter(Boolean);
  const averageLow = parsedCriteria.length ? parsedCriteria.reduce((sum, item) => sum + item.low, 0) / parsedCriteria.length : 0;
  const routeControlled = !safety.routeConflict && /controlled|adequately_developed|fully_extended/.test(String(safety.routeAssessment?.status || safety.routeAssessment?.overallRouteStatus || ""));
  const highBandProfile = (Boolean(safety.languageProfile?.secureBand7Profile) || averageLow >= 6.75) && routeControlled && !safety.languageAccuracyRisk?.blocksSecureBand7 && substantiveCards.length === 0;
  const partialPosition = /partly|partially/.test(String(safety.detectedPosition || ""));

  if (highBandProfile) {
    if (thai) {
      return {
        mainScoreLimitingFactor: `งานมีจุดยืน ${safety.detectedPosition || "ชัดเจน"} และ paragraph route ควบคุมได้ดี จุดที่เหลือเป็นการเกลาภาษาระดับสูง ได้แก่ความเฉพาะเจาะจงของ Thesis และความสอดคล้องของถ้อยคำใน Conclusion ไม่ใช่ความเสียหายของโครงสร้างหลัก`,
        mostUrgentRepair: partialPosition
          ? "คงโครงสร้าง concession -> writer position เดิมไว้ แล้วทำ Thesis ให้ระบุหมวดเหตุผลจาก Body แม่นขึ้น พร้อมทำให้ช่วงเวลาใน Conclusion ตรงกับ long-term route เดิม"
          : "คง route เดิมไว้ แล้วเกลา Thesis และ Conclusion ให้ใช้ concept และช่วงเวลาเดียวกับ Body โดยไม่เพิ่ม premise ใหม่"
      };
    }
    return {
      mainScoreLimitingFactor: `The essay has a controlled ${safety.detectedPosition || "opinion"} route and relevant development. The remaining limitation is high-band precision: the thesis roadmap and conclusion wording can match the body categories and time frame more exactly.`,
      mostUrgentRepair: partialPosition
        ? "Preserve the concession-to-writer-position structure. Tighten the thesis categories and make the conclusion use the same long-term time frame as the developed argument."
        : "Preserve the existing route and refine the thesis and conclusion so their concepts and time frame match the body paragraphs exactly."
    };
  }
  ...
  if (!bodyModerate && conclusionIntegrity.status === "Strong" && analysis.mainScoreLimitingFactor && analysis.mostUrgentRepair) {
    return { mainScoreLimitingFactor: analysis.mainScoreLimitingFactor, mostUrgentRepair: analysis.mostUrgentRepair };
  }
  if (thai) {
    return {
      mainScoreLimitingFactor: `งานมี route หลักที่มองเห็นได้ แต่ ${bodyModerate || "บาง"} Body ยังอธิบายกลไกหรือตัวอย่างไม่แม่นพอ${conclusionIntegrity.status !== "Strong" ? " และ Conclusion ยังปิด policy/route ได้ไม่สมบูรณ์" : ""}${safety.languageAccuracyRisk?.blocksSecureBand7 ? " ขณะเดียวกัน lexical และ grammar error ที่เกิดซ้ำยังขัดขวาง Band 7 ที่มั่นคง" : ""}`,
      mostUrgentRepair: "คง route ที่ถูกต้องไว้ แล้วซ่อมเฉพาะ Body หรือ Conclusion ที่มีหลักฐานว่าอ่อน ก่อนตรวจ language pattern ที่เกิดซ้ำทั้งงาน"
    };
  }
  return {
    mainScoreLimitingFactor: `The main route is visible, but ${bodyModerate || "some"} body paragraph${bodyModerate === 1 ? "" : "s"} still need more precise development${conclusionIntegrity.status !== "Strong" ? ", and the conclusion does not yet close the route securely" : ""}${safety.languageAccuracyRisk?.blocksSecureBand7 ? ". Recurring lexical and grammatical errors also prevent a secure Band 7 profile" : ""}.`,
    mostUrgentRepair: "Keep the valid route and repair only the body or conclusion weaknesses supported by exact evidence, then scan the full response for recurring language patterns."
  };
}
...
function buildParagraphFeedback(records, cards, language, safety = {}) {
  const thai = String(language || "").toLowerCase() === "th";
  const bodyRouteMap = new Map((safety.routeAssessment?.bodyRoutes || []).map((item) => [Number(item.index), item]));
  return records.paragraphs.map((paragraph, paragraphIndex) => {
    ...
    const relevant = cards.filter((card) => String(card.paragraphLocation || "").startsWith(name));
    const sentences = splitSentences(paragraph);
    const positionSentence = sentences.find((sentence) => /\b(?:agree|disagree|i believe|in my (?:view|opinion)|outweigh|valuable|worthwhile)\b/i.test(sentence));
    const exactEvidence = relevant[0]?.exactSentence || (name === "Introduction" ? positionSentence || sentences.at(-1) : sentences[0]) || paragraph;
    if (relevant.length) {
      return {
        paragraphLocation: name,
        exactEvidence,
        diagnosis: dedupeJoined(relevant.map((item) => item.kruPomDiagnosis)),
        action: dedupeJoined(relevant.map((item) => item.studentAction))
      };
    }

    if (name === "Introduction") {
      const clear = !safety.stanceRequired || !["unclear", "contradictory", ""].includes(String(safety.detectedPosition || ""));
      return {
        paragraphLocation: name,
        exactEvidence,
        diagnosis: thai
          ? clear ? `Introduction ระบุจุดยืน ${safety.detectedPosition || "ตามโจทย์"} และวาง route ที่ Body สามารถพัฒนาต่อได้` : "Introduction ยังไม่ล็อกจุดยืนหรือ task route ได้ชัดเจน"
          : clear ? `The introduction establishes a clear ${safety.detectedPosition || "task"} position and provides a usable route for the body paragraphs.` : "The introduction does not yet establish a controlled position or task route.",
        action: thai
          ? clear ? "รักษาจุดยืนเดิมไว้ และตรวจให้คำสำคัญใน Thesis ตรงกับหมวดเหตุผลที่ Body พิสูจน์จริง" : "เขียน Thesis ใหม่ให้ระบุจุดยืนและ route ที่ Body จะพิสูจน์"
          : clear ? "Preserve the position and verify that the thesis categories match the exact ideas developed in the body paragraphs." : "Rewrite the thesis so it states the position and the route the body will prove."
      };
    }

    if (name === "Conclusion") {
      const conclusionStatus = safety.routeAssessment?.conclusionLabel || safety.conclusionStatus || "";
      const strong = /restates|controlled|complete|consistent|adequately/i.test(conclusionStatus) || !safety.unfinishedEndingDetected;
      return {
        paragraphLocation: name,
        exactEvidence,
        diagnosis: thai
          ? strong ? "Conclusion รักษาจุดยืนและปิด route หลักได้ครบ โดยไม่เปลี่ยน task family" : "Conclusion ยังไม่ restate จุดยืนหรือ route เดิมได้ครบ"
          : strong ? "The conclusion preserves the position and closes the established route without changing the task family." : "The conclusion does not yet restate the position or established route securely.",
        action: thai
          ? strong ? "ตรวจเฉพาะความสอดคล้องของคำสำคัญและช่วงเวลากับ Thesis และ Body ก่อนส่ง" : "เขียน Conclusion ใหม่ให้ใช้จุดยืนและเหตุผลเดิมจาก Body"
          : strong ? "Check only that its key terms and time frame match the thesis and body paragraphs before submission." : "Rewrite the conclusion using the same position and reasons developed in the body."
      };
    }

    const route = bodyRouteMap.get(paragraphIndex);
    const label = String(route?.label || "");
    const controlled = /adequate|controlled|aligned/.test(String(route?.status || route?.alignmentStatus || ""));
    const isConcession = /concession|opposing case/.test(label);
    return {
      paragraphLocation: name,
      exactEvidence,
      diagnosis: thai
        ? isConcession
          ? controlled ? "ย่อหน้านี้พัฒนาเหตุผลฝั่งที่ผู้เขียนยอมรับเป็น concession ได้ชัด และใช้หลักฐานที่เกี่ยวข้องกับ partial-position route" : "ย่อหน้านี้เป็น concession ที่เกี่ยวข้อง แต่ความสัมพันธ์กับจุดยืนสุดท้ายยังไม่ชัดพอ"
          : controlled ? "ย่อหน้านี้พัฒนาเหตุผลที่สอดคล้องกับจุดยืนของผู้เขียน และมีตัวอย่างหรือกลไกรองรับอย่างเป็นลำดับ" : "เหตุผลของย่อหน้านี้เกี่ยวข้อง แต่ route หรือ development ยังไม่ควบคุมเต็มที่"
        : isConcession
          ? controlled ? "This paragraph develops the acknowledged opposing case as a valid concession within the partial-position route and supports it with relevant evidence." : "This is a relevant concession, but its relationship to the final position is not yet fully controlled."
          : controlled ? "This paragraph develops a reason aligned with the writer's position and supports it through a coherent sequence of explanation and evidence." : "The paragraph is relevant, but its route or development is not yet fully controlled.",
      action: thai
        ? isConcession
          ? controlled ? "คง concession นี้ไว้ และเพิ่ม bridge สั้น ๆ เฉพาะเมื่อจำเป็นเพื่อเชื่อมกลับไปหาจุดยืนสุดท้าย" : "เพิ่มประโยค bridge ที่อธิบายว่า concession นี้สัมพันธ์กับ judgement สุดท้ายอย่างไร"
          : controlled ? "รักษา route เดิมไว้ แล้วตรวจให้ Topic Sentence, explanation และตัวอย่างใช้ concept เดียวกัน" : "เขียน Topic Sentence และ Explanation ใหม่ให้พิสูจน์ route เดียวกันอย่างชัดเจน"
        : isConcession
          ? controlled ? "Keep this concession and add a brief bridge only if needed to connect it more explicitly to the final judgement." : "Add a bridge explaining how this concession relates to the final judgement."
          : controlled ? "Preserve the route and verify that the topic sentence, explanation and examples use the same controlling concept." : "Rewrite the topic sentence and explanation so they prove one consistent route."
    };
  });
}
...
function buildRepairPlan(cards, safety, conclusionIntegrity, language, existing = []) {
  const thai = String(language || "").toLowerCase() === "th";
  const cardBy = (pattern) => cards.find((card) => pattern.test(`${card.issueType || ""} ${card.paragraphLocation || ""} ${(card.framework || []).join(" ")}`));
  const thesisCard = cardBy(/thesis|introduction/i);
  const body1Card = cards.find((card) => /^Body Paragraph 1/.test(String(card.paragraphLocation || "")));
  const body2Card = cards.find((card) => /^Body Paragraph 2/.test(String(card.paragraphLocation || "")));
  const languageCard = cards.find((card) => /lexical|vocabulary|grammar|sentence control|collocation|word form|punctuation/i.test(`${card.issueType || ""} ${(card.framework || []).join(" ")}`) && !/^(?:Introduction|Conclusion)/.test(String(card.paragraphLocation || "")));
  const conclusionCard = cardBy(/conclusion|closure/i);
  const partialPosition = /partly|partially/.test(String(safety.detectedPosition || ""));
  const bodyRoutes = safety.routeAssessment?.bodyRoutes || [];
  const route1 = bodyRoutes.find((item) => Number(item.index) === 1);
  const route2 = bodyRoutes.find((item) => Number(item.index) === 2);
  const fallbackAction = (card, fallback) => String(card?.studentAction || fallback).trim();
  const highBandProfile = Boolean(safety.languageProfile?.secureBand7Profile) &&
    !safety.developmentRisk?.hasModerateDevelopment &&
    !safety.languageAccuracyRisk?.blocksSecureBand7;

  if (!highBandProfile && !partialPosition) {
    const lowMidEn = [
      ["Map and preserve the accepted task route", "Write the function of the introduction, Body 1, Body 2 and conclusion in one line each. Keep the current position and thesis-to-body route, then label the existing controlling reason in each body paragraph before revising development."],
      ["Repair body explanations", "Rewrite each body explanation as affected group -> mechanism or change -> consequence, and make each causal chain return to the reason already stated in the thesis."],
      ["Upgrade both examples", "Rebuild the examples in both body paragraphs so each one shows a credible situation, a precise mechanism and a result that supports the paragraph claim without an unsupported extreme outcome."],
      [languageCard ? "Repair lexical precision" : "Audit lexical precision", fallbackAction(languageCard, "Replace vague nouns and unnatural word choices, then check word form, countability, noun reference and collocation across the full response.")],
      ["Control grammar and paragraph endings", "Check tense, articles, prepositions, agreement, clause control, punctuation and incomplete endings, then ensure each body paragraph closes as a complete unit."],
      [conclusionCard ? "Repair conclusion closure" : "Verify conclusion closure", fallbackAction(conclusionCard, conclusionIntegrity.status === "Strong"
        ? "Keep the same position and reasons, then align the conclusion wording with the thesis and body paragraphs."
        : "Restate the same position and developed body reasons without adding a new premise or reversing the policy reference.")],
      ["Rewrite and proofread", "Rewrite the full response once, verify exact thesis-to-body-to-conclusion matching, and complete a final development, lexical and grammar audit."]
    ];
    const lowMidTh = [
      ["คง Task Route ที่ตรวจว่าถูกแล้ว", "คงจุดยืนและเส้นทาง Thesis-to-Body เดิมไว้ แล้วเขียนกำกับว่า Body 1 และ Body 2 กำลังพิสูจน์เหตุผลข้อใดก่อนเริ่มแก้ development"],
      ["ซ่อม Explanation ของแต่ละ Body", "เขียน Explanation ของแต่ละ Body ใหม่เป็น ผู้ได้รับผล -> กลไกหรือการเปลี่ยนแปลง -> ผลที่ตามมา แล้วเชื่อม causal chain กลับไปหาเหตุผลเดิมใน Thesis"],
      ["ยกระดับตัวอย่างทั้งสอง Body", "สร้างตัวอย่างของทั้งสองย่อหน้าใหม่ให้มี situation, mechanism และ result ที่น่าเชื่อถือ โดยไม่ใช้ผลลัพธ์สุดโต่งที่ไม่มีหลักรองรับ"],
      [languageCard ? "ซ่อมความแม่นยำของคำศัพท์" : "ตรวจความแม่นยำของคำศัพท์", fallbackAction(languageCard, "แทนคำนามกว้างและคำเลือกใช้ที่ไม่เป็นธรรมชาติ แล้วตรวจ word form, countability, noun reference และ collocation ทั้งงาน")],
      ["ควบคุม Grammar และการจบย่อหน้า", "ตรวจ tense, article, preposition, agreement, clause control, punctuation และประโยคที่จบไม่สมบูรณ์ แล้วทำให้แต่ละ Body ปิดเป็นหน่วยความคิดที่ครบ"],
      [conclusionCard ? "ซ่อม Conclusion Closure" : "ตรวจ Conclusion Closure", fallbackAction(conclusionCard, conclusionIntegrity.status === "Strong"
        ? "คงจุดยืนและเหตุผลเดิม แล้วทำให้ถ้อยคำใน Conclusion ตรงกับ Thesis และ Body"
        : "Restate จุดยืนและเหตุผลที่พัฒนาแล้วโดยไม่เพิ่ม premise ใหม่หรือกลับทิศ policy reference")],
      ["เขียนใหม่และ Proofread", "เขียนงานใหม่ทั้งชิ้นหนึ่งรอบ ตรวจ matching จาก Thesis ไป Body และ Conclusion แล้วทำ development, lexical และ grammar audit รอบสุดท้าย"]
    ];
    return (thai ? lowMidTh : lowMidEn).map(([title, task], index) => ({ day: index + 1, title, task }));
  }

  const en = [
    [partialPosition ? "Confirm the partial-position route" : "Confirm the task route", partialPosition
      ? "Write one line for the concession, one line for the writer's main position, and verify that the thesis and conclusion preserve the same extent of disagreement."
      : "Write the function of the introduction, each body paragraph and the conclusion in one line, then verify that all sections serve the same task route."],
    [thesisCard ? "Tighten the thesis roadmap" : "Audit thesis-to-body matching", fallbackAction(thesisCard, "Underline the thesis categories and match each one to the body paragraph that develops it; do not add a reason that the body does not prove.")],
    [route1 && /concession|opposing case/.test(String(route1.label || "")) ? "Refine the concession paragraph" : "Refine Body Paragraph 1", fallbackAction(body1Card, route1 && /concession|opposing case/.test(String(route1.label || ""))
      ? "Keep the acknowledged opposing case, but make its role in the partial position explicit and remove any sentence that overstates the concession."
      : "Check that the topic sentence, explanation and evidence prove one controlling reason without repetition.")],
    [route2 && /concession|opposing case/.test(String(route2.label || "")) ? "Refine the concession paragraph" : "Refine Body Paragraph 2", fallbackAction(body2Card, route2 && /concession|opposing case/.test(String(route2.label || ""))
      ? "Keep the concession relevant and add a brief return to the final judgement if the route is not already clear."
      : "Group the paragraph's examples under one controlling claim and make the mechanism linking them to the thesis explicit.")],
    [languageCard ? "Edit the recurring language pattern" : "Run a high-band language audit", fallbackAction(languageCard, "Check vague nouns, word form, countability, collocation, reference and clause control; change only wording that is genuinely imprecise or incorrect.")],
    [conclusionCard ? "Align conclusion closure" : "Verify conclusion consistency", fallbackAction(conclusionCard, conclusionIntegrity.status === "Strong"
      ? "Keep the same position and reasons, then make the conclusion use the same key concepts and time frame as the thesis and body paragraphs."
      : "Restate the same position and developed reasons without adding a new premise or changing the policy reference.")],
    ["Rewrite and proofread", "Rewrite the full response once, verify exact paragraph matching from thesis to body to conclusion, and complete a final development and language audit."]
  ];

  const th = [
    [partialPosition ? "ยืนยัน Route แบบ Partial Position" : "ยืนยัน Task Route", partialPosition
      ? "เขียนหน้าที่ของ concession หนึ่งบรรทัด และเหตุผลหลักของผู้เขียนหนึ่งบรรทัด แล้วตรวจว่า Thesis กับ Conclusion รักษาระดับการไม่เห็นด้วยเท่าเดิม"
      : "เขียนหน้าที่ของ Introduction, Body แต่ละย่อหน้า และ Conclusion เป็นหนึ่งบรรทัด แล้วตรวจว่าทุกส่วนตอบ task route เดียวกัน"],
    [thesisCard ? "ทำ Thesis Roadmap ให้แม่น" : "ตรวจ Thesis-to-Body Matching", fallbackAction(thesisCard, "ขีดเส้นใต้หมวดเหตุผลใน Thesis แล้วจับคู่กับ Body ที่พัฒนาจริง ห้ามเพิ่มเหตุผลที่ Body ไม่ได้พิสูจน์")],
    [route1 && /concession|opposing case/.test(String(route1.label || "")) ? "เกลา Concession Paragraph" : "เกลา Body Paragraph 1", fallbackAction(body1Card, route1 && /concession|opposing case/.test(String(route1.label || ""))
      ? "คงเหตุผลฝั่งที่ยอมรับไว้ แต่ทำหน้าที่ของ concession ให้ชัดและตัดประโยคที่ให้น้ำหนักเกินจุดยืนสุดท้าย"
      : "ตรวจให้ Topic Sentence, Explanation และ Evidence พิสูจน์เหตุผลหลักเดียวโดยไม่ซ้ำ")],
    [route2 && /concession|opposing case/.test(String(route2.label || "")) ? "เกลา Concession Paragraph" : "เกลา Body Paragraph 2", fallbackAction(body2Card, route2 && /concession|opposing case/.test(String(route2.label || ""))
      ? "คง concession ที่เกี่ยวข้องไว้ และเพิ่ม bridge กลับไปหา judgement เฉพาะเมื่อ route ยังไม่ชัด"
      : "จัดกลุ่มตัวอย่างภายใต้ controlling claim เดียว และทำกลไกที่เชื่อมกลับ Thesis ให้ชัด")],
    [languageCard ? "แก้ Language Pattern ที่เกิดซ้ำ" : "ตรวจภาษาแบบ High-Band Audit", fallbackAction(languageCard, "ตรวจคำนามกว้าง word form, countability, collocation, reference และ clause control โดยแก้เฉพาะจุดที่ผิดหรือไม่แม่นจริง")],
    [conclusionCard ? "ทำ Conclusion Closure ให้ตรง" : "ตรวจความสอดคล้องของ Conclusion", fallbackAction(conclusionCard, conclusionIntegrity.status === "Strong"
      ? "คงจุดยืนและเหตุผลเดิม แล้วทำให้คำสำคัญกับช่วงเวลาตรงกับ Thesis และ Body"
      : "Restate จุดยืนและเหตุผลเดิมโดยไม่เพิ่ม premise หรือเปลี่ยน policy reference")],
    ["เขียนใหม่และ Proofread", "เขียนงานใหม่ทั้งชิ้นหนึ่งรอบ ตรวจ paragraph matching จาก Thesis ไป Body และ Conclusion แล้วทำ development กับ language audit รอบสุดท้าย"]
  ];

  return (thai ? th : en).map(([title, task], index) => ({ day: index + 1, title, task }));
}

function dedupeJoined(values = []) {
...
function conclusionPolicyClause(prompt = "", position = "") {
  ...
  const lower = text.toLowerCase();
  const negative = /disagree|negative|should not|must not|reject|oppose/.test(String(position || "").toLowerCase());
