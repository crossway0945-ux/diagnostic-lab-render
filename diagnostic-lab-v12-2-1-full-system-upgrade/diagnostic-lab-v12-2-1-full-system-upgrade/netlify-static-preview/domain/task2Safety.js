...
function semanticStemOverlap(paragraph, obligation) {
  const stopWords = new Set(["about", "after", "also", "because", "believe", "both", "could", "discuss", "does", "from", "have", "many", "more", "others", "people", "should", "some", "their", "these", "this", "views", "what", "when", "where", "which", "while", "with", "would", "your", "statement", "arrangement", "policy"]);
  const stem = (word) => {
    let value = String(word || "").toLowerCase().replace(/[^a-z-]/g, "");
    if (value.length > 6 && /ies$/.test(value)) value = `${value.slice(0, -3)}y`;
    else if (value.length > 6 && /ing$/.test(value)) value = value.slice(0, -3);
    else if (value.length > 5 && /ed$/.test(value)) value = value.slice(0, -2);
    else if (value.length > 5 && /es$/.test(value)) value = value.slice(0, -2);
    else if (value.length > 4 && /s$/.test(value)) value = value.slice(0, -1);
    return value;
  };
  const tokens = (value) => new Set((String(value || "").toLowerCase().match(/[a-z][a-z-]{3,}/g) || [])
    .filter((word) => !stopWords.has(word))
    .map(stem)
    .filter((word) => word.length >= 4));
  const obligationWords = tokens(obligation);
  const paragraphWords = tokens(paragraph);
  return [...obligationWords].filter((word) => paragraphWords.has(word)).length;
}
...
export function classifyConcessionStatus({ routeAssessment = {}, introduction = "", bodyParagraphs = [], introPosition = "", conclusionPosition = "", detectedPosition = "" } = {}) {
  const partialPosition = /partly|partially|balanced|conditional/.test(`${detectedPosition} ${introPosition}`);
  const balancedConclusion = /balanced|conditional/.test(String(conclusionPosition || ""));
  const allowedPartialClosure = partialPosition && balancedConclusion;
  if (detectedPosition === "contradictory" || (introPosition && conclusionPosition && positionDirection(introPosition) !== positionDirection(conclusionPosition) && !allowedPartialClosure)) {
  ...
  if (partialPosition && concessionRoutes.length && concessionRoutes.every((item) => !isPartialRouteStatus(item.status))) {
    return "Controlled partial-position concession";
  }
...
function buildStanceConclusionLabel(context) {
  ...
  if (context.conclusionPosition === "balanced/conditional position" && /partly|partially/.test(String(context.introPosition || context.detectedPosition || ""))) return `restates a balanced position consistent with ${context.introPosition || context.detectedPosition}`;
...
function detectPosition(text, prompt = "") {
  ...
  if (opinionPropositionPolarity(prompt) === "negative") {
    const acknowledgesPriority = /\b(?:solve|address|support|meet|prioriti[sz]e)[^.!?]{0,100}\b(?:urgent|earth|poverty|hunger|healthcare|social problems?|people'?s needs?|needs of people)\b|\b(?:urgent|earth|social problems?|people'?s needs?|needs of people)[^.!?]{0,100}\b(?:first|important|priority)\b/i.test(source);
    const preservesInvestment = /\b(?:investing|investment|spending|funding|space exploration|space research)[^.!?]{0,100}\b(?:still\s+)?(?:valuable|worthwhile|beneficial|important|justified|should continue|can continue)\b/i.test(source);
    if (acknowledgesPriority && preservesInvestment) return "balanced/conditional position";
    const rejectsNegativeClaim = /\b(?:is|are|remains?|seems?|should be)\s*(?:fully\s+|largely\s+|clearly\s+)?(?:justified|valuable|worthwhile|beneficial|important)\b|\b(?:investing|investment|spending|funding)[^.!?]{0,80}\b(?:valuable|worthwhile|beneficial|important|justified)\b|\bshould\s+(?:continue|remain|be funded|be supported)\b|\b(?:benefits?|advantages?)\s+(?:clearly\s+|far\s+)?outweigh\b/i.test(source);
...
function buildSemanticPositionModel({
  ...
  return {
    ...
    positionConsistency: introPosition && conclusionPosition && (positionDirection(introPosition) === positionDirection(conclusionPosition) || (/partly|partially/.test(introPosition) && /balanced|conditional/.test(conclusionPosition)))
      ? "consistent"
...
function opinionPropositionPolarity(prompt) {
  ...
  return /\b(?:not worth(?: it)?|waste of money|better spent(?: elsewhere| on [^.!?]+)?|money should be used to solve|should not (?:be )?(?:spend|fund|support|provide|allow)|not justified|unjustified|too (?:much|costly|expensive)|more beneficial ways? to spend)\b/i.test(source)
    ? "negative"
    : "positive";
}
...
function classifyOpinionBodyRoute({ paragraph, index, prompt, position, introduction }) {
  ...
  if (cleanWaterTask && /budget|tax|cost|money/i.test(text)) {
    return { label: "raises a budget and tax counterargument", status: countWords(text) >= 55 ? "adequate" : "partial" };
  }
  ...
  const partialPosition = /partly|partially|balanced|conditional/.test(position);
  ...
  const promptNegativeCaseSignals = countMatches(text, /\b(?:waste|wasteful|costly|expensive|billions?|poverty|hunger|poor healthcare|clean water|proper education|people in need|urgent needs?|problems? on earth|better spent|build hospitals?|build schools?|build houses?)\b/gi);
  const investmentValueSignals = countMatches(text, /\b(?:valuable|worthwhile|benefit|useful results?|technology|technological|satellites?|weather forecasts?|gps|communication|climate|protect earth|inventions?|innovation|science|long[- ]term|future opportunities?|society grow)\b/gi);
  const genericOpposingSignals = countMatches(text, /\b(?:cost|burden|drawback|disadvantage|risk|waste|unjustified|alternative|priority|better spent|should not|cannot afford|too expensive)\b/gi);
  const genericSupportingSignals = countMatches(text, /\b(?:benefit|advantage|justified|essential|valuable|worthwhile|improve|protect|enable|support|long-term|basic right|should (?:continue|receive|be provided|be funded))\b/gi);
  const thesisOverlap = semanticKeywordOverlap(text, introduction);
  const positionSentence = splitSentences(introduction).find((sentence) => POSITION_PATTERN.test(sentence)) || lastSentence(introduction);
  const writerRouteText = String(positionSentence || "").replace(POSITION_PATTERN, " ");
  const writerRouteOverlap = semanticStemOverlap(text, writerRouteText);
  const promptOverlap = semanticKeywordOverlap(text, prompt);
  ...
  let supportsPromptView = false;
  let supportsWriterPosition = false;
  if (negativeProposition && writerDisagrees) {
    supportsPromptView = promptNegativeCaseSignals > investmentValueSignals;
    supportsWriterPosition = investmentValueSignals >= Math.max(1, promptNegativeCaseSignals);
  } else if (negativeProposition && writerAgrees) {
    supportsWriterPosition = promptNegativeCaseSignals >= Math.max(1, investmentValueSignals);
    supportsPromptView = supportsWriterPosition;
  } else {
    if (writerAgrees) {
      supportsWriterPosition = writerRouteOverlap >= 1 || genericSupportingSignals > genericOpposingSignals;
      supportsPromptView = supportsWriterPosition;
    } else if (writerDisagrees) {
      supportsWriterPosition = writerRouteOverlap >= 1 || genericOpposingSignals > genericSupportingSignals;
      supportsPromptView = !supportsWriterPosition && promptOverlap >= 1 && genericSupportingSignals > genericOpposingSignals;
    } else {
      supportsWriterPosition = writerRouteOverlap >= 2 || thesisOverlap >= 2;
      supportsPromptView = !supportsWriterPosition && promptOverlap >= 2;
    }
  }

  const concession = partialPosition
    ? supportsPromptView && !supportsWriterPosition
    : explicitConcession || (supportsPromptView && !supportsWriterPosition);

  if (concession) {
    const adequate = countWords(text) >= 55;
    return {
      label: partialPosition
        ? "develops the acknowledged opposing case as a controlled concession within the writer's partial position"
        : explicitReturn
          ...
          : `presents a relevant concession that should return more explicitly to the writer's ${writerDisagrees ? "disagreement" : writerAgrees ? "agreement" : "position"}`,
      status: adequate && (partialPosition || explicitReturn) ? "adequate" : "partial"
    };
  }

  if (supportsWriterPosition || thesisOverlap >= 2) {
    return {
  ...
  return {
    label: raw === "supports the proposition"
      ...
      : raw === "opposes or limits the proposition"
        ? "presents an opposing or concessive reason that needs clearer relation to the writer's position"
...
function reconcilePosition({ introPosition, conclusionPosition, unfinishedEndingDetected, bodyRoutes }) {
  const introPartial = /partly|partially|balanced|conditional/.test(String(introPosition || ""));
  const conclusionBalanced = /balanced|conditional/.test(String(conclusionPosition || ""));
  if (introPosition && conclusionPosition && positionDirection(introPosition) !== positionDirection(conclusionPosition) && !(introPartial && conclusionBalanced)) return "contradictory";
...
function positionConfidenceFor({ introPosition, conclusionPosition, unfinishedEndingDetected, bodyRoutes }) {
  ...
  const introPartial = /partly|partially|balanced|conditional/.test(String(introPosition || ""));
  const conclusionBalanced = /balanced|conditional/.test(String(conclusionPosition || ""));
  if (conclusionPosition && positionDirection(introPosition) !== positionDirection(conclusionPosition) && !(introPartial && conclusionBalanced)) return "low";
  if (conclusionPosition && (positionDirection(introPosition) === positionDirection(conclusionPosition) || (introPartial && conclusionBalanced))) return "high";
...
function detectRouteConflict({ essayRoute, detectedPosition, introPosition, conclusionPosition, bodyRoutes }) {
  if (essayRoute === TASK2_CANONICAL_TYPES.OPINION) {
    const introPartial = /partly|partially|balanced|conditional/.test(String(introPosition || detectedPosition || ""));
    const conclusionBalanced = /balanced|conditional/.test(String(conclusionPosition || ""));
    return Boolean(introPosition && conclusionPosition && positionDirection(introPosition) !== positionDirection(conclusionPosition) && !(introPartial && conclusionBalanced));
  }
