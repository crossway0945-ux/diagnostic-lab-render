...
document.addEventListener("click", (event) => {
  ...
  if (alertButton) {
    if (alertButton.dataset.alert === "pdf") {
      exportDiagnosticPdf(currentAnalysis, currentReportId, alertButton);
      return;
    }
...
function handleCompletedAnalysis(result, payload) {
  ...
  if (result.progressRecord) {
    currentReportId = result.progressRecord.submissionId || "";
...
async function exportDiagnosticPdf(_analysis, reportId = currentReportId, triggerButton = null) {
  const id = String(reportId || "").trim();
  if (!id) {
    showError("The official PDF is available after the report has been saved. Open the saved report from Progress and try again.");
    return;
  }
  const button = triggerButton || document.querySelector("[data-alert='pdf']");
  ...
  try {
    ...
    showToast("Generating the official PDF on the server. This may take up to one minute on the first export...");
    ...
    if (!response.ok) {
      ...
      throw new Error(payload.error || `The PDF report could not be generated (${response.status}).`);
    }
    const blob = await response.blob();
    if (!blob.size || !String(blob.type || "").includes("pdf")) {
      throw new Error("The server returned an invalid PDF file. Please try the export again.");
    }
    const disposition = response.headers.get("content-disposition") || "";
    const filename = disposition.match(/filename="([^"]+)"/i)?.[1] || "Kru-Pom-IELTS-Diagnostic.pdf";
    ...
    const message = error?.name === "AbortError"
      ? "PDF generation timed out. The server did not complete the file within two minutes; check the Render log for PDF_BROWSER_NOT_INSTALLED or PDF_GENERATION_TIMEOUT."
      : error.message || "The PDF report could not be generated.";
    showError(message);
  } finally {
    ...
    if (button) {
      ...
      button.textContent = originalText || "Export Diagnostic PDF";
    }
  }
}
...
function loadHistoryReport(submissionId, shouldPrint) {
  ...
  currentReportId = record.submissionId || "";
  renderAnalysis(record.report);
  showSection("dashboard");
  showToast("Saved diagnostic report loaded.");

  if (shouldPrint) {
    exportDiagnosticPdf(record.report, record.submissionId);
  }
}
