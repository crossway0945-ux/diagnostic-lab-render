...
import { getEmbeddedReportFontCss } from "../reports/reportFonts.js";
...
const REPORT_PDF_CACHE_LIMIT = 8;
...
export function createApiHandler(options = {}) {
  ...
  return async function handleApiRequest(request) {
    ...
    try {
      ...
      const reportMatch = apiPath.match(/^\/api\/reports\/([^/]+)\/(html|pdf)$/);
      if (method === "GET" && reportMatch) {
        return await handleStudentReportDocument(request, storage, decodeURIComponent(reportMatch[1]), reportMatch[2]);
      }
...
function sanitizeCanonicalForClient(canonical = {}) {
  ...
  const { ownerAccountId, studentProfileId, ...safeMetadata } = metadata;
...
async function handleStudentReportDocument(request, storage, reportId, format) {
  const session = await requireSession(request, storage);
  const records = await storage.getSubmissionHistory(session.user.username);
  const record = records.find((item) => String(item.submissionId || "") === String(reportId || ""));
  if (!record || record.analysisValidity === "invalid") {
    throw statusError("The saved report was not found for this account.", 404, "VALIDATION_ERROR");
  }
  const analysis = record.report || record.analysis;
  if (!analysis || typeof analysis !== "object") {
    throw statusError("The saved report does not contain a validated analysis.", 409, "VALIDATION_ERROR");
  }
  const taskRecords = records.filter((item) => item.taskType === record.taskType && isValidProgressRecord(item));
  const progressSummary = buildServerProgressSummary(taskRecords, record, record.taskType);
  const model = buildStudentReportViewModel(analysis, {
    studentDisplayName: record.studentDisplayNameSnapshot,
    generatedAt: record.dateTime,
    progressSummary
  });
  const filename = safeReportFilename(model.studentMetadata.student, model.studentMetadata.taskType);
  if (format === "html") {
    const fontCss = await getEmbeddedReportFontCss();
    return rawResponse(200, renderStudentReportDocument(model, { fontCss }), {
      "content-type": "text/html; charset=utf-8",
      "content-disposition": `inline; filename="${filename}.html"`,
      "cache-control": "private, no-store"
    });
  }
  const cacheKey = `${session.user.username}:${record.submissionId}:${record.reportVersionId || record.dateTime || "latest"}`;
  let generated = reportPdfCache.get(cacheKey);
  if (!generated) {
    const startedAt = Date.now();
    try {
      generated = await generateStudentReportPdf(model);
      reportPdfCache.set(cacheKey, { pdfBuffer: generated.pdfBuffer, pageCount: generated.pageCount });
      while (reportPdfCache.size > REPORT_PDF_CACHE_LIMIT) reportPdfCache.delete(reportPdfCache.keys().next().value);
      console.info("[diagnostic-lab] student PDF generated", {
        reportId: record.submissionId,
        username: session.user.username,
        pageCount: generated.pageCount,
        bytes: generated.pdfBuffer.length,
        durationMs: Date.now() - startedAt
      });
    } catch (error) {
      console.error("[diagnostic-lab] student PDF generation failed", {
        reportId: record.submissionId,
        username: session.user.username,
        errorCode: error?.errorCode || "PDF_GENERATION_FAILED",
  ...
  return rawResponse(200, generated.pdfBuffer.toString("base64"), {
    "content-type": "application/pdf",
    "content-disposition": `attachment; filename="${filename}.pdf"`,
    "content-length": String(generated.pdfBuffer.length),
    "cache-control": "private, no-store",
    "x-report-page-count": String(generated.pageCount)
  }, true);
}

function safeReportFilename(studentName, taskType) {
  const student = String(studentName || "student").normalize("NFKD").replace(/[^a-zA-Z0-9ก-๙_-]+/gu, "-").replace(/^-+|-+$/g, "") || "student";
  const task = String(taskType || "IELTS").replace(/\s+/g, "-");
  return `Kru-Pom-IELTS-${student}-${task}-Diagnostic`;
}
...
function rawResponse(statusCode, body, extraHeaders = {}, isBase64Encoded = false) {
  return {
    statusCode,
    headers: {
      ...corsHeaders(),
