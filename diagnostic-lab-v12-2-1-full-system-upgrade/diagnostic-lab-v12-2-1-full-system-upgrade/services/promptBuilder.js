...
- For genuinely strong Band 7+ work, do not force generic repair days for causal mechanisms, examples, grammar, or conclusion closure when the exact evidence does not support those weaknesses. Build a refinement plan from the actual high-band issues only.
- A partial-agreement or partial-disagreement Opinion Essay may validly use one body paragraph as a controlled concession and the other as the writer's main case. Do not reverse these paragraph functions, and do not require the concession paragraph to rebut itself when the thesis and conclusion already control the partial position.
- A separate formulaic Link-Back sentence is not mandatory. Judge whether the paragraph maintains and completes its controlling idea rather than checking only for words such as therefore or thus.
- paragraphFeedback must never use a generic placeholder when the paragraph is strong. State the paragraph's real function and positive evidence, then give only an optional refinement supported by that paragraph.
...
V12.2 full-system diagnostic integrity contract:
- Diagnose by task obligations, paragraph function, exact evidence, and criterion impact. Never trigger feedback from a named fixture, topic keyword, or memorised student sentence.
- Match each feedback card to one exact submitted sentence and its true paragraph location. Cover every submitted paragraph and do not skip a body explanation merely because a later example expresses the same route.
- Judge Conclusion Closure from position consistency, policy/reference accuracy, route overlap, clause completion, and language control. A visible position alone is not enough for Strong closure.
- Do not criticise an example merely because its entity is absent from the prompt. Judge relevance, mechanism, credibility, precision, and development.
- Preserve student meaning in Minimal Correction and Route-Preserving Revision. Any added premise, wider consequence, selected stance, or unsupported intensity must be labelled Teacher-Guided Expansion.
- Keep IELTS criterion estimates independent. Reconcile the overall estimate from all four criteria rather than treating a clear route as automatic Band 7.
