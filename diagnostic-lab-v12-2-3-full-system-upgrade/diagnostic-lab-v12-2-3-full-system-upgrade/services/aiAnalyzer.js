...
const DEFAULT_OPENAI_MAX_OUTPUT_TOKENS = 8000;
const DEFAULT_SERVERLESS_PROVIDER_TIMEOUT_CAP_MS = 55000;
const DEFAULT_SERVERLESS_MAX_OUTPUT_TOKENS = 8000;
  ...
  const result = await runOpenAiAnalysisAttempt({ config, payload, prompt, isRetry: false }).catch(async (error) => {
    ...
    try {
      return await runOpenAiAnalysisAttempt({
        config: {
          ...config,
          maxOutputTokens: Math.max(config.maxOutputTokens, 10000),
...
export function buildLocalAnalysis(payload) {
...
function buildSafeTask2TargetedRevision(sentence, card = {}) {
  ...
  if (normalizeEvidenceText(repaired) !== normalizeEvidenceText(source)) return repaired;
  const proposed = repairDeterministicLanguageSentence(String(card.targetedRevision || "").trim());
  return normalizeEvidenceText(proposed) !== normalizeEvidenceText(source) ? proposed : "";
}

function explainIntegrityRepair(sentence) {
  const source = String(sentence || "");
  const repairs = [];
  if (/\b(?:clusterization|congestion of traffic|difficulty of traveling|issue of traveling|travel through long distance)\b/i.test(source)) repairs.push("word choice and collocation");
  if (/\b(?:is living|are living)\b/i.test(source)) repairs.push("tense control");
  if (/\b(?:specific|certain|same) places?\b/i.test(source)) repairs.push("noun reference");
  if (/[,;:]\s*$/.test(source)) repairs.push("sentence closure");
  const focus = repairs.length ? repairs.join(", ") : "the diagnosed language-control problems";
  return `The revision repairs ${focus} while preserving the student's original paragraph route and level of claim.`;
}
...
function repairDeterministicLanguageSentence(sentence) {
  let text = String(sentence || "")
    ...
    .replace(/\bEvery ([a-z]+) is living\b/gi, "Each $1 lives")
    .replace(/\bin different places and distances\b/gi, "in different locations at varying distances")
    .replace(/\bspecific places like towns and cities\b/gi, "urban areas")
    .replace(/\ball the same places are in the same area\b/gi, "similar facilities are concentrated in the same area")
    .replace(/\b(?:a certain|a specific|certain|specific) place is clustered in one area\b/gi, "similar facilities are concentrated in one area")
...
function reportOutputValidationError(issues, stage = "report_validation") {
    ...
    .map((issue) => {
      if (issue && typeof issue === "object") {
        return {
          ...
          message: String(issue.message || issue.reason || issue.detail || "Report output failed validation.").trim()
        };
      }
      return { code: "", field: "", message: String(issue || "Report output failed validation.").trim() };
    })
  ...
  const validationDetails = deduped.map((issue) => ({
    code: issue.code || validationIssueCode(issue.message),
    severity: "fatal_integrity",
    stage,
    field: issue.field || validationIssueField(issue.message),
