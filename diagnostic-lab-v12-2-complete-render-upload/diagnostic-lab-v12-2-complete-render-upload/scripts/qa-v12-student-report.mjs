...
const enPdfPath = path.join(outputDir, "Sun-V12-Student-Report-English.pdf");
const thPdfPath = path.join(outputDir, "Sun-V12-Student-Report-Thai.pdf");
await Promise.all([
  writeFile(enPdfPath, en.pdfBuffer),
  writeFile(thPdfPath, th.pdfBuffer),
  writeFile(path.join(outputDir, "Sun-V12-Student-Report-English.html"), en.sourceHtml, "utf8"),
  writeFile(path.join(outputDir, "Sun-V12-Student-Report-Thai.html"), th.sourceHtml, "utf8"),
  writeFile(path.join(outputDir, "Sun-V12-PDFJS-English.txt"), en.pdfJsText, "utf8"),
  writeFile(path.join(outputDir, "Sun-V12-PDFJS-Thai.txt"), th.pdfJsText, "utf8")
]);
...
if (pdftotextAvailable) {
  ...
  await writeFile(path.join(outputDir, "Sun-V12-PDFTOTEXT-English.txt"), enPoppler, "utf8");
  await writeFile(path.join(outputDir, "Sun-V12-PDFTOTEXT-Thai.txt"), thPoppler, "utf8");
}
...
function buildMarkdown({ en, th, textRows, pdftotextAvailable, adminQA, enAnalysis }) {
  ...
  const expectedMatrix = [
    ...
    ["Estimated Overall Range", "6.0-6.5", enAnalysis.estimatedBandRange],
    ["Writer Position", "strongly disagree", enAnalysis.detectedPosition],
    ["Task Response", "6.5", enAnalysis.criteriaScores?.["Task Response"]?.range],
