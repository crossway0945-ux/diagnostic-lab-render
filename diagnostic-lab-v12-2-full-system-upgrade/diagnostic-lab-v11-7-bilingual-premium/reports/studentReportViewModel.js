...
function projectFeedbackCard(card = {}, language = "en") {
  ...
  if (!projected.revisionIntegrity.pass) {
    const fallback = safeMinimalFallback(projected.exactSentence);
    projected = {
      ...projected,
      revisionType: fallback === projected.exactSentence ? "High-Band Refinement" : "Minimal Correction",
      targetedRevision: fallback
    };
...
function safeMinimalFallback(value) {
  let text = clean(value, { preserveStudentText: true })
    .replace(/([.!?])(?=[A-Z])/g, "$1 ")
    .replace(/\s{2,}/g, " ")
    .replace(/[,;:]\s*$/g, ".")
    .trim();
