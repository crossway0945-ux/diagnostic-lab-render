...
function ensureTask2ReportCompleteness(output, used, payload) {
  // V12.2: fixture-specific runtime cards were removed.
  // Coverage is now enforced by domain/task2DiagnosticIntegrity.js from paragraph functions,
  // deterministic language evidence and task-family route data.
  return output;
}
...
function buildTeacherGuidedBody2Revision(sentence, prompt) {
  const original = String(sentence || "").replace(/[.!?]+$/g, "").trim();
  const promptNouns = String(prompt || "").toLowerCase().match(/[a-z][a-z-]{4,}/g)?.filter((token) => !/^(?:should|would|could|about|there|their|which|where|these|those|people)$/.test(token)).slice(0, 3) || [];
  const anchor = promptNouns.length ? ` the ${promptNouns.join("/")} context` : " the situation in the prompt";
  return `${original}. This example should then explain the affected group, the mechanism operating in${anchor}, and the specific result that proves the paragraph claim.`;
}

function repairDeterministicLanguageSentence(sentence) {
  let text = String(sentence || "")
    .replace(/\bclusterization\b/gi, "concentration")
    .replace(/\ba (?:large|heavy) traffic congestion\b/gi, "heavy traffic congestion")
    ...
    .replace(/\b(?:the difficulty|an issue|the issue) of traveling\b/gi, "travel difficulties")
...
function buildTask2CoverageTopIssueCards(cards, payload) {
  // V12.2: topic-specific aggregation removed. Generic issue aggregation is produced
  // by applyTask2FullSystemUpgrade after paragraph/evidence reconciliation.
  return [];
}
...
function localizeExecutiveFields(payload = {}, mainScoreLimitingFactor = "", mostUrgentRepair = "") {
  // Learner-facing language is projected generically by the full-system integrity layer.
...
function normalizeAnalysis(analysis, payload) {
  ...
  let feedbackCards = recoveredFeedbackCards;
  let upgradedAnalysis = guardedAnalysis;
  let upgradedTopIssues = null;
  let upgradedParagraphFeedback = null;
  let upgradedPracticePlan = null;
  if (payload.taskType === "Task 2") {
    const fullUpgrade = applyTask2FullSystemUpgrade({
      payload,
      analysis: guardedAnalysis,
      feedbackCards,
      paragraphFeedback: guardedAnalysis.paragraphFeedback,
      practicePlan: guardedAnalysis.practicePlan
    });
    upgradedAnalysis = fullUpgrade.analysis;
    feedbackCards = fullUpgrade.feedbackCards;
    upgradedTopIssues = fullUpgrade.topIssues;
    upgradedParagraphFeedback = fullUpgrade.paragraphFeedback;
    upgradedPracticePlan = fullUpgrade.practicePlan;
  }
  const reconciledMainScoreLimitingFactor = reconcileTask1ExecutiveSummary(
    upgradedAnalysis.mainScoreLimitingFactor,
    upgradedAnalysis.criteriaScores,
    feedbackCards,
    upgradedAnalysis,
    payload
  );
  const mainScoreLimitingFactor = recoverExecutiveField(reconciledMainScoreLimitingFactor, feedbackCards, "summary");
  const mostUrgentRepair = recoverExecutiveField(upgradedAnalysis.mostUrgentRepair, feedbackCards, "repair");
  const localizedExecutive = localizeExecutiveFields(payload, mainScoreLimitingFactor, mostUrgentRepair);
  const top3Issues = normalizeTopIssues(
    upgradedTopIssues || selectDeterministicTopIssueCards(feedbackCards, payload),
    feedbackCards,
    payload.taskType === "Task 2" ? 5 : 3
  );
  const paragraphFeedback = upgradedParagraphFeedback || normalizeParagraphFeedback(upgradedAnalysis.paragraphFeedback, payload, feedbackCards);
  const practicePlan = upgradedPracticePlan || (payload.taskType === "Task 2"
    ? buildCanonicalTask2PracticePlan(payload, feedbackCards)
    : enrichPracticePlan(
        Array.isArray(upgradedAnalysis.practicePlan) && upgradedAnalysis.practicePlan.length
          ? upgradedAnalysis.practicePlan
  ...
  const normalized = {
    ...
    minimumRequiredWords: payload.taskType === "Task 2" ? upgradedAnalysis.minimumRequiredWords : payload.minimumWordCount,
    ...
    essayType: payload.publicEssayType || upgradedAnalysis.essayType || payload.essayType,
    ...
    visualType: payload.publicVisualType || upgradedAnalysis.visualType || payload.visualType,
    ...
    targetBand: upgradedAnalysis.targetBand || payload.targetBand,
    generatedAt: upgradedAnalysis.generatedAt || new Date().toISOString(),
    analysisMode: upgradedAnalysis.analysisMode || "Full diagnostic engine",
    estimatedBandRange: upgradedAnalysis.estimatedBandRange || "6.0-6.5",
    mainScoreLimitingFactor: localizedExecutive.mainScoreLimitingFactor,
    mostUrgentRepair: localizedExecutive.mostUrgentRepair,
    criteriaScores: localizeCriterionScores(upgradedAnalysis.criteriaScores || {}, payload),
    kruPomScores: upgradedAnalysis.kruPomScores || {},
    canonicalTask2Analysis: upgradedAnalysis.canonicalTask2Analysis || null,
    task2EssayType: upgradedAnalysis.task2EssayType || "",
    task2EssayTypeLabel: payload.publicEssayType || upgradedAnalysis.task2EssayTypeLabel || "",
    stanceRequired: Boolean(upgradedAnalysis.stanceRequired),
    taskRequirements: Array.isArray(upgradedAnalysis.taskRequirements) ? upgradedAnalysis.taskRequirements : [],
    routeAssessment: upgradedAnalysis.routeAssessment || null,
    capMetadata: upgradedAnalysis.capMetadata || null,
    taskAchievementCapReason: upgradedAnalysis.taskAchievementCapReason || "",
    overviewAccuracyStatus: upgradedAnalysis.overviewAccuracyStatus || "",
    criticalOverviewError: Boolean(upgradedAnalysis.criticalOverviewError),
    mainTrendRecognition: upgradedAnalysis.mainTrendRecognition || "",
    dataSelectionQuality: upgradedAnalysis.dataSelectionQuality || "",
    unsafeGeneralisationDetected: Boolean(upgradedAnalysis.unsafeGeneralisationDetected),
    majorOmissionDetected: Boolean(upgradedAnalysis.majorOmissionDetected),
    contradictionDetected: Boolean(upgradedAnalysis.contradictionDetected),
    dataAccuracyRisk: upgradedAnalysis.dataAccuracyRisk || "",
    groupingLogicStatus: upgradedAnalysis.groupingLogicStatus || "",
    recommendedTaskAchievementRange: upgradedAnalysis.recommendedTaskAchievementRange || "",
    promptCoverageStatus: upgradedAnalysis.promptCoverageStatus || "",
    thesisRouteStatus: upgradedAnalysis.thesisRouteStatus || "",
    brokenPromiseDetected: Boolean(upgradedAnalysis.brokenPromiseDetected),
    bodyRouteAlignmentStatus: upgradedAnalysis.bodyRouteAlignmentStatus || "",
    SARExampleStatus: upgradedAnalysis.SARExampleStatus || "",
    intruderSentenceDetected: Boolean(upgradedAnalysis.intruderSentenceDetected),
    conclusionClosureStatus: upgradedAnalysis.conclusionClosureStatus || "",
    completionStatus: upgradedAnalysis.completionStatus || (payload.wordShortfall > 0 ? "mostly complete" : "complete"),
    unfinishedEndingDetected: Boolean(upgradedAnalysis.unfinishedEndingDetected),
    completionEvidence: Array.isArray(upgradedAnalysis.completionEvidence) ? upgradedAnalysis.completionEvidence : [],
    detectedPosition: upgradedAnalysis.detectedPosition || "",
    positionConfidence: upgradedAnalysis.positionConfidence || "",
    bodyRouteSummary: upgradedAnalysis.bodyRouteSummary || "",
    detectedStructure: upgradedAnalysis.detectedStructure || "",
    paragraphDetectionConfidence: upgradedAnalysis.paragraphDetectionConfidence || "",
    conclusionStatus: upgradedAnalysis.conclusionStatus || "",
    routeConflict: Boolean(upgradedAnalysis.routeConflict),
    recommendedRoute: upgradedAnalysis.recommendedRoute || "",
    recommendedRouteRationale: upgradedAnalysis.recommendedRouteRationale || "",
    routeIntegrity: upgradedAnalysis.routeIntegrity || "",
    completionIntegrity: upgradedAnalysis.completionIntegrity || "",
    languageControlIntegrity: upgradedAnalysis.languageControlIntegrity || "",
    compoundSeverity: upgradedAnalysis.compoundSeverity || "",
    criticalInteractionSummary: upgradedAnalysis.criticalInteractionSummary || "",
    meaningChangingErrors: Array.isArray(upgradedAnalysis.meaningChangingErrors) ? upgradedAnalysis.meaningChangingErrors : [],
    meaningReversingErrors: Array.isArray(upgradedAnalysis.meaningReversingErrors) ? upgradedAnalysis.meaningReversingErrors : [],
    languageProfile: upgradedAnalysis.languageProfile || {},
    languageAccuracyRisk: upgradedAnalysis.languageAccuracyRisk || {},
    developmentRisk: upgradedAnalysis.developmentRisk || {},
    revisedThesisRevisionType: upgradedAnalysis.revisedThesisRevisionType || "",
    taskResponseCapReason: upgradedAnalysis.taskResponseCapReason || "",
    overallBandCap: upgradedAnalysis.overallBandCap || "",
    strictModeApplied: Boolean(upgradedAnalysis.strictModeApplied),
    mainScoreLimitingIssue: upgradedAnalysis.mainScoreLimitingIssue || mainScoreLimitingFactor || "",
    taskRouteDiagnosis: upgradedAnalysis.taskRouteDiagnosis || {},
    criticalFlags: Array.isArray(upgradedAnalysis.criticalFlags) ? upgradedAnalysis.criticalFlags : [],
    capsApplied: Array.isArray(upgradedAnalysis.capsApplied) ? upgradedAnalysis.capsApplied : [],
    severitySummary: upgradedAnalysis.severitySummary || "",
    highBandLimiters: Array.isArray(upgradedAnalysis.highBandLimiters) ? upgradedAnalysis.highBandLimiters : [],
    topRepairPriority: upgradedAnalysis.topRepairPriority || upgradedAnalysis.mostUrgentRepair || "",
    top3Issues,
    feedbackCards,
    paragraphFeedback,
    revisedThesis: upgradedAnalysis.revisedThesis || "",
    revisedParagraphSuggestions: upgradedAnalysis.revisedParagraphSuggestions || [],
    practicePlan,
    warnings: payload.wordCountStatus === "below_minimum"
      ? [
          ...(Array.isArray(upgradedAnalysis.warnings) ? upgradedAnalysis.warnings : []),
          `Verified word count: ${payload.wordCount}; shortfall: ${payload.wordShortfall}.`
        ]
      : (upgradedAnalysis.warnings || []),
    disclaimer: upgradedAnalysis.disclaimer || (normalizeReportLanguage(payload.reportLanguage) === "th" ? THAI_DISCLAIMER : DISCLAIMER),
    thaiDisclaimer: upgradedAnalysis.thaiDisclaimer || THAI_DISCLAIMER
  };
...
function collectTask2ReportOutputIssues(analysis, payload) {
  ...
  for (const [index, issue] of (analysis.top3Issues || []).entries()) {
    ...
    const issueEvidence = normalizeEvidenceText(issue.exactSentence);
    const cardEvidence = normalizeEvidenceText(card.exactSentence);
    const evidenceMatches = issueEvidence === cardEvidence || (issueEvidence && cardEvidence && (cardEvidence.includes(issueEvidence) || issueEvidence.includes(cardEvidence)));
    if (!evidenceMatches) {
