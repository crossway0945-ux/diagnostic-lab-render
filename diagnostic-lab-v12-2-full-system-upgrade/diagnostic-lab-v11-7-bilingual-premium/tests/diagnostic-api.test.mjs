...
async function testTeacherStudentSelectionReenablesAnalyzeButton() {
  ...
  const cacheBustedScript = "script.js?v=diagnostic-v12-student-report-architecture";
...
async function testEvinSemanticOutweighAndIssueMapping() {
  ...
  try {
    ...
    assert.ok(new Set(repaired.top3Issues.map((issue) => issue.feedbackCardId)).size >= 2);
    const grammarIssue = repaired.top3Issues.find((issue) => /grammatical|grammar|punctuation/i.test(issue.issueType));
    assert.ok(grammarIssue);
    assert.equal(grammarIssue.exactSentence, "Young workers provide an abundant supply of labor that boosts industrial productivity and increases tax revenues for the government.");
    assert.match(grammarIssue.paragraphLocation, /^Body Paragraph 2/);
    assert.match(grammarIssue.whyItLimitsBand, /punctuation|spacing|missing space/i);
...
async function testTask2LowBandCompletionAndRouteSafety() {
  ...
  for (const source of [rootScript, previewScript]) {
    ...
    assert.ok(source.includes("renderFeedbackCard"));
  }
}
