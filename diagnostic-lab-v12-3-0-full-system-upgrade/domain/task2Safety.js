...
function evaluateConclusionClosure(conclusion, languageProfile = {}) {
  ...
  const vagueReferenceCount = (evidence.match(/\b(?:thing|things|place|places|something|somewhere)\b/gi) || []).length;
  const clauseControlRisk = /,\s*(?:thus|hence|therefore)\b/i.test(evidence) || /[,;:]\s*$/u.test(evidence);
  const sentenceControlRisk = (languageProfile.sentenceCompletionErrors || []).some((item) => {
    const exact = String(item?.exactSentence || item?.exactEvidence || "").trim();
    return exact && evidence && (evidence.includes(exact) || exact.includes(evidence));
  });
  if (clauseControlRisk || sentenceControlRisk || vagueReferenceCount >= 2) {
    return { status: "Moderate", reason: "route is preserved, but policy reference, clause control, or lexical precision needs repair" };
  }
  return { status: "Strong", reason: "position and established body routes are closed consistently with controlled wording" };
}
...
export function reconcileTask2CanonicalAnalysis(payload = {}, providerAnalysis = {}, suppliedSafety = null) {
  ...
  return {
    ...
    consistency: {
      analysisSource: "canonical-analysis-v12.3.0",
      routeSource: "canonical-analysis-v12.3.0",
