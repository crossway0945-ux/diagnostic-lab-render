...
const DEFAULT_STUDENT_PROFILES_FILE = "student-profiles.json";
...
export function normalizeStudentProfile(profile = {}) {
  const displayName = normalizeStudentDisplayName(profile.displayName);
  const now = new Date().toISOString();
  const active = profile.active !== false;
  return {
    id: String(profile.id || randomUUID()),
    ownerAccountId: String(profile.ownerAccountId || "").trim(),
    displayName,
    ...
    active,
    archivedAt: active ? "" : String(profile.archivedAt || profile.updatedAt || now)
  };
}
...
class JsonFileStorage {
  constructor({ dataDir, seedDir = dataDir, runtime = "node" }) {
    ...
    this.studentProfilesPath = path.join(dataDir, DEFAULT_STUDENT_PROFILES_FILE);
    this.seedUsersPath = path.join(seedDir, DEFAULT_USERS_FILE);
    this.seedHistoryPath = path.join(seedDir, DEFAULT_HISTORY_FILE);
    this.seedStudentProfilesPath = path.join(seedDir, DEFAULT_STUDENT_PROFILES_FILE);
  ...
  async listStudentProfiles(ownerAccountId, options = {}) {
    const profiles = await this.readStudentProfiles();
    return profiles.filter((profile) => profile.ownerAccountId === ownerAccountId && (options.includeArchived || profile.active));
  }

  async getStudentProfile(ownerAccountId, profileId, options = {}) {
    const profiles = await this.readStudentProfiles();
    return profiles.find((profile) => profile.ownerAccountId === ownerAccountId && profile.id === profileId && (options.includeArchived || profile.active)) || null;
  }

  async createStudentProfile(ownerAccountId, displayName) {
    return this.withLock(async () => {
      const profiles = await this.readStudentProfiles();
      const next = validateNewStudentProfile(ownerAccountId, displayName);
      if (profiles.some((profile) => profile.ownerAccountId === ownerAccountId && profile.normalizedName === next.normalizedName && profile.active)) {
        throw statusError("A student with this name already exists. Select the existing student.", 409);
      }
      profiles.push(next);
  ...
  async archiveStudentProfile(ownerAccountId, profileId) {
    return this.withLock(async () => {
      const profiles = await this.readStudentProfiles();
      const index = profiles.findIndex((profile) => profile.ownerAccountId === ownerAccountId && profile.id === profileId);
      if (index === -1) throw statusError("Student profile was not found.", 404);
      const now = new Date().toISOString();
      profiles[index] = { ...profiles[index], active: false, archivedAt: now, updatedAt: now };
      await this.writeStudentProfiles(profiles);
      return profiles[index];
    });
  }

  async restoreStudentProfile(ownerAccountId, profileId) {
    return this.withLock(async () => {
      const profiles = await this.readStudentProfiles();
      const index = profiles.findIndex((profile) => profile.ownerAccountId === ownerAccountId && profile.id === profileId);
      if (index === -1) throw statusError("Student profile was not found.", 404);
      if (profiles.some((profile, otherIndex) => otherIndex !== index && profile.ownerAccountId === ownerAccountId && profile.active && profile.normalizedName === profiles[index].normalizedName)) {
        throw statusError("An active student with this name already exists. Archive or rename that profile before restoring this one.", 409);
      }
      const now = new Date().toISOString();
      profiles[index] = { ...profiles[index], active: true, archivedAt: "", updatedAt: now };
      await this.writeStudentProfiles(profiles);
      return profiles[index];
    });
  }

  async deleteStudentProfile(ownerAccountId, profileId) {
    return this.withLock(async () => {
      const profiles = await this.readStudentProfiles();
      const profileIndex = profiles.findIndex((profile) => profile.ownerAccountId === ownerAccountId && profile.id === profileId);
      if (profileIndex === -1) throw statusError("Student profile was not found.", 404);
      if (profiles[profileIndex].active !== false) {
        throw statusError("Archive the student profile before permanently deleting it.", 409);
      }

      const history = await this.readHistory();
      const remainingHistory = history.filter((record) => !(record.username === ownerAccountId && record.studentProfileId === profileId));
      const deletedReportCount = history.length - remainingHistory.length;
      const [deletedProfile] = profiles.splice(profileIndex, 1);
      await this.writeHistory(remainingHistory);
      await this.writeStudentProfiles(profiles);
      return { profile: deletedProfile, deletedReportCount };
    });
  }

  async ensureStudentProfile(ownerAccountId, displayName) {
    const normalizedName = normalizeStudentDisplayName(displayName).toLocaleLowerCase("en-US");
    const profiles = await this.readStudentProfiles();
    const existing = profiles.find((profile) => profile.ownerAccountId === ownerAccountId && profile.normalizedName === normalizedName && profile.active);
    return existing || this.createStudentProfile(ownerAccountId, displayName);
  }

  async getSubmissionHistory(username, studentProfileId = "", taskType = "") {
  ...
  async findSubmissionByHash(username, submissionHash, studentProfileId = "") {
    ...
    const records = await this.readHistory();
    return records.find((record) => record.username === username && (record.submissionHash === submissionHash || record.inputFingerprint === submissionHash) && record.analysisValidity !== "invalid" && (!studentProfileId || record.studentProfileId === studentProfileId)) || null;
  }
  ...
  async commitAnalysis(record, auditEntry) {
    return this.withLock(async () => {
      const [records, users, audit] = await Promise.all([this.readHistory(), this.readUsers(), this.readAudit()]);
      const existing = findDuplicateSubmission(records, record);
      const userIndex = users.findIndex((user) => user.username === record.username);
      if (userIndex < 0) throw statusError("Session user was not found. Please log in again.", 401);
      if (existing) return { record: existing, user: users[userIndex], created: false };

      const previousUsers = users.map((user) => ({ ...user }));
      const nextUser = incrementUserUsage(users[userIndex]);
      users[userIndex] = nextUser;
      records.push(record);
      audit.push(normalizeAuditEntry(auditEntry));
      try {
        await this.writeHistory(records);
        await this.writeUsers(users);
        ...
        await Promise.allSettled([
          this.writeHistory(records.filter((item) => item !== record)),
          this.writeUsers(previousUsers),
          this.writeAudit(audit.slice(0, -1))
        ]);
        throw error;
      }
      return { record, user: nextUser, created: true };
    });
  }
  ...
  async readStudentProfiles() {
    const parsed = await readJsonWithSeed(this.studentProfilesPath, this.seedStudentProfilesPath, []);
    return Array.isArray(parsed) ? parsed.map(normalizeStudentProfile) : [];
  }

  async writeStudentProfiles(profiles) {
    await writeJson(this.studentProfilesPath, profiles.map(normalizeStudentProfile));
  }

  async getDiagnosticResetManifest() {
    return buildResetManifest(this, await this.readHistory(), await this.readStudentProfiles(), await this.readUsers(), await this.readAudit());
  }

  async getProtectedDataSnapshot() {
    return { users: await this.readUsers(), audit: await this.readAudit() };
  }

  async clearDiagnosticData() {
    await this.writeHistory([]);
    await this.writeStudentProfiles([]);
...
class MemoryStorage {
  ...
  async readStudentProfiles() {
    if (!this.studentProfiles) {
      const parsed = await readJson(this.studentProfilesPath, []);
      this.studentProfiles = Array.isArray(parsed) ? parsed.map(normalizeStudentProfile) : [];
    }
    return this.studentProfiles.map((profile) => ({ ...profile }));
  }
...
class BlobJsonStorage {
  ...
  async readStudentProfiles() {
    const seeded = await this.readSeededJson("student-profiles.json", this.studentProfilesPath, []);
    return Array.isArray(seeded) ? seeded.map(normalizeStudentProfile) : [];
  }

  async writeStudentProfiles(profiles) {
    await this.writeBlobJson("student-profiles.json", profiles.map(normalizeStudentProfile));
  }
...
function validateNewStudentProfile(ownerAccountId, displayName) {
  ...
  const name = normalizeStudentDisplayName(displayName);
  if (!owner) throw statusError("Student profile owner is required.", 400);
  if (!name) throw statusError("Student name is required.", 400);
  if ([...name].length > 80) throw statusError("Student name must be 80 characters or fewer.", 400);
  return normalizeStudentProfile({ ownerAccountId: owner, displayName: name });
}

function buildResetManifest(storage, history, profiles, users, audit) {
  return {
    adapter: storage.name,
    diagnosticStores: [
      {
        store: storage.historyPath || DEFAULT_HISTORY_FILE,
        dataType: "diagnostic reports, submissions, progress and duplicate-report cache",
        recordCount: history.length,
        willClear: true
      },
      {
        store: storage.studentProfilesPath || DEFAULT_STUDENT_PROFILES_FILE,
        dataType: "student profiles",
    ...
    protectedStores: [
      {
        store: storage.usersPath || DEFAULT_USERS_FILE,
        dataType: "users, credentials, roles, quota, package access and expiry",
        recordCount: users.length,
        willClear: false
      },
      {
        store: storage.auditPath || DEFAULT_AUDIT_FILE,
        dataType: "security and usage audit",
