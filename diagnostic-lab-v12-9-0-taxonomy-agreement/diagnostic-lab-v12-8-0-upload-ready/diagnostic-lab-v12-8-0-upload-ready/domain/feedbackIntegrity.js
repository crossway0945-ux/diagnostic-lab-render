  ...
  for (const record of records) {
    ...
    if (!completeness.complete) {
      ...
      candidates.push({
        ...
        exactEvidence: record.exactText,
        exactSentence: record.exactText,
        diagnosis: "The sentence begins with a dependent or non-finite structure but has no independent main clause.",
        whyItLimitsBand: "The fragment cannot function as a complete proposition, so it weakens sentence control and the paragraph function it is meant to perform.",
        studentAction: "Turn the fragment into a complete sentence by supplying an independent subject-and-verb clause while preserving its original paragraph function.",
        targetedRevision: corrected,
...
function normalizeIssueCategory(value, card = {}, taskType = "") {
  ...
  const explicitRouteCard = task2 && ["Position Clarity", "Thesis Route Clarity", "Cause Route Clarity", "Solution Route Clarity", "Body Route Alignment", "Prompt Coverage"].includes(explicitCategory) &&
    /position|qualified|thesis|route|prompt|judgement/i.test([card.diagnosis, card.kruPomDiagnosis, card.whyItLimitsBand].filter(Boolean).join(" "));
  // A route repair can quote a sentence that also contains a local word-form error. The local
  // validator may record that secondary defect, but it must not replace the card's explicit route
  // category or demote the task-level repair in Top Issues.
  if (explicitRouteCard) return explicitCategory;
