...
export function buildCanonicalConsistencyAudit({
  ...
  const profileParagraphLabels = new Set(
    (Array.isArray(languageProfile?.validatedIssues) ? languageProfile.validatedIssues : [])
      .filter((item) => String(item.classification || "").toLowerCase() !== "awkward-but-understandable")
...
export function buildEvidenceBasedRepairPlan(issues = [], reportLanguage = "en", requiredDays = 7, context = {}) {
  ...
  const qualifiedState = context.routeAssessment?.qualifiedPositionState || context.routeAssessment?.semanticPosition?.finalPositionState;
  if (qualifiedState?.qualified && unique.length) {
    const findIssue = (pattern, paragraphPattern = null) => unique.find((issue) =>
      pattern.test(String(issue.issueCategory || issue.primaryCategory || "")) &&
      (!paragraphPattern || paragraphPattern.test(String(issue.paragraphLabel || issue.paragraphLocation || "")))
    ) || ranked.find((issue) =>
      pattern.test(String(issue.issueCategory || issue.primaryCategory || "")) &&
      (!paragraphPattern || paragraphPattern.test(String(issue.paragraphLabel || issue.paragraphLocation || "")))
    );
    const routeIssue = findIssue(/Position Clarity|Thesis Route Clarity|Route Alignment|Prompt Coverage/) || unique[0];
    const body1Issue = routeIssue;
    const body2Issue = routeIssue;
    const fragments = ranked.filter((issue) => /Sentence Completion/.test(String(issue.issueCategory || issue.primaryCategory || "")));
    const damagedBody = ranked.find((issue) => /Body Paragraph 1/.test(String(issue.paragraphLabel || issue.paragraphLocation || "")) && /Grammar and Sentence Control|Verb Form|Modal/.test(String(issue.issueCategory || issue.primaryCategory || ""))) || body1Issue;
    const recurringLanguage = ranked.find((issue) => /Word Form|Spelling|Collocation|Grammar|Sentence|Verb|Preposition/.test(String(issue.issueCategory || issue.primaryCategory || ""))) || body2Issue;
    const plan = [
      [routeIssue, "Clarify the qualified thesis", "Rewrite the thesis as one qualified judgement so the main position and the limited exception are explicit; do not turn the exception into a second global position."],
      [body1Issue, "Control the exception paragraph", "Label Body Paragraph 1 as the limited safety exception and make every sentence support only that exception."],
      [body2Issue, "Prove the main disagreement", "Use Body Paragraph 2 to show why the blanket rule fails for normal daily facilities, then link the result back to the qualified thesis."],
      [fragments[0] || body1Issue, "Repair both fragments", "Rewrite the two validated fragments as complete independent clauses while preserving their original paragraph functions."],
      [damagedBody, "Repair the damaged Body 1 sentence", "Rebuild the most structurally damaged Body Paragraph 1 sentence with one controlled subject-verb frame and the same safety claim."],
      [recurringLanguage, "Control recurring language", "Correct the recurring verb-complement, word-form, spelling and modifier/collocation patterns, then scan the full response for the same patterns."],
      [routeIssue, "Transfer to a new Opinion prompt", "Plan a new qualified Opinion response: write one scoped thesis, one exception paragraph, one main-position paragraph and a conclusion that preserves exactly the same scope."]
    ];
    return plan.slice(0, requiredDays).map(([issue, title, task], index) => ({
      day: index + 1,
      issueId: String(issue?.issueId || routeIssue?.issueId || ""),
      title: `${String(issue?.issueCategory || issue?.primaryCategory || routeIssue?.issueCategory || "Focused repair")}: ${title}`,
      task,
      source: "canonical-qualified-position-plan"
    }));
  }
  if (!unique.length) {
...
function repairPlanPriority(issue = {}) {
  ...
  if (["Position Clarity", "Thesis Route Clarity", "Body Route Alignment", "Prompt Coverage"].includes(category)) return -10;
