...
function rebuildParagraphCoverageFromIssues(paragraphFeedback = [], issues = [], conclusionFunction = null, routeAssessment = null) {
  ...
  return paragraphFeedback.map((paragraph) => {
    ...
    if (qualifiedState?.qualified && /Introduction/i.test(paragraphLabel) && qualifiedState.thesisClarity !== "clear") {
      ...
      status = status.replace(/^Route Aligned/, "Exception/Concession Route Aligned");
    } else if (bodyRole === "main-position-support") {
      status = status.replace(/^Route Aligned/, "Main-Position Route Aligned");
    }
...
const THAI_DISCLAIMER = "รายงานวินิจฉัยนี้เป็นการประเมินช่วงคะแนนโดยอิงเกณฑ์ IELTS Writing และกรอบการสอนของ Kru Pom IELTS ไม่ใช่คะแนน IELTS อย่างเป็นทางการ และไม่ทดแทนการประเมินโดยผู้ตรวจ IELTS ที่ได้รับการรับรอง";
...
function buildLocalTask2Analysis(payload) {
  ...
  const cards = [];
  const used = new Set();
  for (const issue of (safety.languageProfile?.validatedIssues || []).slice(0, 4)) {
    const repaired = repairDeterministicLanguageSentence(issue.exactSentence);
    addCard(cards, used, {
      issueType: issue.criterion === "Lexical Resource" ? "Vocabulary Precision" : "Grammar and Sentence Control",
      severity: issue.classification === "clear-error" ? "Moderate" : "Minor Repair",
      criteria: [issue.criterion],
      framework: [issue.category === "vague noun choice" ? "Vocabulary Precision" : "Clear / Concise / Precise / Comprehensive Control"],
      paragraphLocation: records.find((record) => normalizeEvidenceText(record.sentence) === normalizeEvidenceText(issue.exactSentence))?.location || "Full response",
      exactSentence: issue.exactSentence,
      sentenceFunction: "This sentence contributes to the paragraph's explanation or evidence route.",
      whyItLimitsBand: `The ${issue.category} issue reduces precision or sentence control in the quoted evidence.`,
      kruPomDiagnosis: issue.explanation,
      targetedRevision: repaired,
      revisionType: "Minimal Correction",
      whyRevisionIsStronger: "The revision repairs the diagnosed language pattern while preserving the original claim and argumentative direction.",
      studentAction: `Check the full response for the same ${issue.category} pattern and repair each verified occurrence.`
    });
  }
  ...
  if (!cards.length && !(safety.canonicalDevelopmentEvidence?.findings || []).length) {
    ...
    addCard(cards, used, {
      ...
      framework: ["Body Paragraph Route Alignment", "Vocabulary Precision"],
      paragraphLocation: records.find((record) => record.sentence === evidence)?.location || "Full response",
      exactSentence: evidence,
      sentenceFunction: "This sentence establishes or develops a route that is already broadly controlled.",
      whyItLimitsBand: "The route is functional; the remaining opportunity is tighter precision, balance and explicit control of the relationship between the two sides.",
      kruPomDiagnosis: "This is a refinement rather than a major weakness, so the writer's existing position and evidence route should be preserved.",
      targetedRevision: evidence,
      revisionType: "High-Band Refinement",
      whyRevisionIsStronger: "The original sentence is retained because it is already natural and route-consistent; the refinement work belongs in the surrounding explanation.",
      studentAction: "Refine the surrounding paragraph for balance and precision without forcing a formulaic link-back."
    });
  }
  ...
  return buildAnalysisFromCards({
    ...
    mainScoreLimitingFactor: "The main limiting factor is the highest-severity evidence-based issue identified across route development, precision and language control.",
    mostUrgentRepair: "Repair the highest-severity verified issue first, then check that the position, body routes and conclusion remain consistent.",
...
function buildTask1IntroWhyStronger(payload) {
  ...
  const unit = extractMeasurementUnit(payload.prompt);
  const period = extractTimePeriod(payload.prompt);
  const details = [
    visualType ? `identifies the ${visualType}` : "identifies the visual type",
    unit ? `preserves the unit ${unit}` : "does not invent an unsupported unit",
    items.length ? `names the short category list (${formatInlineList(items)})` : "keeps the category scope controlled",
    frequency ? `preserves the qualifier ${frequency}` : "does not invent a frequency qualifier",
    period ? `keeps the timeframe ${period}` : "does not invent a timeframe"
  ];
  return `This version ${details.join(", ")} and remains a neutral Task 1 introduction.`;
}
...
function buildTask1IntroTargetedRevision(payload) {
  ...
  if (/table/.test(visualType)) return buildTableIntroRevision(promptText);
  if (/line/.test(visualType)) return buildLineIntroRevision(promptText);
  return buildBarIntroRevision(promptText, payload);
}

function countTask1VisualTypes(value) {
...
function buildLineIntroRevision(prompt) {
  ...
  const unit = extractMeasurementUnit(prompt);
  const items = extractShortTask1ItemList(prompt);
  const details = [
    unit && !normalizedSubject.includes(unit.toLowerCase()) ? `measured in ${unit}` : "",
    items.length && !items.every((item) => normalizedSubject.includes(String(item).toLowerCase())) ? `across ${formatInlineList(items)}` : "",
    period && !normalizedSubject.includes(period.toLowerCase()) ? period : ""
  ].filter(Boolean).join(", ");
  return `The line graph compares changes in ${subject}${details ? `, ${details}` : ""}.`;
}
...
function buildBarIntroRevision(prompt, payload = {}) {
  ...
  const period = extractTimePeriod(prompt);
  const details = [
    unit && !normalizedSubject.includes(unit.toLowerCase()) ? `${/^per\b/i.test(unit) ? "expressed" : "measured in"} ${unit}` : "",
    period && !normalizedSubject.includes(period.toLowerCase()) ? period : ""
  ].filter(Boolean).join(", ");
  return `The ${descriptor.noun} ${verb} figures for ${subject}${details ? `, ${details}` : ""}.`;
}
...
function buildDiagramIntroRevision(prompt) {
  const structure = String(prompt || "").match(/(?:structure|components?)\s+of\s+(.+?)\s+and\s+how\b/i);
  const operation = String(prompt || "").match(/(?:used|designed)\s+to\s+([^.?]+)/i);
  if (structure && operation) {
    const device = structure[1].replace(/^(?:a|an|the)\s+/i, "").trim();
    const action = operation[1].replace(/^heat\b/i, "warm").trim();
    return `The diagrams compare the basic components of a ${device} and illustrate how the device can ${action}.`;
  }
  const subject = compactTask1Prompt(prompt);
...
function extractLocationPhrase(value) {
  const text = String(value || "");
  const inLocation = text.match(/\bin\s+(the\s+United Kingdom|the\s+UK|Australia|Britain|Turkey|India|Canada|Japan|Germany|France|City\s+[A-Z])\b/i);
...
function ensureTask2SafetyCards(cards, payload, safety) {
  ...
  if (thesisNeedsRepair && !qualifiedThesisNeedsRepair) {
    const introductionRecords = getSentenceRecords(payload.writing, "Task 2")
      .filter((record) => record.paragraphIndex === 0);
    const evidenceRecord = introductionRecords.find((record) =>
      /\b(?:agree|disagree|position|opinion|discuss|extent|view)\b/i.test(record.sentence)
    ) || introductionRecords[0];
    if (evidenceRecord) {
      const safeThesisRevision = String(evidenceRecord.sentence)
        .replace(/\bdiscuss\s+about\s+agree\s+or\s+disagree\s+(?:for|with)\b/i, "discuss whether I agree or disagree with")
        .replace(/\bdiscuss\s+agree\s+or\s+disagree\s+(?:for|with)\b/i, "discuss whether I agree or disagree with")
        .replace(/\s+/g, " ")
        .trim();
      upsertTask2CoverageCard(output, used, {
        issueType: "Thesis Route Clarity",
        severity: safety.positionConfidence === "low" ? "Critical" : "Major",
        criteria: ["Task Response", "Coherence & Cohesion"],
        framework: ["Thesis Route Clarity", "Body Paragraph Route Alignment"],
        paragraphLocation: evidenceRecord.location,
        exactSentence: evidenceRecord.sentence,
        sentenceFunction: "This sentence is intended to state the writer's position and establish the route for the body paragraphs.",
        whyItLimitsBand: "The introduction does not yet state a sufficiently explicit position-and-reason route for the opinion task.",
        kruPomDiagnosis: "The thesis must make the required judgement clear and preview only the reasons that the body paragraphs actually develop.",
        targetedRevision: safeThesisRevision,
        revisionType: normalizeEvidenceText(safeThesisRevision) === normalizeEvidenceText(evidenceRecord.sentence)
          ? "High-Band Refinement"
          : "Route-Preserving Revision",
        whyRevisionIsStronger: "The revision preserves the student's stated route and repairs only recoverable wording; the writer must still supply an explicit judgement rather than having the system invent one.",
        studentAction: "Choose the exact extent of agreement first, rewrite the thesis around that judgement, and verify that Body Paragraphs 1 and 2 prove the same route."
      });
    }
  }
...
function ensureTask2ReportCompleteness(output, used, payload) {
  const records = getSentenceRecords(payload.writing, payload.taskType);
  const order = new Map(records.map((record, index) => [normalizeEvidenceText(record.sentence), index]));
  output.sort((left, right) => {
    const leftIndex = order.get(normalizeEvidenceText(left.exactSentence));
    const rightIndex = order.get(normalizeEvidenceText(right.exactSentence));
...
function buildTask2CoverageTopIssueCards(cards, payload) {
  ...
  const severityRank = { Critical: 5, Major: 4, Moderate: 3, "Minor Repair": 2, "High-Band Refinement": 1, "Pass / Strong": 0 };
  const ordered = [...source].sort((left, right) => (severityRank[right.severity] ?? 2) - (severityRank[left.severity] ?? 2));
  const groups = new Map();
  for (const card of ordered) {
    const category = feedbackCardCategory(card);
  ...
  for (const group of groups.values()) {
    ...
    output.push(group.length === 1 ? representative : {
      ...representative,
      scope: "multi-location",
      paragraphLocations: [...new Set(group.map((card) => card.paragraphLocation).filter(Boolean))],
      evidenceItems: group.slice(0, 6).map((card) => ({
        paragraphLocation: card.paragraphLocation,
        exactSentence: card.exactSentence,
        evidenceRole: `${card.issueType || "Diagnostic"} evidence`
      })),
      summary: `${representative.issueType}: ${group.length} evidence locations across the response.`,
      studentAction: representative.studentAction
    });
    if (output.length >= 5) break;
  }
  for (const card of ordered) {
    if (output.length >= 5) break;
    if (!output.includes(card) && !output.some((item) => normalizeEvidenceText(item.exactSentence) === normalizeEvidenceText(card.exactSentence))) output.push(card);
  }
  return output;
}
...
function localizeExecutiveFields(payload = {}, mainScoreLimitingFactor = "", mostUrgentRepair = "") {
  ...
  const safety = payload.taskType === "Task 2" ? (payload.task2Safety || analyzeTask2Safety(payload)) : null;
  const routeStatus = safety?.routeAssessment?.status || "";
  return {
    mainScoreLimitingFactor: payload.taskType === "Task 2"
      ? `งานมีเส้นทางการตอบโจทย์ระดับ ${routeStatus || "ที่ตรวจพบ"} แต่ยังต้องยกระดับความลึกของคำอธิบาย ความแม่นยำของภาษา และการควบคุมประโยคจากหลักฐานในรายงาน`
      : "รายงานต้องยกระดับการเลือก key features, grouping, data accuracy และการควบคุมภาษาโดยอ้างอิงจาก visual จริง",
    mostUrgentRepair: payload.taskType === "Task 2"
      ? "ซ่อมประเด็นที่มี severity สูงสุดก่อน จากนั้นตรวจความสัมพันธ์ระหว่าง Thesis, Body route, Conclusion และภาษาเต็มทั้งงาน"
      : "ซ่อม Overview และ key-feature route ก่อน แล้วตรวจข้อมูล หน่วย เวลา grouping และภาษาเต็มทั้งรายงาน"
  };
}

function localizeFrameworkScores(scores = {}, payload = {}) {
  if (normalizeReportLanguage(payload.reportLanguage) !== "th") return scores;
  return Object.fromEntries(Object.entries(scores || {}).map(([name, value]) => [name, {
    ...(value || {}),
    diagnosis: `หัวข้อ ${name} อยู่ในระดับ ${value?.status || "ที่ตรวจพบ"} โดยพิจารณาจากหลักฐานจริงในบทเขียน ไม่ได้ตัดสินจากการมีคำเชื่อมหรือโครงสร้าง template เพียงอย่างเดียว`
  }]));
}

function localizeFeedbackCards(cards = [], payload = {}) {
  if (normalizeReportLanguage(payload.reportLanguage) !== "th") return cards;
  return cards.map((card) => ({
    ...card,
    sentenceFunction: `ประโยคนี้ทำหน้าที่ในส่วน ${card.paragraphLocation || "ที่ระบุ"} และต้องสอดคล้องกับเส้นทางหลักของย่อหน้า`,
    whyItLimitsBand: `ประเด็น ${card.issueType || "ที่ตรวจพบ"} ทำให้ความชัดเจน ความแม่นยำ หรือการพัฒนาเหตุผลยังไม่สม่ำเสมอพอสำหรับช่วงคะแนนเป้าหมาย`,
    kruPomDiagnosis: `จากหลักฐานใน ${card.paragraphLocation || "บทเขียน"} ควรซ่อมเฉพาะจุดที่วินิจฉัย โดยรักษาความหมาย จุดยืน และหน้าที่เดิมของประโยคไว้`,
    whyRevisionIsStronger: `ฉบับแก้ไขควบคุมภาษาและความสัมพันธ์ของใจความได้ชัดขึ้น โดยไม่เปลี่ยนเส้นทางเหตุผลหรือเพิ่มข้ออ้างใหม่ที่ไม่มีในต้นฉบับ`,
    studentAction: `แก้ประเด็น ${card.issueType || "นี้"} ในตำแหน่งที่อ้างถึง แล้วตรวจหารูปแบบเดียวกันตลอดทั้งงานก่อนส่งครั้งถัดไป`
  }));
}

function localizeTopIssues(issues = [], payload = {}) {
  if (normalizeReportLanguage(payload.reportLanguage) !== "th") return issues;
  return issues.map((issue) => ({
    ...issue,
    diagnosis: `ประเด็น ${issue.issueType || "สำคัญ"} เชื่อมกับหลักฐานที่ระบุและมีผลต่อเกณฑ์คะแนนที่แสดงในรายงาน`,
    summary: `ควรแก้ ${issue.issueType || "ประเด็นนี้"} ตามหลักฐานจริงก่อน เพื่อเพิ่มความชัดเจนและความสม่ำเสมอของงาน`,
    studentAction: `ทบทวนทุกตำแหน่งหลักฐานของ ${issue.issueType || "ประเด็นนี้"} แล้วซ่อมให้ครบโดยไม่เปลี่ยนความหมายเดิม`
  }));
}

function localizeParagraphGuidance(items = [], payload = {}) {
  if (normalizeReportLanguage(payload.reportLanguage) !== "th") return items;
  return items.map((item) => ({
    ...item,
    diagnosis: `ย่อหน้า ${item.paragraph || item.paragraphLocation || "นี้"} ต้องควบคุมหน้าที่ของแต่ละประโยคและเชื่อมหลักฐานเข้ากับใจความหลักให้ชัดเจน`,
    action: `ตรวจ claim, explanation, evidence และการปิดย่อหน้าให้สัมพันธ์กัน โดยแก้เฉพาะจุดที่มีหลักฐานรองรับ`
  }));
}

function localizePracticePlan(items = [], payload = {}) {
  if (normalizeReportLanguage(payload.reportLanguage) !== "th") return items;
...
function normalizeAnalysis(analysis, payload) {
  ...
  const normalized = {
    ...
    practicePlan: studentPracticePlan,
...
function collectReportLanguageIssues(analysis = {}, payload = {}) {
  ...
  if (language === "en") {
    ...
    return unexpectedThai ? [{
      code: "REPORT_LANGUAGE_MISMATCH",
      message: `English report contains Thai explanatory text (${unexpectedThai.slice(0, 90)}).`
    }] : [];
  }
...
export function validateReportOutput(analysis, payload = {}) {
  ...
  if (analysis.feedbackIntegrityValidationIssues?.length) {
    throw reportOutputValidationError(analysis.feedbackIntegrityValidationIssues, "feedback_integrity");
  }
...
function collectTask2ReportOutputIssues(analysis, payload) {
  ...
  for (const [index, issue] of (analysis.top3Issues || []).entries()) {
    ...
    const issueCategory = feedbackIssueCategory(issue.issueCategory)
      || feedbackIssueCategory([issue.issueType, issue.title].filter(Boolean).join(" "))
      || feedbackIssueCategory(issue.summary);
...
function reportOutputValidationError(issues, stage = "report_validation") {
  const uniqueIssues = Array.from(new Map((issues || []).map((issue) => {
    const message = typeof issue === "object" && issue ? String(issue.message || "Report output validation failed.") : String(issue);
    return [`${typeof issue === "object" && issue ? issue.code || "" : ""}|${message}`, { issue, message }];
  })).values());
  const validationDetails = uniqueIssues.map(({ issue, message }) => ({
    code: typeof issue === "object" && issue?.code ? issue.code : validationIssueCode(message),
...
function feedbackCardCategory(card = {}) {
  const canonicalCategory = feedbackIssueCategory(card.issueCategory);
