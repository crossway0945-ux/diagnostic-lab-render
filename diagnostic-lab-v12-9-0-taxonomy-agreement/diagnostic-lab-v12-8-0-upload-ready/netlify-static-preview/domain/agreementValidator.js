...
export function detectSyntacticHeadAgreementDefects(evidence = "", absoluteStart = 0) {
  ...
  for (const match of text.matchAll(FINITE_PATTERN)) {
    ...
    // A base-form verb governed by a modal, auxiliary DO, or infinitive marker is not a finite
    // agreement target. Without this guard, correct strings such as “can help” and “to make” were
    // misread as matrix verbs and harmful revisions such as “can helps” / “to makes” were generated.
    // In ambiguous non-finite frames we deliberately decline to assert an agreement defect.
    if (isGovernedNonFiniteVerb(text, verbStart)) continue;
...
function isGovernedNonFiniteVerb(text = "", verbStart = -1) {
  if (!Number.isInteger(verbStart) || verbStart <= 0) return false;
  const prefix = String(text || "").slice(0, verbStart);
  // Allow a small number of intervening adverbs or negation: “can only help”, “does not make”,
  // “to significantly reduce”. The lexical verb remains non-finite in each frame.
  return /\b(?:can|could|may|might|must|shall|should|will|would|do|does|did|to)\s+(?:(?:not|never|only|also|still|usually|often|significantly|dramatically|substantially|directly|easily|better|more)\s+){0,3}$/i.test(prefix);
}
