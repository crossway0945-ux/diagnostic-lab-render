...
import { projectRouteAlignmentDisplay } from "./feedbackIntegrity.js";
...
export function buildCanonicalAnalysis({
  ...
  if (taskType === "Task 2" && analysis.canonicalTask2Analysis) {
    ...
    const finalValidatedState = analysis.feedbackIntegrity?.finalValidatedState?.terminal === true
      ? analysis.feedbackIntegrity.finalValidatedState
      : null;
    ...
    const display = projectCanonicalTask2Framework({
      ...
      paragraphCoverage,
      thesisDimensions,
      frameworkScores: finalValidatedState?.frameworkScores || analysis.feedbackIntegrity?.frameworkScores || {}
...
export function projectCanonicalTask2Framework(canonical = {}, reportLanguage = "") {
  ...
  const projected = {
    ...
    "Position Clarity": integrityFramework["Position Clarity"] || (canonical.taskRequirements?.stanceRequired
      ? {
    ...
    "Body Paragraph Route Alignment": integrityFramework["Body Paragraph Route Alignment"] || projectRouteAlignmentDisplay({
      status: routeStatus,
    ...
    "Link Back Control": integrityFramework["Link Back Control"] || {
  ...
  // A terminal report must not be reinterpreted by this projection. Each supported display card
  // above reads the immutable terminal framework record first, while the established public card
  // set remains unchanged for backwards compatibility.
  return projected;
}
