...
const STUDENT_SECTION_FIELDS = Object.freeze({
  ...
  languagePatternSummary: [
    "category", "paragraphLocation", "targetSpan", "diagnosis", "action", "recurrence",
    "revisionType", "targetedRevision", "whyRevisionIsStronger"
  ],
...
function publicFeedback(card = {}) {
  ...
  return {
    ...
    revisionType: publicRevisionType(card.revisionType),
...
function publicRevisionType(type) {
  return {
    "Minimal Correction": "Minimal Correction",
    "Route-Preserving Revision": "Route-Preserving Revised Version",
    "Teacher-Guided Expansion": "Teacher-Guided Revised Version",
    "High-Band Refinement": "Minimal Correction"
...
function publicLanguagePattern(row = {}) {
  return {
    ...
    revisionType: publicRevisionType(row.revisionType),
    targetedRevision: String(row.targetedRevision || ""),
    whyRevisionIsStronger: String(row.whyRevisionIsStronger || "")
  };
}
