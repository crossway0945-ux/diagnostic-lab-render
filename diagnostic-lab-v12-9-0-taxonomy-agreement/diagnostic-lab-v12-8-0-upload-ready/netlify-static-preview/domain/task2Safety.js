...
export function analyzeTask2Safety(payload = {}) {
  ...
  const positionBodyRoutes = stanceRequired
    ...
    : [];
  const qualifiedPositionState = stanceRequired && essayRoute === TASK2_CANONICAL_TYPES.OPINION
    ? buildQualifiedOpinionPositionState({
  ...
  const detectedPosition = stanceRequired
    ? reconcilePosition({ introPosition, conclusionPosition, unfinishedEndingDetected, bodyRoutes: positionBodyRoutes, qualifiedPositionState })
    : "";
  const positionConfidence = stanceRequired
    ? positionConfidenceFor({ introPosition, conclusionPosition, unfinishedEndingDetected, bodyRoutes: positionBodyRoutes, qualifiedPositionState })
    : "not-applicable";
  ...
  calibrateOpinionSupportQuality(routeAssessment, languageProfile);
...
export function classifyTask2Prompt(payload = {}) {
  ...
  const promptObligations = winner.promptObligations?.length
    ? winner.promptObligations
    : requiredRoutesForType(effectivePublicFamily, effectiveInternalSubtype, false, promptParts.length)
      .map((route, index) => promptObligation(`route-${index + 1}`, route, promptParts[index] || prompt, false, "develop"));
  // A public Direct Question can still contain a required judgement. This is common in two-part
  // and hybrid wording such as "Why ... and should ...?" or "Why ... and do the advantages
  // outweigh ...?". The judgement belongs to the prompt obligation; counting question marks alone
  // must not make it disappear.
  const directQuestionJudgementRequired = effectivePublicFamily === TASK2_PUBLIC_FAMILIES.DIRECT_QUESTION
    && promptObligations.some((item) => item.judgementRequired);
  const stanceRequired = isStanceRequired(
    { ...payload, essayType: TASK2_PUBLIC_TYPE_LABELS[effectivePublicFamily] },
    effectivePublicFamily,
    effectiveInternalSubtype
  ) || directQuestionJudgementRequired;
  ...
  const hybridPrompt = effectivePublicFamily === TASK2_PUBLIC_FAMILIES.DIRECT_QUESTION
    && effectiveInternalSubtype === TASK2_INTERNAL_SUBTYPES.MULTI_QUESTION
    && promptObligations.length >= 2
    && promptObligations.some((item) => item.judgementRequired)
    && promptObligations.some((item) => !item.judgementRequired);
  const comparisonRequired = effectiveInternalSubtype === TASK2_INTERNAL_SUBTYPES.OUTWEIGH
    || promptObligations.some((item) => item.judgementRequired && /\boutweigh\b/i.test(item.questionText));
  ...
  return {
    ...
    // Prompt subtype / route mode. "Outweigh" qualifies Advantages & Disadvantages — it is never a
    // separate top-level essay type. outweighDirection records which side the prompt asks the writer
    // to weigh as greater, and comparisonRequired marks that an explicit comparative judgement is
    // part of the task, evaluated separately from route presence.
    promptSubtype: effectiveInternalSubtype === TASK2_INTERNAL_SUBTYPES.OUTWEIGH ? "outweigh" : hybridPrompt ? "hybrid" : "",
    outweighDirection: comparisonRequired
      ? (/\bdisadvantages?\b[^.?]{0,40}\boutweigh\b/i.test(prompt) ? "disadvantages" : /\badvantages?\b[^.?]{0,40}\boutweigh\b/i.test(prompt) ? "advantages" : "unspecified")
      : "",
    comparisonRequired,
...
function internalSubtypeLabel(publicFamily, internalSubtype) {
  const labels = {
    // "Outweigh" is a prompt subtype of Advantages & Disadvantages, never a separate essay type, so
    // the label must read as a subtype qualifier and not as a standalone type name.
    [TASK2_INTERNAL_SUBTYPES.OUTWEIGH]: "Advantages & Disadvantages (Outweigh)",
...
function extractPromptParts(prompt) {
  ...
  const questions = (text.match(/[^?]+\?/g) || [])
    .flatMap((item) => splitCoordinatedPromptQuestion(item));
  return questions.length ? questions : text ? [text] : [];
}

function splitCoordinatedPromptQuestion(question) {
  const text = String(question || "").trim();
  const body = text.replace(/\?+\s*$/, "").trim();
  // Some legitimate two-part prompts use one final question mark:
  // "What motivates X, and how does X affect Y?". Split only when the coordinated clause begins
  // with an interrogative or auxiliary; ordinary compound objects such as "advantages and
  // disadvantages" remain one clause.
  const parts = body.split(
    /\s*(?:,\s*)?(?:and|while)\s+(?=(?:why|how|what|which|who|where|when|is|are|was|were|do|does|did|should|can|could|would|will|has|have)\b)/i
...
function scoreParagraphForQuestion(paragraph, question) {
  ...
  // Use modal wording as a routing signal only when the obligation itself begins with the modal.
  // An Opinion prompt such as "To what extent ... areas should ..." contains `should` inside its
  // proposition; treating that embedded word as a question-route marker can misroute normal body
  // paragraphs and destabilise otherwise valid evidence cards.
  if (/^(?:should|ought)\b/i.test(promptPart.trim()) && /\b(?:should|ought to)\b/i.test(text)) score += 5;
  if (/\boutweigh\b/i.test(promptPart) && (
    /\boutweigh(?:s|ed|ing)?\b/i.test(text)
    || /\b(?:advantages?|benefits?)\b[^.!?]{0,100}\b(?:stronger|greater|more important|more significant)\b[^.!?]{0,80}\b(?:disadvantages?|drawbacks?)\b/i.test(text)
    || /\b(?:disadvantages?|drawbacks?)\b[^.!?]{0,100}\b(?:stronger|greater|more important|more significant)\b[^.!?]{0,80}\b(?:advantages?|benefits?)\b/i.test(text)
    || /\bmatter(?:s|ed)? more than\b/i.test(text)
  )) score += 7;
  return score;
}
...
function buildOpinionRouteAssessment(context) {
  const bodyRoutes = context.bodyParagraphs.map((paragraph, index) => {
    const opinionRoute = classifyOpinionBodyRoute({
      ...
      qualifiedPositionState: context.qualifiedPositionState
    });
    return routeItem(
      index,
      opinionRoute.label,
  ...
  const conflict = !context.qualifiedPositionState?.qualified && (directConflict || (
    missingPosition &&
  ...
  const qualifiedThesisNeedsRepair = context.qualifiedPositionState?.qualified && context.qualifiedPositionState.thesisClarity !== "clear";
  const status = conflict
    ? "conflicting"
    : missingRequirements.length
      ? "failed"
      : qualifiedThesisNeedsRepair || bodyRoutes.some((item) => item.status === "partial")
        ? "partial"
        : "controlled";
  ...
  return finalizeRouteAssessment(context, {
    ...
    requirements: [
      requirement("position", "Clear extent of agreement", !missingPosition, context.qualifiedPositionState?.finalPositionEvidence || context.introPosition || firstSentence(context.introduction)),
    ...
    position: context.detectedPosition,
    qualifiedPositionState: context.qualifiedPositionState,
    overallRouteLabel: context.qualifiedPositionState?.overallRouteLabel,
    recommendedRoute: context.qualifiedPositionState?.qualified
      ? "Clarify the qualified position in the thesis, keep the selective exception distinct, make the main-position paragraph reject the blanket rule, and preserve that same scope in the conclusion."
      : "",
    recommendedRouteRationale: context.qualifiedPositionState?.qualified
      ? "The response contains a coherent qualified position. The repair must clarify its scope in the thesis without converting the exception into a global contradiction."
      : ""
  });
}
...
function finalizeRouteAssessment(context, assessment) {
  ...
  const bodyRoutes = assessment.bodyRoutes.map((item, index) => {
    ...
    return {
      ...item,
      status,
      alignmentStatus: ["exception-concession", "main-position-support"].includes(String(item.routeRole || ""))
        ? ROUTE_COVERAGE.ADEQUATELY_DEVELOPED
        : /not clear|opposing or concessive|mixes supporting|contradict/i.test(item.label || "")
  ...
  const summary = [
    ...
    `Overall route status: ${assessment.overallRouteLabel || routeCoverageLabel(overallRouteStatus)}`
  ].join(" | ");
  return {
    ...assessment,
    requirements,
    ...
    confidence,
    summary,
    recommendedRoute: assessment.recommendedRoute || buildTaskTypeRecommendedRoute(context.essayRoute, context.stanceRequired, context.internalSubtype),
    recommendedRouteRationale: assessment.recommendedRouteRationale || buildTaskTypeRouteRationale(context.essayRoute, overallRouteStatus, context.internalSubtype),
...
function assessThesisRouteCoverage(context) {
  ...
  if (context.stanceRequired) {
    if (context.qualifiedPositionState?.qualified && context.qualifiedPositionState.thesisClarity !== "clear") {
      return ROUTE_COVERAGE.MENTIONED_ONLY;
    }
...
export function classifyConcessionStatus({ routeAssessment = {}, introduction = "", bodyParagraphs = [], introPosition = "", conclusionPosition = "", detectedPosition = "", qualifiedPositionState = null } = {}) {
  if (qualifiedPositionState?.qualified) return "Controlled selective exception";
  ...
  if (concessionRoutes.length) {
    if (/partly|partially|partial|balanced/i.test(detectedPosition || introPosition)) return "Controlled concession";
...
function routeItem(index, label, paragraph, status, details = {}) {
  const sentences = splitSentences(paragraph);
  const controllingSentence = sentences[0] || "";
  const exampleSupport = sentences.find((sentence) => /\b(?:for example|for instance|to illustrate)\b/i.test(sentence)) ||
    sentences.find((sentence) => /\bsuch as\b/i.test(sentence)) || "";
...
function buildStanceConclusionLabel(context) {
  ...
  if (context.qualifiedPositionState?.qualified) {
    return `clearly states ${context.qualifiedPositionState.finalPosition} and preserves the selective exception`;
  }
...
function countPromptQuestions(prompt) {
  const punctuationCount = (String(prompt || "").match(/\?/g) || []).length;
  return punctuationCount ? extractPromptParts(prompt).length : 0;
}
...
function buildSemanticPositionModel({
  ...
  if (qualifiedPositionState?.qualified) {
    const thesisNeedsRepair = qualifiedPositionState.thesisClarity !== "clear";
    return {
      positionRequired: true,
      writerJudgement: qualifiedPositionState.finalPosition,
      relationToPromptClaim: qualifiedPositionState.finalDirection === "disagree" ? "qualified disagreement" : "qualified agreement",
      positionClarity: thesisNeedsRepair ? "needs work in the thesis; clarified by the conclusion" : "clear qualified position",
      positionConsistency: "globally consistent qualified route",
      positionConfidence: thesisNeedsRepair ? "medium" : "high",
      stanceWordingQuality: thesisNeedsRepair ? "thesis locally contradictory; conclusion resolves the intended scope" : "controlled",
      concessionControl: "controlled selective exception",
      exactPositionEvidence: qualifiedPositionState.finalPositionEvidence,
      thesisPositionEvidence: qualifiedPositionState.thesisPositionEvidence,
      finalPositionEvidence: qualifiedPositionState.finalPositionEvidence,
      thesisClarity: qualifiedPositionState.thesisClarity,
      qualified: true,
      finalPositionState: qualifiedPositionState
    };
  }
...
function opinionPropositionPolarity(prompt) {
  ...
  return /\b(?:waste of money|not worth it|better spent(?: elsewhere| on [^.!?]+)?|should not (?:be )?(?:spend|fund|support|provide|allow)|not justified|unjustified|too (?:much|costly|expensive)|more beneficial ways? to spend)\b/i.test(source)
    ? "negative"
    : "positive";
}

function classifyOpinionBodyRoute({ paragraph, index, prompt, position, introduction, qualifiedPositionState = null }) {
  const text = String(paragraph || "");
  const qualifiedRole = qualifiedPositionState?.bodyRoles?.[index];
  if (qualifiedRole?.role === "exception-concession") {
    return {
      label: "functions as the selective exception/concession and supports only the limited category accepted by the writer",
      status: countWords(text) >= 55 ? "adequate" : "partial",
      routeRole: "exception-concession",
      propositionRelation: qualifiedRole.propositionRelation,
      routeRoleEvidence: qualifiedRole.evidence
    };
  }
  if (qualifiedRole?.role === "main-position-support") {
    return {
      label: "develops the main-position reason against applying the proposition as a blanket rule",
      status: countWords(text) >= 55 ? "adequate" : "partial",
      routeRole: "main-position-support",
      propositionRelation: qualifiedRole.propositionRelation,
      routeRoleEvidence: qualifiedRole.evidence
    };
  }
  ...
  const explicitConcession = /^(?:on (?:the )?one hand|admittedly|granted|although|while|even though|despite|it is true that)\b/i.test(first);
  const contrastOpener = /^on the other hand\b/i.test(first);
  ...
  const alternativePrioritySignals = negativeProposition && writerDisagrees
    ? countMatches(text, /\b(?:poverty|hunger|healthcare|food supply|basic needs|people in need|public services?|hospitals?|schools?|housing|problems? on earth|areas? that need financial support|should (?:use|invest|spend|allocate) (?:the |this )?money)\b/gi)
    : 0;
  ...
  const concession = explicitConcession ||
    alternativePrioritySignals >= 2 ||
    (contrastOpener && opposingSignals + alternativePrioritySignals > supportingSignals) ||
    opposingSignals > supportingSignals + 1 ||
    (negativeProposition && writerDisagrees && opposingSignals + alternativePrioritySignals > 0 && thesisOverlap < 2);
  ...
  if (concession) {
    return {
      ...
      status: (explicitReturn || /partly|partially|partial|balanced/i.test(position)) && countWords(text) >= 55 ? "adequate" : "partial"
    };
  }
...
function buildQualifiedOpinionPositionState({
  prompt = "",
  introduction = "",
  bodyParagraphs = [],
  conclusion = "",
  ...
  if (unfinishedEndingDetected || !conclusion) return null;
  const bodyRelations = bodyParagraphs.map((paragraph) => inferOpinionPropositionRelation(paragraph, prompt));
  const conclusionRelation = inferOpinionPropositionRelation(conclusion, prompt);
  const combined = `${introduction} ${bodyParagraphs.join(" ")} ${conclusion}`;
  const scoped = /\b(?:although|while|whereas|but|yet|except(?: for)?|only (?:for|when|if|in)|blanket|universal|across[- ]the[- ]board|not (?:every|all)|some (?:cases?|areas?|categories?|groups?|sites?)|certain (?:cases?|areas?|categories?|groups?|sites?))\b/i.test(conclusion) ||
    /\b(?:partly|partially|to some extent|in limited cases?|as a general rule)\b/i.test(combined);
  const hasSupportRoute = bodyRelations.includes("support") || conclusionRelation === "support" || conclusionRelation === "mixed";
  const hasOpposingRoute = bodyRelations.includes("oppose") || conclusionRelation === "oppose" || conclusionRelation === "mixed";
  if (!scoped || !hasSupportRoute || !hasOpposingRoute) return null;

  let finalDirection = positionDirection(conclusionPosition);
  if (!new Set(["agree", "disagree"]).has(finalDirection)) {
    if (/\b(?:reject|oppose|cannot support|should not adopt|should not apply)\b[^.!?]{0,100}\b(?:blanket|universal|all|every|entire|across[- ]the[- ]board)\b|\bnot (?:every|all)\b/i.test(conclusion)) {
      finalDirection = "disagree";
    } else if (/\b(?:support|endorse|accept|approve|sound|appropriate)\b[^.!?]{0,100}\b(?:generally|overall|as a rule|in most cases)\b/i.test(conclusion)) {
      finalDirection = "agree";
    }
  }
  if (!new Set(["agree", "disagree"]).has(finalDirection)) return null;

  const bodyRoles = bodyRelations.map((propositionRelation, index) => {
    const mainRelation = finalDirection === "disagree" ? "oppose" : "support";
    const role = propositionRelation === mainRelation
      ? "main-position-support"
      : propositionRelation && propositionRelation !== "mixed"
        ? "exception-concession"
        : "unresolved-qualified-role";
    return {
      index: index + 1,
      role,
      propositionRelation,
      evidence: firstSentence(bodyParagraphs[index] || "")
    };
  });
  if (!bodyRoles.some((item) => item.role === "main-position-support") || !bodyRoles.some((item) => item.role === "exception-concession")) {
    return null;
  }

  const thesisPositionEvidence = findExactPositionEvidence(introduction) || lastSentence(introduction);
  const finalPositionEvidence = findExactPositionEvidence(conclusion) || firstSentence(conclusion);
  const introDirection = positionDirection(introPosition);
  const locallyContradictory = hasLocalEvaluativeConflict(thesisPositionEvidence);
  const thesisClarity = !locallyContradictory && introDirection === finalDirection && !/balanced|conditional|unclear|contradictory/.test(introPosition)
    ? "clear"
    : locallyContradictory ? "locally-contradictory" : "initially-unclear";
  return Object.freeze({
    version: "qualified-opinion-position-v12.9.9",
    qualified: true,
    finalDirection,
    finalPosition: finalDirection === "disagree" ? "qualified disagreement" : "qualified agreement",
    thesisClarity,
    thesisPositionEvidence,
    finalPositionEvidence,
    conclusionRelation,
    conclusionResolvesPosition: Boolean(finalPositionEvidence) && ["support", "oppose", "mixed"].includes(conclusionRelation),
    bodyRoles: Object.freeze(bodyRoles.map((item) => Object.freeze(item))),
    overallRouteLabel: thesisClarity === "clear" ? "controlled qualified route" : "present but initially unclear",
    globalContradiction: false
  });
}

function inferOpinionPropositionRelation(value, prompt = "") {
  const text = String(value || "");
  const controlling = firstSentence(text) || text;
  const positive = /\b(?:advantage|benefit|beneficial|valuable|useful|effective|appropriate|convenien|safe(?:ty)?|protect|improve|reduce|easier|better|necessary|essential|sound)\w*\b/i.test(text);
  const negative = /\b(?:problem|harm|harmful|damage|damaging|burden|drawback|disadvantage|risk|worse|difficult|congestion|pollution|unsafe|unnecessary|cost|costly|expensive|charge|restrict|limit|reject|oppose|disagree)\w*\b/i.test(text);
  let support = countMatches(text, /\b(?:i\s+(?:strongly\s+|firmly\s+|generally\s+|partly\s+|partially\s+)?agree|i support|support(?:s|ed|ing)? (?:this|the) (?:rule|policy|proposal|claim|idea)|should (?:apply|be adopted|be retained)|ought to (?:apply|be adopted|be retained))\b/gi);
  let oppose = countMatches(text, /\b(?:i\s+(?:strongly\s+|firmly\s+|generally\s+|partly\s+|partially\s+|completely\s+)?disagree|reject(?:s|ed|ing)?|oppose(?:s|d|ing)?|should not (?:apply|be adopted|be imposed)|cannot support)\b/gi);

  const separationTask = /\b(?:separat|divid|split|segregat|isolat|zon)\w*\b/i.test(prompt);
  if (separationTask) {
    const separation = /\b(?:separat|divid|split|segregat|isolat|zon|kept? (?:apart|away)|stay far away|special (?:area|areas|district|districts))\w*\b/i.test(controlling);
    const integration = /\b(?:integrat|mix(?:ed|ing)?|combin|together|co[- ]?locat|shared|nearby|same (?:area|district|neighbou?rhood))\w*\b/i.test(controlling);
    const controllingPositive = /\b(?:advantage|benefit|beneficial|valuable|useful|effective|appropriate|convenien|safe(?:ty)?|protect|improve|reduce|easier|better|necessary|essential|sound)\w*\b/i.test(controlling);
    const controllingNegative = /\b(?:problem|harmful|damaging|burden|drawback|disadvantage|risk|worse|difficult|unsafe|unnecessary|reject|oppose|disagree)\w*\b/i.test(controlling);
    const preventedHarm = /\b(?:reduce|prevent|avoid|lower|without|won['’]?t|will not|no longer|far (?:from|away)|protect)\b[^.!?]{0,100}\b(?:harm|pollution|congestion|risk|traffic|emission|damage|danger|problem)\w*\b/i.test(text) ||
      /\b(?:harm|pollution|congestion|risk|traffic|emission|damage|danger|problem)\w*\b[^.!?]{0,100}\b(?:reduce|prevent|avoid|lower|decline|less|fewer)\w*\b/i.test(text);
    if (separation && (controllingPositive || preventedHarm)) support += 3;
    if (separation && controllingNegative && !preventedHarm) oppose += 3;
    if (integration && (controllingPositive || preventedHarm)) oppose += 3;
    if (integration && controllingNegative && !preventedHarm) support += 3;
  } else {
    const promptTerms = semanticContentTokens(prompt);
    const textTerms = new Set(semanticContentTokens(text));
    const overlap = [...new Set(promptTerms)].filter((token) => textTerms.has(token)).length;
    const preventedHarm = /\b(?:reduce|remove|prevent|avoid|lower|eliminate|without|protect)\w*\b[^.!?]{0,100}\b(?:harm|problem|difficult|risk|cost|pollution|congestion|damage|danger|burden)\w*\b/i.test(text);
    if (overlap >= 1 && positive && (!negative || preventedHarm)) support += 1;
    if (overlap >= 1 && negative && !positive && !preventedHarm) oppose += 1;
  }
...
function semanticContentTokens(value) {
  const stop = new Set(["should", "would", "could", "must", "all", "every", "into", "that", "this", "with", "from", "have", "been", "being", "area", "areas"]);
  return (String(value || "").toLowerCase().match(/[a-z][a-z-]{3,}/g) || [])
    .filter((token) => !stop.has(token))
    .map((token) => {
      if (/[^aeiou]ies$/.test(token)) return `${token.slice(0, -3)}y`;
      if (/ing$/.test(token) && token.length > 5) return token.slice(0, -3).replace(/(.)\1$/, "$1");
      if (/ed$/.test(token) && token.length > 5) return token.slice(0, -2);
      if (/s$/.test(token) && !/ss$/.test(token) && token.length > 4) return token.slice(0, -1);
      return token;
    });
}

function hasLocalEvaluativeConflict(value) {
  const text = String(value || "");
  const positive = /\b(?:beneficial|advantageous|effective|valuable|positive|good|better|improve|helpful|convenient)\b/i.test(text);
  const negative = /\b(?:problem|problems|harmful|damaging|negative|worse|burden|drawback|disadvantage|difficult)\b/i.test(text);
  return positive && negative;
}

function reconcilePosition({ introPosition, conclusionPosition, unfinishedEndingDetected, bodyRoutes, qualifiedPositionState = null }) {
  if (qualifiedPositionState?.qualified) return qualifiedPositionState.finalPosition;
...
function positionConfidenceFor({ introPosition, conclusionPosition, unfinishedEndingDetected, bodyRoutes, qualifiedPositionState = null }) {
  if (qualifiedPositionState?.qualified) return qualifiedPositionState.thesisClarity === "clear" ? "high" : "medium";
...
const TASK2_GRAMMAR_CATEGORIES = new Set([
  ...
  "relative clause construction", "verb complement control"
]);
...
export function buildTask2LanguageProfile(writing, providerCandidates = []) {
  ...
  const lexicalSevere = lexicalIssues.filter((item) => /word form|meaning-sensitive word choice|collocation/.test(item.category));
  const grammarSevere = grammarIssues.filter((item) => /verb form|relative clause construction|verb complement control|clause completion|sentence fragments|run-on sentences/.test(item.category));
  const issuesBySentence = new Map(records.map((record) => [record.sentenceIndex, []]));
  for (const issue of activeIssues) {
    if (!issuesBySentence.has(issue.sentenceIndex)) issuesBySentence.set(issue.sentenceIndex, []);
    issuesBySentence.get(issue.sentenceIndex).push(issue);
  }
  const sentenceFragmentIssues = grammarIssues.filter((item) => /clause completion|sentence fragments/.test(item.category));
  const severeClauseFailures = grammarIssues.filter((item) =>
    item.classification === "clear-error" &&
    /verb form|relative clause construction|verb complement control|clause completion|sentence fragments|run-on sentences|pronoun\/reference control/.test(item.category)
  );
  const meaningObscuringIssues = activeIssues.filter((item) => item.affectsMeaning ||
    (item.classification === "clear-error" && /meaning-sensitive word choice|relative clause construction|verb form|clause completion|sentence fragments/.test(item.category))
  );
  const minorOnlyRecords = records.filter((record) => {
    const sentenceIssues = issuesBySentence.get(record.sentenceIndex) || [];
    return sentenceIssues.length > 0 && sentenceIssues.every((item) =>
      item.classification === "awkward-but-understandable" || item.severity === "minor"
    );
  });
  const reconstructionRecords = records.filter((record) => {
    const sentenceIssues = issuesBySentence.get(record.sentenceIndex) || [];
    const clearErrors = sentenceIssues.filter((item) => item.classification === "clear-error");
    return sentenceIssues.some((item) => meaningObscuringIssues.includes(item) || severeClauseFailures.includes(item)) || clearErrors.length >= 2;
  });
  const collocationIssues = lexicalIssues.filter((item) => /collocation/.test(item.category));
  const readerReconstructionRatio = records.length ? reconstructionRecords.length / records.length : 0;
  ...
  const lexicalBand = classifyLanguageBand({
    ...
    confidence,
    severeIssueCount: lexicalSevere.length,
    severeParagraphCount: new Set(lexicalSevere.map((item) => item.paragraphIndex)).size
  });
  const grammarBand = classifyLanguageBand({
    ...
    confidence,
    severeIssueCount: grammarSevere.length,
    severeParagraphCount: new Set(grammarSevere.map((item) => item.paragraphIndex)).size
  });
  ...
  return {
    ...
    languageControlDensity: {
      totalSentences: records.length,
      substantiallyErrorFreeSentences: errorFreeRecords.length,
      substantiallyErrorFreeRatio: roundEvidenceRatio(errorFreeRatio),
      minorOnlySentences: minorOnlyRecords.length,
      meaningObscuringIssueCount: meaningObscuringIssues.length,
      sentenceFragmentCount: sentenceFragmentIssues.length,
      severeClauseControlFailureCount: severeClauseFailures.length,
      recurringErrorFamilies: repeatedPatterns,
      lexicalMisuseFrequency: roundEvidenceRatio(records.length ? lexicalIssues.length / records.length : 0),
      collocationFailureFrequency: roundEvidenceRatio(records.length ? collocationIssues.length / records.length : 0),
      readerReconstructionSentenceCount: reconstructionRecords.length,
      readerReconstructionRatio: roundEvidenceRatio(readerReconstructionRatio),
      varietyVsAccuracy: {
        complexSentenceCount: complexRecords.length,
        controlledComplexSentenceCount: controlledComplexRecords.length,
        varietyRatio: roundEvidenceRatio(complexRatio),
        accuracyWithinComplexSentences: roundEvidenceRatio(complexControlRatio),
        judgement: complexRatio >= 0.35 && complexControlRatio < 0.5
          ? "Variety is attempted, but accuracy is not controlled consistently."
          : complexControlRatio >= 0.65
            ? "Complex variety is supported by generally controlled accuracy."
            : "The available evidence does not show secure variety-and-accuracy control."
      }
    },
...
function collectDeterministicLanguageIssues(source, records) {
  ...
  const rules = [
    ...
    lexicalRule(/\b(?:their|its|our|your)\s+specific\s+own\b/gi, "collocation", "clear-error", "The possessive, 'specific' and 'own' are stacked redundantly in this noun phrase."),
    lexicalRule(/\btoo\s+many\s+problems\b/gi, "precision", "clear-error", "This lexical-precision phrase is too general to identify the specific adverse effects claimed in an academic conclusion."),
    lexicalRule(/\b(?:planning|integrating|separating)\s+(?:city|society)\b/gi, "word form", "clear-error", "The noun phrase uses an action form where a controlled planning or integration noun is required."),
    lexicalRule(/\bthis\s+(?:integrate|separate|divide)\s+(?:is|are|will|would|can)\b/gi, "word form", "clear-error", "The sentence uses a base verb where a noun is required."),
    lexicalRule(/\b(?:significantly\s+advantages?|significantly\s+much\s+more|much\s+more\s+convenience)\b/gi, "collocation", "clear-error", "The modifier and noun combination is not controlled."),
    lexicalRule(/\b(?:industrials|chemist from (?:a )?chemical factor(?:y|ies))\b/gi, "meaning-sensitive word choice", "clear-error", "The word form names the wrong entity for an industrial site or chemical plant."),
    lexicalRule(/\b(?:have|has)\s+(?:an?\s+)?effect\s+with\b/gi, "collocation", "clear-error", "The effect collocation requires a controlled preposition and object."),
    lexicalRule(/\buseing\b/gi, "spelling", "clear-error", "The word is misspelled."),
    ...
    grammarRule(/\b(?:should|could|would|might|must|may)\s+(?:divide|separate|integrate)\s+into\b/gi, "verb form", "clear-error", "The modal construction does not express the intended passive relationship."),
    grammarRule(/\bwhere\s+(?:is|are|was|were)\s+[^.!?]{1,100}\s+(?:is|are|was|were)\s+(?:kept|placed|located|situated)\b/gi, "relative clause construction", "clear-error", "The relative clause combines two incompatible finite structures."),
    grammarRule(/\b(?:is|are|was|were)\s+moved\s+to\s+(?:placed|located|situated)\b/gi, "verb form", "clear-error", "Two passive verb frames have been combined in one clause."),
    grammarRule(/\b(?:can|could|would|should|may|might|must)\s+only\s+(?:in|at|on)\s+(?:a\s+|an\s+|the\s+)?\w+/gi, "verb form", "clear-error", "The modal clause is missing a base-form main verb."),
    grammarRule(/\b(?:is|are|was|were|will|would|can|could|may|might)\s+(?:not\s+|n't\s+)?be\s+(?:hurt|harm|damage)\s+(?:people|residents|workers|children)\b/gi, "verb form", "clear-error", "The passive and active verb frames are incompatible with the affected object."),
    grammarRule(/\ballow(?:s|ed|ing)?\s+(?:[a-z'-]+\s+){1,4}for\s+(?:walk|travel|go|use)\b/gi, "verb complement control", "clear-error", "The verb 'allow' requires an object followed by a to-infinitive in this meaning."),
  ...
  for (const record of records) {
    const sentence = String(record.exactSentence || "").trim();
    const dependentOpening = sentence.match(/^(While|Although|Because|If|When)\b[^.!?]*[.!?]?$/i);
    const hasIndependentClauseAfterComma = /,\s*(?:I|we|they|people|residents|cities|this|that|it|there|[A-Z][a-z]+)\s+[a-z]+\b/i.test(sentence) ||
      /,\s*(?:dividing|integrating|separating|implementing|mixing|applying)\b[^,.!?]{0,100}\b(?:is|are|was|were|has|have|causes?|creates?|reduces?|provides?|makes?|protects?|supports?)\b/i.test(sentence) ||
      /,\s*[^,.!?]{1,100}\b(?:am|is|are|was|were|has|have|can|could|will|would|should|may|might|must|[a-z]+ed|[a-z]+s)\b/i.test(sentence);
    const gerundClosureCandidate = /^(?:Consequently|Therefore|Thus),\s+(?:implementing|dividing|integrating|separating)\b[^.!?]*\b(?:for|to)\s+[a-z]+ing\b[^.!?]*[.!?]?$/i.test(sentence);
    const gerundClosureHasFiniteVerb = /^(?:Consequently|Therefore|Thus),\s+(?:implementing|dividing|integrating|separating)\b[^.!?]{0,120}\b(?:is|are|was|were|has|have|causes?|creates?|reduces?|provides?|makes?|protects?|supports?)\b/i.test(sentence);
    const gerundClosureFragment = gerundClosureCandidate && !gerundClosureHasFiniteVerb;
    if ((dependentOpening && !hasIndependentClauseAfterComma) || gerundClosureFragment) {
      issues.push(buildLanguageIssue({
        exactSentence: record.exactSentence,
        exactProblemSpan: dependentOpening?.[1] || sentence.replace(/[.!?]+$/, ""),
        criterion: "Grammatical Range & Accuracy",
        category: "sentence fragments",
        classification: "clear-error",
        severity: "major",
        explanation: dependentOpening
          ? "The dependent-clause opener is not followed by an independent main clause, so the sentence is incomplete."
          : "The sentence has no finite main verb and therefore cannot function as a complete proposition.",
        affectsMeaning: true,
        recurringPatternKey: "sentence-fragment",
        paragraphIndex: record.paragraphIndex,
        paragraphLocation: record.paragraphLocation,
        sentenceIndex: record.sentenceIndex,
...
function classifyLanguageBand({ widespread, issueCount, paragraphCount, rangeSupported, controlRatio, confidence, severeIssueCount = 0, severeParagraphCount = 0 }) {
  if (severeIssueCount >= 4 && severeParagraphCount >= 3 && controlRatio < 0.45) return "belowBand6";
...
function calibrateOpinionSupportQuality(routeAssessment, languageProfile) {
  if (!routeAssessment?.qualifiedPositionState?.qualified) return;
  for (const route of routeAssessment.bodyRoutes || []) {
    const example = String(route.exampleSupport || "").trim();
    const paragraphIssues = (languageProfile.validatedIssues || []).filter((item) => item.paragraphLocation === `Body Paragraph ${route.index}`);
    const exampleIssues = paragraphIssues.filter((item) => normalizeLanguageEvidence(example).includes(normalizeLanguageEvidence(item.exactProblemSpan)) ||
      normalizeLanguageEvidence(item.exactSentence) === normalizeLanguageEvidence(example));
    const languageRepairNeeded = exampleIssues.length > 0;
    const mechanismPresent = Boolean(String(route.explanationMechanism || "").trim()) ||
      /\b(?:because|since|so|therefore|as a result|which|leading|resulting|instead of|reduce|prevent|protect)\b/i.test(example);
    let status = "Not Demonstrated";
    if (example && !mechanismPresent) status = "Development Repair Needed";
    else if (example && languageRepairNeeded) status = route.routeRole === "exception-concession"
      ? "Development Repair Needed"
      : "Controlled Support - Language Repair Needed";
    else if (example) status = "Controlled Support";
    route.supportAssessment = {
      status,
      relevance: example ? "relevant" : "not demonstrated",
      causalCompleteness: mechanismPresent ? "traceable" : "partly obscured",
      languageAccuracy: languageRepairNeeded ? "repair needed" : "controlled",
      exactEvidence: example
    };
  }
}
...
function buildCanonicalFrameworkAssessment(routeAssessment, developmentRisk = {}, languageProfile = {}) {
  ...
  return {
    ...
    conclusionClosure: conclusionStatus,
...
function evaluateConclusionClosure(conclusion, overallRouteStatus) {
  ...
  return {
    status: "Strong",
    reason: "The conclusion is present and functionally closes the established position and body routes; language precision is assessed separately under LR, GRA and LFC-CPC."
  };
}
...
function normalizeCanonicalCriterionScores(input = {}, safety, capMetadata) {
  ...
  for (const criterion of TASK2_CRITERIA) {
    ...
    output[criterion] = typeof source === "object" && source
      ? {
          ...
          scoreSource: "canonical-language-profile-v12.3.0"
        }
      : { range: formatBandRange(parsed.low, parsed.high), diagnosis: deterministicCriterionDiagnosis(criterion, safety), evidence: "", scoreSource: "canonical-language-profile-v12.3.0" };
  }

  const route = safety.routeAssessment;
  const qualifiedThesisRepair = Boolean(route.qualifiedPositionState?.qualified && route.qualifiedPositionState.thesisClarity !== "clear");
  ...
  if (fullLengthComplete && !qualifiedThesisRepair && isPartialRouteStatus(route.status) && !route.missingRequirements.length && taskResponse.high <= 6) {
...
export function deriveDeterministicTask2CriterionRanges(safety = {}) {
  ...
  const lexicalRange = languageCriterionRange(profile.overallLexicalControl, developmentLimitsUpperBand, "lexical");
  const grammarRange = languageCriterionRange(profile.overallGrammarControl, developmentLimitsUpperBand, "grammar");
  const coherenceLanguageLimiter = Boolean(profile.sentenceCompletionErrors?.length);
  const qualifiedThesisRepair = Boolean(route.qualifiedPositionState?.qualified && route.qualifiedPositionState.thesisClarity !== "clear");
  const density = profile.languageControlDensity || {};
  const denseReconstructionBurden = profile.belowBand6 && (
  ...
  const baseTaskResponse = safety.underLength
    ? "6.0-6.5"
    : qualifiedThesisRepair ? (denseReconstructionBurden ? "5.0-5.5" : "5.5-6.0") : partial ? "6.0-6.5" : "6.5";
  const baseCoherence = qualifiedThesisRepair
    ? (denseReconstructionBurden ? "5.0-5.5" : "5.5")
    : partial || coherenceLanguageLimiter ? "6.0-6.5" : "6.5-7.0";
  return {
...
function languageCriterionRange(control, developmentLimitsUpperBand = false, criterion = "") {
  return {
    belowBand6: criterion === "grammar" ? "4.5-5.0" : "5.0",
    band6: "6.0",
...
function deterministicCriterionDiagnosis(criterion, safety) {
  ...
  if (criterion === "Task Response") {
    if (safety.routeAssessment?.qualifiedPositionState?.qualified && safety.routeAssessment.qualifiedPositionState.thesisClarity !== "clear") {
      return "The qualified judgement is relevant and is clarified by the conclusion, but the thesis initially combines the limited exception and main position without making their scope explicit.";
    }
  ...
  if (criterion === "Coherence & Cohesion") {
    if (safety.routeAssessment?.qualifiedPositionState?.qualified && safety.routeAssessment.qualifiedPositionState.thesisClarity !== "clear") {
      return "The exception paragraph and main-position paragraph are globally consistent, but the reader must wait until the conclusion to resolve the scope left unclear in the thesis.";
    }
...
export function assessTask2RevisionFidelity({ exactSentence = "", targetedRevision = "", revisionType = "" } = {}) {
  ...
  if (type === "Teacher-Guided Recommended Route") type = "Teacher-Guided Expansion";
...
export function validateTask2RevisionIntegrity({
  ...
  const originalContentTokens = new Set(revisionContentTokens(exact));
  const sharedContentTokenCount = revisionContentTokens(fidelity.targetedRevision)
    .filter((token) => originalContentTokens.has(token)).length;
  const languageRepairWithoutStance = !originalStance && !revisedStance && (sharedRoute || sharedContentTokenCount >= 3);
  const disclosedTeacherClarification = fidelity.revisionType === "Teacher-Guided Expansion" && !stanceChanged;
  const routePreserved = !stanceChanged && (
    originalPolarity === revisedPolarity ||
    languageRepairWithoutStance ||
    disclosedTeacherClarification
  );
