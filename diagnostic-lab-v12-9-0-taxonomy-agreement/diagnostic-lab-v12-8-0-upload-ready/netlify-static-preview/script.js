...
function printSectionUnits(section) {
  ...
  if (patterns) {
    ...
    // Two compact rows per fragment let a validated revision-bearing pattern group use residual
    // page space instead of forcing the entire four-row group onto a fresh sheet.
    return groupedPrintUnits(section, heading, [...patterns.children], "print-language-pattern-list", 2, note);
  }
...
// One compact Language Pattern Summary row. Demoted diagnostic issues retain their validated model
// repair, so every visible issue still shows one of the three student-facing revision types.
function renderPrintLanguagePatternRow(row = {}, copy = getReportCopy("en"), language = "en") {
  const hasRevision = Boolean(row.targetedRevision && row.revisionType);
  return `<div class="print-language-pattern-row">
    ...
    ${hasRevision ? `<p><strong>${escapePrintHtml(copy.revisionType)}:</strong> ${escapePrintHtml(row.revisionType)}<br>
      <strong>${escapePrintHtml(copy.targetedRevision)}:</strong> ${escapePrintHtml(row.targetedRevision)}</p>
      <p><strong>${escapePrintHtml(copy.whyStronger)}:</strong> ${escapePrintHtml(row.whyRevisionIsStronger)}</p>`
      : `${row.diagnosis ? `<p>${escapePrintHtml(row.diagnosis)}</p>` : ""}${row.action ? `<p><strong>${escapePrintHtml(copy.studentAction)}:</strong> ${escapePrintHtml(row.action)}</p>` : ""}`}
  </div>`;
}
