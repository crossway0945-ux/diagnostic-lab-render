...
if (!args.output) throw new Error("Usage: node qa/render-poon-stopship-pdf.mjs --output=<dir> [--chrome=<path>]");
...
const browser = await chromium.launch({ executablePath: args.chrome || process.env.CHROME_PATH || undefined, headless: true, args: ["--font-render-hinting=none"] });

try {
  const analysis = JSON.parse(await readFile(path.join(outputDir, "Poon-render-input.json"), "utf8"));
  ...
  const prepared = await appPage.evaluate(async (input) => {
    ...
    return {
      ...
      preparedPageCount: doc.querySelectorAll(".pdf-sheet").length,
  ...
  const metrics = await page.evaluate(() => {
    ...
    const pageRows = sheets.map((sheet, index) => {
      ...
      const protectedBlocks = [...sheet.querySelectorAll(".print-info,.print-card,.print-issue,.print-feedback-primary-group,.print-feedback-revision-group,.print-exact-evidence-group,.print-target-revision-group,.print-plan-item,.print-disclaimer")];
      const clippedBlocks = protectedBlocks.filter((block) => {
        const rect = block.getBoundingClientRect();
        return rect.left < sheetRect.left - 1 || rect.right > sheetRect.right + 1 || rect.top < sheetRect.top - 1 || rect.bottom > sheetRect.bottom + 1;
      }).map((block) => ({ className: String(block.className || ""), text: String(block.textContent || "").replace(/\s+/g, " ").trim().slice(0, 120) }));
      ...
      const horizontalOverflow = visibleElements.reduce((max, element) => {
        const rect = element.getBoundingClientRect(); return Math.max(max, rect.right - sheetRect.right, sheetRect.left - rect.left, 0);
      }, 0);
      const pageNumberCollision = Boolean(numberRect && protectedBlocks.some((block) => {
        const rect = block.getBoundingClientRect();
        return !(rect.right < numberRect.left || rect.left > numberRect.right || rect.bottom < numberRect.top || rect.top > numberRect.bottom);
      }));
      const text = String(content?.innerText || "").replace(/\s+/g, " ").trim();
      return {
        page: index + 1,
        numbering: String(pageNumber?.textContent || "").trim(),
        headings: [...sheet.querySelectorAll("h1,h2,h3")].map((heading) => String(heading.textContent || "").replace(/\s+/g, " ").trim()).filter(Boolean),
    ...
    return {
      ...
      pageRows,
      internalIdLeak: /(?:priority-[a-z0-9-]+|submissionGroupId|reportVersionId|parentReportId|normalizedResponseFingerprint|legacy-[a-f0-9])/i.test(bodyText),
      forbiddenUnicode: /[\uFDD0-\uFDEF\uFFFE\uFFFF\uD800-\uDFFF\u200B\u00AD]/u.test(bodyText),
      paginationComplete: document.querySelector("#print-report")?.dataset.paginationComplete === "true"
    };
  });

  await page.pdf({
    path: path.join(outputDir, "Poon-final-report.pdf"),
    format: "A4", printBackground: true, preferCSSPageSize: true, displayHeaderFooter: false,
    margin: { top: "0", right: "0", bottom: "0", left: "0" }
  });
  await page.screenshot({ path: path.join(outputDir, "Poon-final-report-browser.png"), fullPage: true });
  await Promise.all([
    writeFile(path.join(outputDir, "Poon-final-report.html"), prepared.html, "utf8"),
    writeFile(path.join(outputDir, "Poon-final-report-browser-metrics.json"), `${JSON.stringify(metrics, null, 2)}\n`, "utf8")
  ]);
  const summary = {
  ...
  await writeFile(path.join(outputDir, "Poon-pdf-render-summary.json"), `${JSON.stringify(summary, null, 2)}\n`, "utf8");
  if (summary.pageCount < 10 || summary.pageCount > 16 || !summary.paginationComplete || summary.internalIdLeak || summary.forbiddenUnicode || summary.blankPages || summary.worstVerticalOverflowPx || summary.worstHorizontalOverflowPx || summary.clippedBlocks || summary.pageNumberCollisions) {
    throw new Error(`Poon PDF render gate failed: ${JSON.stringify(summary)}`);
  }
