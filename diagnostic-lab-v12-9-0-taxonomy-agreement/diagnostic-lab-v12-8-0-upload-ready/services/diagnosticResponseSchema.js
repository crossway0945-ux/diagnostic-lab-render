...
function commonProperties(taskType) {
  return {
    ...
    feedbackCards: arraySchema(feedbackCardSchema(taskType)),
...
function task2Properties() {
  return {
    ...
    overallBandCap: stringSchema(),
    strictModeApplied: booleanSchema(),
    wordCount: integerSchema(),
    minimumRequiredWords: integerSchema(),
    underLength: booleanSchema(),
    underLengthBy: integerSchema(),
    completionStatus: stringSchema(),
    unfinishedEndingDetected: booleanSchema(),
    completionEvidence: arraySchema(stringSchema()),
    detectedPosition: stringSchema(),
    positionConfidence: stringSchema(),
    bodyRouteSummary: stringSchema(),
    routeConflict: booleanSchema(),
    recommendedRoute: stringSchema(),
    recommendedRouteRationale: stringSchema(),
    routeIntegrity: stringSchema(),
    completionIntegrity: stringSchema(),
    languageControlIntegrity: stringSchema(),
    compoundSeverity: stringSchema(),
    criticalInteractionSummary: stringSchema(),
    meaningChangingErrors: arraySchema(meaningErrorSchema()),
    meaningReversingErrors: arraySchema(meaningErrorSchema()),
    languageAudit: arraySchema(languageAuditItemSchema()),
    revisedThesisRevisionType: stringSchema()
  };
}
...
function feedbackCardSchema(taskType) {
  const properties = {
    issueType: stringSchema(),
    severity: stringSchema(),
    criteria: arraySchema(stringSchema()),
    ...
    ...(taskType === "Task 2" ? { revisionType: stringSchema() } : {})
  };
  const required = [
    "issueType",
    "severity",
...
function meaningErrorSchema() {
  return objectSchema({
    exactEvidence: stringSchema(),
    category: stringSchema(),
    explanation: stringSchema()
  }, ["exactEvidence", "category", "explanation"]);
}

function languageAuditItemSchema() {
  return objectSchema({
    exactSentence: stringSchema(),
    exactProblemSpan: stringSchema(),
    criterion: stringSchema(),
    category: stringSchema(),
    classification: stringSchema(),
    severity: stringSchema(),
    explanation: stringSchema(),
    affectsMeaning: booleanSchema(),
    recurringPatternKey: stringSchema()
  }, [
    "exactSentence",
    "exactProblemSpan",
    "criterion",
