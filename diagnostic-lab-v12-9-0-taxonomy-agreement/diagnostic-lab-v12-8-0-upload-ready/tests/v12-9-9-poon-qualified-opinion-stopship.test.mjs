...
assert.match(poon.kruPomScores["Body Paragraph Route Alignment"].status, /Mixed|Mostly Controlled/);
...
assert.equal(poon.estimatedBandRange, "5.0-5.5");
...
assert.match(coverage["Body Paragraph 1"].status, /^Exception\/Concession Route (?:Aligned|Repair Needed) - Development Repair Needed/);
