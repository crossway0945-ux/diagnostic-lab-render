...
async function testThirtyTask2Fixtures() {
  ...
  for (const definition of TASK2_TYPES) {
    for (const scenario of TASK2_SCENARIOS) {
      ...
      let analysis;
      try {
        analysis = await analyzeWriting(payload);
      } catch (error) {
        error.message = `${definition.name}/${scenario}: ${error.message}`;
        throw error;
      }
...
async function testCanonicalRendererAndPdfSerializationPath() {
  ...
  assert.match(script, /projectStudentReportViewModel\(analysis\.studentReportViewModel\)/);
  assert.match(script, /assertStudentReportViewModel/);
  ...
  assert.match(index, /script\.js\?v=diagnostic-v12-7-0-root-cause-correction/);
...
function assertCanonicalReport(analysis, payload, label) {
  ...
  assert.equal(canonical.version, "12.3.1", `${label}: canonical version`);
