...
export function projectCanonicalTask2Framework(canonical = {}, reportLanguage = "") {
  ...
  const thesisDiagnosis = thai
    ? (isZoningOpinion && positionClear
        ? "Thesis วางเหตุผลสองข้อชัด ได้แก่ reduced accessibility และ traffic congestion"
        : positionClear
          ? `Introduction ระบุจุดยืน ${route.position} ชัด และแยกการวาง Body route ออกจากความชัดเจนของ position`
          : "Thesis ยังวางเส้นทางที่โจทย์ต้องการไม่ชัดพอ")
    : (isZoningOpinion && positionClear
  ...
  const bodyRouteDiagnosis = thai
    ? (isZoningOpinion && (route.bodyRoutes || []).length >= 2
        ? "Body Paragraph 1 พัฒนาเหตุผลเรื่อง accessibility จาก Thesis และ Body Paragraph 2 พัฒนาเหตุผลเรื่อง traffic congestion"
  ...
  return {
    ...
    "Explanation Depth": {
      ...
      diagnosis: partialConcession
        ...
        : (thai ? "เหตุผลทั้งสองเกี่ยวข้องกับโจทย์ แต่ causal mechanism และผลที่กว้างขึ้นยังต้องแม่นกว่านี้" : "Both reasons are relevant, but the causal mechanisms and broader consequences need greater precision.")
    },
    "SAR Example Quality": {
      status: framework.sarExampleQuality?.status || routeStatus,
      diagnosis: thai ? "ตัวอย่างทั้งสองเกี่ยวข้อง แต่ยังไม่โน้มน้าวและพัฒนาในวงกว้างพอ" : "Both examples are relevant but not sufficiently convincing or broadly developed."
    },
    "Link Back Control": {
    ...
    "Conclusion Closure": {
      ...
      diagnosis: conclusionStatus === "Strong"
        ? (thai ? "Conclusion restate judgement ของผู้เขียนได้สม่ำเสมอ" : "The conclusion restates the writer's judgement consistently.")
        : (thai ? "Conclusion ขาด ไม่สมบูรณ์ หรือยังปิด route ที่โจทย์ต้องการไม่ได้" : "The conclusion is missing, unfinished, or does not close the required route.")
    },
    "LFC CPC Control": {
