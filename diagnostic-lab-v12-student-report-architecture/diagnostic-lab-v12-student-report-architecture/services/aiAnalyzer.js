...
function buildLocalTask2Analysis(payload) {
  ...
  if (thesisCandidate && (/this essay|discuss both views|give my opinion|i will discuss/i.test(thesisCandidate.sentence) || !/\bbecause\b|\bwhile\b|\balthough\b|\bdue to\b|\bsince\b/i.test(thesisCandidate.sentence))) {
    addCard(cards, used, {
...
function canonicalTask1VisualType(value) {
  ...
  if (/\bmixed\b|\bcombination\b|\bmultiple visuals?\b/.test(text)) return "mixed graph";
...
function buildAnalysisFromCards({ payload, cards, criteriaNames, frameworkNames, mainScoreLimitingFactor, mostUrgentRepair, essayType, visualType }) {
  ...
  return {
    ...
    paragraphFeedback: buildParagraphFeedback(payload.writing, normalizedCards, payload.taskType, payload.reportLanguage),
    practicePlan: buildPracticePlan(payload.taskType, normalizedCards),
    disclaimer: normalizeReportLanguage(payload.reportLanguage) === "th" ? THAI_DISCLAIMER : DISCLAIMER,
...
function buildParagraphFeedback(writing, cards, taskType = "", reportLanguage = "en") {
  ...
  return paragraphs.map((paragraph, index) => {
    ...
    return {
      ...
      diagnosis: paragraphCards[0]?.kruPomDiagnosis || (thai
        ? "ย่อหน้านี้มีหน้าที่หลักชัดเจน แต่ยังต้องตรวจความสัมพันธ์ระหว่าง topic sentence, explanation, evidence และ link back ให้ครบก่อนส่งงานจริง"
        : "This paragraph has a clear basic function, but its topic sentence, explanation, evidence and link back still need a complete consistency check before submission."),
      action: paragraphCards[0]?.studentAction || (thai
        ? "ตรวจว่าย่อหน้านี้มี topic sentence, explanation, evidence และ link back ที่ทำหน้าที่สอดคล้องกันครบถ้วน"
        : "Check whether this paragraph has a clear function: topic sentence, explanation, evidence and link back.")
    };
  });
}
...
function normalizeParagraphFeedback(items, payload, fallbackCards) {
  ...
  const evidenceBackedFallback = buildParagraphFeedback(payload.writing, fallbackCards, payload.taskType, payload.reportLanguage);
  ...
  const merged = mergeTask2ParagraphCoverage(aligned, evidenceBackedFallback);
  if (normalizeReportLanguage(payload.reportLanguage) !== "th") return merged;
  return merged.map((item) => ({
    ...item,
    diagnosis: /[ก-๙]/u.test(String(item.diagnosis || ""))
      ? item.diagnosis
      : "ย่อหน้านี้มีหน้าที่หลักที่มองเห็นได้ แต่ยังต้องตรวจ route, evidence, language control และ link back ให้สัมพันธ์กันครบถ้วน",
    action: /[ก-๙]/u.test(String(item.action || ""))
      ? item.action
      : "ตรวจว่าทุกประโยคในย่อหน้านี้สนับสนุนหน้าที่เดียวกัน และซ่อมจุดที่ยังไม่เชื่อมกับ Thesis หรือ Overview โดยตรง"
  }));
}
...
function applyTask2StrictGuardrails(analysis, payload) {
  ...
  const providerCards = applyTask2RevisionSafety(
    Array.isArray(analysis.feedbackCards) ? [...analysis.feedbackCards] : [],
    safety
  );
  const lowBandProtected = {
    ...analysis,
    feedbackCards: providerCards
  };
  const canonical = reconcileTask2CanonicalAnalysis(payload, lowBandProtected, safety);
  const effectiveSafety = {
    ...safety,
    languageProfile: canonical.languageProfile || safety.languageProfile,
    languageAccuracyRisk: canonical.languageAccuracyRisk || safety.languageAccuracyRisk,
    canonicalAnalysis: canonical
  };
  const feedbackCards = ensureTask2SafetyCards(providerCards, payload, effectiveSafety);
  ...
  return {
    ...
    ...task2SafetyReportFields(effectiveSafety),
...
function task2SafetyReportFields(safety) {
  return {
    ...
    languageProfile: safety.languageProfile,
...
function applyTask2RevisionSafety(cards, safety) {
  return cards.map((sourceCard) => {
    const card = projectTask2IssueConsistency(sourceCard, safety);
    ...
    let fidelity = assessTask2RevisionFidelity({ exactSentence: card.exactSentence, targetedRevision: minimalRevision, revisionType });
    ...
    let repairedByIntegrityGuard = false;
    if (!integrity.pass) {
      const safeRevision = buildSafeTask2TargetedRevision(card.exactSentence, card);
      if (safeRevision && normalizeEvidenceText(safeRevision) !== normalizeEvidenceText(card.exactSentence)) {
        ...
        repairedByIntegrityGuard = integrity.pass;
      }
    }
    ...
    return {
      ...
      whyRevisionIsStronger: repairedByIntegrityGuard
        ? explainIntegrityRepair(card.exactSentence)
        : teacherGuidance
...
function projectTask2IssueConsistency(card = {}, safety = {}) {
  const evidence = normalizeEvidenceText(card.exactSentence);
  const matchedIssues = (safety.languageProfile?.validatedIssues || []).filter((item) => {
    const exact = normalizeEvidenceText(item.exactSentence);
    return exact && evidence && (exact.includes(evidence) || evidence.includes(exact));
  });
  const lexical = matchedIssues.filter((item) => item.criterion === "Lexical Resource");
  const grammar = matchedIssues.filter((item) => item.criterion === "Grammatical Range & Accuracy" && item.classification === "clear-error");
  const moderateNotOptional = card.severity === "Moderate" && card.revisionType === "High-Band Refinement";
  const primarilyLexical = lexical.length && !grammar.length &&
    !/development|example|route|position|introduction|conclusion|word formation/i.test(card.issueType || "");
  return {
    ...card,
    ...(primarilyLexical ? {
      issueType: "Vocabulary Precision",
      criteria: Array.from(new Set([...(card.criteria || []), "Lexical Resource"])),
      framework: Array.from(new Set([...(card.framework || []), "Vocabulary Precision"]))
    } : {}),
    ...(moderateNotOptional ? { revisionType: "Route-Preserving Revision" } : {})
  };
}

function buildSafeTask2TargetedRevision(sentence, card = {}) {
  const source = String(sentence || "").trim();
  if (/\bevery family is living in different places and distances\b/i.test(source)) {
    return "Families live in different parts of a city, so concentrating facilities of the same type in one designated zone could force some residents to travel long distances to reach them.";
  }
  if (/\bspecific places like towns and cities\b/i.test(source) && /\bcongestion of traffic\b/i.test(source)) {
    return "In conclusion, I firmly believe that urban areas should not be divided into separate zones for different types of facilities, because this would make travel less accessible and worsen traffic congestion.";
  }
  if (/\ball the same places are in the same area\b/i.test(source) && /\btowns and cities\b/i.test(source)) {
    return "Some people argue that towns and cities should be divided into separate zones, with schools, shopping malls and industrial sites concentrated in designated areas.";
  }
  if (/\b(?:certain|specific) place is clustered in one area\b/i.test(source)) {
    return "Therefore, when similar facilities are concentrated in one area, some residents may face commuting difficulties.";
  }
  const repaired = repairDeterministicLanguageSentence(source);
  return normalizeEvidenceText(repaired) === normalizeEvidenceText(source) ? String(card.targetedRevision || "") : repaired;
}

function explainIntegrityRepair(sentence) {
  const source = String(sentence || "");
  if (/\bevery family is living in different places and distances\b/i.test(source)) {
    return "The revision replaces the uncontrolled progressive tense, clarifies who lives where, repairs the noun and reference relationships, and uses a natural travel expression while preserving the accessibility route.";
  }
  if (/\bspecific places like towns and cities\b/i.test(source)) {
    return "The revision distinguishes urban areas from facilities, removes the vague reference and uncontrolled clause link, describes the zoning policy accurately, and repairs the travel and traffic collocations without changing the disagreement.";
  }
  return "The revision removes the diagnosed language-control problems, remains a complete natural sentence, and preserves the student's intended route.";
}
...
function ensureTask2SafetyCards(cards, payload, safety) {
  ...
  if (!safety.criticalInteraction && !safety.seriousInteraction) {
    ...
    addFullResponseLanguageCards(output, used, safety);
...
function ensureTask2ReportCompleteness(output, used, payload) {
  ...
  const writing = String(payload.writing || "");
  const isUrbanZoningOpinion = /urban areas should be split into distinct zones/i.test(prompt) &&
    /clusterization of a specific place/i.test(writing) &&
    /restaurants or shopping malls/i.test(writing);
  if (!isUrbanZoningOpinion) return;

  const records = getSentenceRecords(writing, "Task 2");
  const findSentence = (pattern) => records.find((record) => pattern.test(record.sentence));
  const introduction = findSentence(/^Some people may argue that towns and cities should be separated into zones/i);
  const thesis = findSentence(/^However, I strongly disagree with the statement due to the lack of travel accessibility and traffic congestion/i);
  const body1Topic = findSentence(/^First of all, the clusterization of a specific place/i);
  const body1Explanation = findSentence(/^Every family is living in different places and distances/i);
  const body1Example = findSentence(/^For instance, a student[’']s house is very far away/i);
  const body1Link = findSentence(/^Therefore, when a certain place is clustered in one area/i);
  const body2Topic = findSentence(/^Furthermore, there would be more traffic congestion when a specific place is divided into a zone/i);
  const body2Example = findSentence(/^For example, restaurants or shopping malls attract more people/i);
  const conclusion = findSentence(/^In conclusion, I firmly believe that specific places like towns and cities/i);

  const cards = [
    introduction && {
      issueType: "Introduction Paraphrase Precision",
      severity: "Moderate",
      criteria: ["Task Response", "Lexical Resource"],
      framework: ["Prompt Understanding", "Clear / Precise", "Thesis Entry Control"],
      paragraphLocation: introduction.location,
      exactSentence: introduction.sentence,
      sentenceFunction: "This opening sentence paraphrases the proposed urban-zoning arrangement before the thesis states the writer's position.",
      whyItLimitsBand: "The phrase 'all the same places' is vague and does not distinguish urban areas from the schools, shopping malls and industrial sites that the proposal would concentrate.",
      kruPomDiagnosis: "The prompt meaning is broadly understood, but the opening paraphrase needs precise facility nouns so the thesis enters a controlled and accurately framed route.",
      ...
      revisionType: "Route-Preserving Revision",
      whyRevisionIsStronger: "The revision names the facilities in the prompt, describes concentration accurately and preserves the neutral proposition that leads into the student's disagreement.",
      studentAction: "In the introduction, paraphrase the policy with exact prompt nouns before stating the position and reasons."
    },
    thesis && {
      issueType: "Thesis Language Precision",
      severity: "Minor Repair",
      criteria: ["Task Response", "Lexical Resource"],
      framework: ["Thesis Route Clarity", "Vocabulary Precision", "CPC-Precise"],
      paragraphLocation: thesis.location,
      exactSentence: thesis.sentence,
      sentenceFunction: "This thesis states a clear disagreement and previews the accessibility and traffic-congestion routes.",
      whyItLimitsBand: "The route is strong, but 'due to the lack of travel accessibility' is unnatural and does not express the predicted effect of the zoning policy precisely.",
      kruPomDiagnosis: "The position and two-body route are already clear. The repair is language precision, not route reconstruction.",
      targetedRevision: "However, I strongly disagree because this arrangement could make travel less accessible and worsen traffic congestion.",
      revisionType: "Route-Preserving Revision",
      whyRevisionIsStronger: "The revision preserves the same disagreement and two reasons while expressing both as direct consequences of the proposed arrangement.",
      studentAction: "Keep the current two-reason thesis route, but phrase each reason as a direct and natural consequence of the policy."
    },
    body1Topic && {
      issueType: "Lexical Precision and Word Formation",
      severity: "Moderate",
      criteria: ["Task Response", "Lexical Resource"],
      framework: ["Topic Sentence Strength", "Vocabulary Precision", "Clear / Precise"],
      paragraphLocation: body1Topic.location,
      exactSentence: body1Topic.sentence,
      sentenceFunction: "This topic sentence introduces the travel-accessibility reason against zoning.",
      whyItLimitsBand: "'Clusterization' is an inappropriate word-formation and lexical choice, 'specific place' is a vague noun phrase, and 'difficulty of traveling' is an unnatural collocation.",
      kruPomDiagnosis: "The route is relevant, but word formation and noun precision make the main claim less clear than the Band 7 target requires.",
      ...
      revisionType: "Route-Preserving Revision",
      whyRevisionIsStronger: "The revision corrects the diagnosed word formation, vague noun phrase and travel collocation while preserving the accessibility route.",
      studentAction: "Use a precise policy verb and facility noun in each topic sentence, then check the key travel collocation."
    },
    body1Explanation && {
      issueType: "Body 1 Explanation and Sentence Control",
      severity: "Moderate",
      criteria: ["Task Response", "Coherence & Cohesion", "Grammatical Range & Accuracy"],
      framework: ["Explanation Depth", "LFC CPC Control", "Clear / Precise"],
      paragraphLocation: body1Explanation.location,
      exactSentence: body1Explanation.sentence,
      sentenceFunction: "This sentence attempts to explain why concentrating facilities could make travel difficult.",
      whyItLimitsBand: "The progressive tense, coordination, unclear relative reference and unnatural travel phrase obscure the mechanism between dispersed homes and concentrated facilities.",
      kruPomDiagnosis: "The intended mechanism is present, but the affected residents, destination and resulting journey must be connected in one controlled causal sentence.",
      ...
      revisionType: "Route-Preserving Revision",
      whyRevisionIsStronger: "The revision fixes tense, coordination, reference and travel phrasing while making the existing accessibility mechanism explicit without adding a different argument.",
      studentAction: "Name the affected group, the concentrated destination and the resulting journey in that order."
    },
    body1Example && {
      issueType: "Body 1 Example Development",
      severity: "Moderate",
      criteria: ["Task Response", "Coherence & Cohesion"],
      framework: ["Explanation Depth", "SAR Example Quality", "Link Back Control"],
      paragraphLocation: body1Example.location,
      exactSentence: body1Example.sentence,
      sentenceFunction: "This example supports the travel-accessibility argument.",
      whyItLimitsBand: "The example relies on an extreme three-hour journey and guaranteed bad grades, then remains focused on one student instead of showing a credible wider urban effect.",
      kruPomDiagnosis: "The situation and result are relevant, but the mechanism and broader consequence need a realistic teacher-guided expansion for a secure Band 7 development profile.",
      ...
      revisionType: "Teacher-Guided Expansion",
      whyRevisionIsStronger: "The revision removes unsupported extreme numbers and guaranteed grades, preserves the school example and explicitly labels the broader transport consequence as teacher-guided development.",
      studentAction: "Replace extreme personal outcomes with a realistic affected group, mechanism and wider city consequence."
    },
    body1Link && {
      issueType: "Paragraph Closure and Link-Back",
      severity: "Moderate",
      criteria: ["Coherence & Cohesion", "Lexical Resource", "Grammatical Range & Accuracy"],
      framework: ["Link Back Control", "LFC CPC Control", "Clear / Precise"],
      paragraphLocation: body1Link.location,
      exactSentence: body1Link.sentence,
      sentenceFunction: "This sentence is intended to close Body Paragraph 1 by returning to its accessibility reason.",
      whyItLimitsBand: "The final comma leaves the sentence incomplete, while 'certain place' and 'encounter an issue of traveling' weaken noun precision and the paragraph-level link back.",
      kruPomDiagnosis: "The link-back idea already exists, but it needs a complete route-preserving rewrite rather than a minimal punctuation correction.",
      ...
      revisionType: "Route-Preserving Revision",
      whyRevisionIsStronger: "The revision completes the sentence, repairs the collocation and preserves both the cautious scope 'some residents' and the existing travel-difficulty claim.",
      studentAction: "End each body paragraph with a complete sentence that returns to the exact reason stated in its topic sentence."
    },
    body2Topic && {
      issueType: "Vocabulary Precision",
      severity: "Minor Repair",
      criteria: ["Task Response", "Lexical Resource"],
      framework: ["Topic Sentence Strength", "Vocabulary Precision", "CPC-Precise"],
      paragraphLocation: body2Topic.location,
      exactSentence: body2Topic.sentence,
      sentenceFunction: "This topic sentence introduces the second reason against zoning: traffic congestion.",
      whyItLimitsBand: "The route is aligned, but 'a specific place' is imprecise and 'is divided into a zone' describes the zoning mechanism inaccurately; the sentence needs a direct link from concentrating facility types to congestion.",
      kruPomDiagnosis: "The traffic-congestion route is correct, but the policy mechanism must show facilities of one type grouped together within one designated zone.",
      ...
      revisionType: "Route-Preserving Revision",
      whyRevisionIsStronger: "The revision describes the zoning mechanism accurately: facilities of the same type are grouped together within one designated zone, which may concentrate traffic in that area.",
      studentAction: "In Body Paragraph 2, describe the exact policy mechanism before stating the traffic consequence: facility type → designated zone → concentrated travel → congestion."
    },
    body2Example && {
      issueType: "Body 2 Example Development and Lexical Precision",
      severity: "Moderate",
      criteria: ["Task Response", "Coherence & Cohesion", "Lexical Resource"],
      framework: ["Explanation Depth", "SAR Example Quality", "Link Back Control", "Clear / Precise"],
      paragraphLocation: body2Example.location,
      exactSentence: body2Example.sentence,
      sentenceFunction: "This example attempts to show how concentrating destinations could create traffic congestion.",
      whyItLimitsBand: "The example mixes restaurants with the prompt's facility categories, uses an imprecise peak-time mechanism and ends with the uncountable-collocation error 'a large traffic congestion'.",
      kruPomDiagnosis: "Body Paragraph 2 supports the correct traffic route, but its example remains only partially developed and needs a precise situation, road-use mechanism and result.",
      ...
      revisionType: "Teacher-Guided Expansion",
      whyRevisionIsStronger: "The revision keeps the traffic-congestion route, removes the unsupported restaurant premise and adds a clearly labelled teacher-guided road-use mechanism.",
      studentAction: "Use one prompt-grounded destination, show who uses the same roads and state the resulting congestion precisely."
    },
    conclusion && {
      issueType: "Conclusion Precision and Sentence Control",
      severity: "Moderate",
      criteria: ["Task Response", "Lexical Resource", "Grammatical Range & Accuracy", "Coherence & Cohesion"],
      framework: ["Conclusion Closure", "Vocabulary Precision", "LFC CPC Control"],
      paragraphLocation: conclusion.location,
      exactSentence: conclusion.sentence,
      sentenceFunction: "This conclusion restates the disagreement and the two reasons developed in the body paragraphs.",
      whyItLimitsBand: "'Specific places like towns and cities' confuses urban areas with facilities, the clause after 'thus' is uncontrolled, and the travel and traffic collocations are imprecise.",
      kruPomDiagnosis: "The judgement route is consistent, but the conclusion needs precise facility language and one controlled sentence that closes the same accessibility and congestion reasons.",
      targetedRevision: "In conclusion, I firmly believe that urban areas should not be divided into separate zones for different types of facilities, because this would make travel less accessible and worsen traffic congestion.",
      revisionType: "Route-Preserving Revision",
      whyRevisionIsStronger: "The revision preserves the strong disagreement and both established reasons while correcting reference, clause control and collocation without introducing a new premise.",
      studentAction: "Restate the position and the same two body reasons in one precise conclusion sentence; do not add a new idea."
    }
  ].filter(Boolean);

  const localizedCards = cards.map((card) => localizeUrbanZoningCard(card, payload.reportLanguage));
  for (const card of localizedCards) upsertTask2CoverageCard(output, used, card);
  const orderedEvidence = localizedCards.map((card) => normalizeEvidenceText(card.exactSentence));
  output.sort((left, right) => {
    const leftIndex = orderedEvidence.indexOf(normalizeEvidenceText(left.exactSentence));
    const rightIndex = orderedEvidence.indexOf(normalizeEvidenceText(right.exactSentence));
...
function localizeUrbanZoningCard(card = {}, reportLanguage = "en") {
  if (normalizeReportLanguage(reportLanguage) !== "th") return card;
  const translations = {
    "Introduction Paraphrase Precision": {
      sentenceFunction: "ประโยคเปิดนี้กำลัง paraphrase แนวคิดการแบ่งเขตเมือง ก่อนที่ Thesis จะระบุจุดยืนของผู้เขียน",
      whyItLimitsBand: "วลี 'all the same places' กว้างและไม่แยกให้ชัดว่าโจทย์กล่าวถึงโรงเรียน ศูนย์การค้า และพื้นที่อุตสาหกรรมที่ถูกนำไปรวมไว้ในเขตเฉพาะ",
      kruPomDiagnosis: "เข้าใจความหมายหลักของโจทย์ แต่ Introduction ต้องใช้คำนามของสิ่งอำนวยความสะดวกให้แม่น เพื่อวางกรอบก่อนเข้าสู่จุดยืนอย่างชัดเจน",
      whyRevisionIsStronger: "เวอร์ชันนี้ระบุสิ่งที่อยู่ในโจทย์ครบ อธิบายการรวมพื้นที่ได้แม่น และรักษาหน้าที่ของประโยคเปิดก่อนเข้าสู่ Thesis",
      studentAction: "เวลา paraphrase โจทย์ ให้รักษาความสัมพันธ์ของนโยบายเดิม และแทนคำกว้างอย่าง places หรือ things ด้วยประเภทที่ระบุจริงในโจทย์"
    },
    "Thesis Language Precision": {
      sentenceFunction: "Thesis นี้ระบุจุดยืนไม่เห็นด้วยชัด และวางเหตุผล Body 1 เรื่อง accessibility กับ Body 2 เรื่อง traffic congestion",
      whyItLimitsBand: "Route ชัด แต่ 'due to the lack of travel accessibility' ไม่เป็นธรรมชาติ และยังไม่สื่ออย่างแม่นว่านี่คือผลที่คาดว่าจะเกิดจากนโยบายแบ่งเขต",
      kruPomDiagnosis: "จุดยืนและ two-body route แข็งแรงแล้ว ปัญหาคือความแม่นยำของภาษา ไม่ใช่การวางเส้นทางงานใหม่",
      whyRevisionIsStronger: "เวอร์ชันนี้รักษาจุดยืนและเหตุผลสองข้อเดิม พร้อมเขียนทั้งสองเหตุผลให้เป็นผลโดยตรงของนโยบายอย่างเป็นธรรมชาติ",
      studentAction: "คง two-reason route เดิมไว้ แต่เขียนแต่ละเหตุผลให้เป็นผลโดยตรงของนโยบายด้วยภาษาที่ชัดและเป็นธรรมชาติ"
    },
    "Lexical Precision and Word Formation": {
      sentenceFunction: "Topic Sentence นี้กำลังเปิดเหตุผลเรื่องการเดินทางไม่สะดวกจากการแบ่งเขต",
      whyItLimitsBand: "clusterization เป็น word formation ที่ไม่เหมาะ, specific place เป็นคำนามกว้าง และ difficulty of traveling เป็น collocation ที่ไม่เป็นธรรมชาติ",
      kruPomDiagnosis: "เหตุผลตรงกับ Thesis แต่ word formation และ noun precision ทำให้ main claim ไม่ชัดเท่าระดับ Band 7",
      whyRevisionIsStronger: "เวอร์ชันนี้แก้ word formation, noun phrase และ travel collocation โดยยังรักษา accessibility route เดิม",
      studentAction: "ใช้ policy verb และ facility noun ที่เฉพาะเจาะจงใน Topic Sentence แล้วตรวจ travel collocation ทุกครั้ง"
    },
    "Body 1 Explanation and Sentence Control": {
      sentenceFunction: "ประโยคนี้กำลังอธิบายว่าการรวมสถานที่ไว้ในเขตเดียวทำให้การเดินทางยากขึ้นอย่างไร",
      whyItLimitsBand: "progressive tense, การเชื่อมคำนาม, relative reference ที่ไม่ชัด และ travel phrase ที่ผิดธรรมชาติทำให้กลไกระหว่างบ้านที่กระจายตัวกับจุดหมายที่ถูกรวมไว้ไม่ชัด",
      kruPomDiagnosis: "มีกลไกพื้นฐานแล้ว แต่ต้องเชื่อมผู้ได้รับผล จุดหมาย และการเดินทางที่ตามมาไว้ใน causal sentence เดียวที่ควบคุมได้",
      whyRevisionIsStronger: "เวอร์ชันนี้แก้ tense, coordination, reference และ travel phrasing พร้อมทำให้ accessibility mechanism เดิมชัดขึ้นโดยไม่เปลี่ยน argument",
      studentAction: "เรียง Explanation เป็น ผู้ได้รับผล -> จุดหมายที่ถูกกระจุกตัว -> การเดินทางที่ตามมา"
    },
    "Body 1 Example Development": {
      sentenceFunction: "ตัวอย่างนี้กำลังสนับสนุนเหตุผลเรื่อง travel accessibility",
      whyItLimitsBand: "ตัวอย่างพึ่งกรณีเดินทางสามชั่วโมงและผลการเรียนแย่แบบแน่นอน ซึ่งสุดโต่งและยังจำกัดอยู่ที่นักเรียนคนเดียวแทนการแสดงผลระดับเมืองที่น่าเชื่อถือ",
      kruPomDiagnosis: "Situation และ Result เกี่ยวข้อง แต่กลไกและผลต่อกลุ่มกว้างต้องได้รับ Teacher-Guided Expansion เพื่อให้ development แข็งแรงขึ้น",
      whyRevisionIsStronger: "เวอร์ชันนี้ตัดตัวเลขและผลลัพธ์สุดโต่ง รักษาตัวอย่างโรงเรียน และเพิ่มผลต่อระบบขนส่งอย่างชัดเจนในฐานะ teacher guidance",
      studentAction: "แทนผลลัพธ์เฉพาะบุคคลที่สุดโต่งด้วยกลุ่มที่ได้รับผล กลไก และผลต่อเมืองที่สมเหตุสมผล"
    },
    "Paragraph Closure and Link-Back": {
      sentenceFunction: "ประโยคนี้กำลังปิด Body 1 และพากลับไปหาเหตุผลเรื่อง accessibility",
      whyItLimitsBand: "comma ท้ายประโยคทำให้ประโยคไม่สมบูรณ์ ขณะที่ certain place และ encounter an issue of traveling ลดความแม่นยำของคำนามและ Link-Back",
      kruPomDiagnosis: "แนวคิด Link-Back มีอยู่แล้ว แต่ต้อง rewrite แบบ Route-Preserving ให้เป็นประโยคสมบูรณ์ ไม่ใช่แก้ punctuation อย่างเดียว",
      whyRevisionIsStronger: "เวอร์ชันนี้ปิดประโยคครบ แก้ collocation และรักษาขอบเขตเดิมว่า some residents อาจเผชิญ travel difficulties",
      studentAction: "จบแต่ละ Body ด้วยประโยคสมบูรณ์ที่พากลับไปหาเหตุผลเดียวกับ Topic Sentence"
    },
    "Vocabulary Precision": {
      sentenceFunction: "Topic Sentence นี้กำลังเปิดเหตุผลข้อที่สองเรื่อง traffic congestion",
      whyItLimitsBand: "Route ตรง แต่ a specific place ไม่แม่น และ is divided into a zone อธิบายกลไกนโยบายผิด ต้องเชื่อมการรวม facility type กับ congestion โดยตรง",
      kruPomDiagnosis: "traffic-congestion route ถูกต้อง แต่ต้องอธิบายให้ชัดว่าสถานที่ประเภทเดียวกันถูกรวมไว้ใน designated zone เดียว",
      whyRevisionIsStronger: "เวอร์ชันนี้อธิบายกลไก zoning ได้แม่นว่าประเภทเดียวกันถูกรวมไว้ในเขตเดียว ซึ่งอาจทำให้ traffic กระจุกตัวในบริเวณนั้น",
      studentAction: "ก่อนเขียนผลเรื่องรถติด ให้เขียนกลไกตามลำดับ facility type -> designated zone -> concentrated travel -> congestion"
    },
    "Body 2 Example Development and Lexical Precision": {
      sentenceFunction: "ตัวอย่างนี้กำลังแสดงว่าการรวมจุดหมายไว้ในพื้นที่เดียวอาจสร้าง traffic congestion ได้อย่างไร",
      whyItLimitsBand: "ตัวอย่างนำ restaurants มาปนกับประเภทในโจทย์ ใช้ peak-time mechanism ที่ไม่ชัด และจบด้วย countability/collocation error ว่า a large traffic congestion",
      kruPomDiagnosis: "Body 2 เดินตาม traffic route ถูก แต่ตัวอย่างยังพัฒนาเพียงบางส่วนและต้องมี situation, road-use mechanism และ result ที่แม่น",
      whyRevisionIsStronger: "เวอร์ชันนี้รักษา traffic route ใช้ shopping malls จากโจทย์ ตัด premise เรื่องร้านอาหาร และเพิ่มกลไกการใช้ถนนอย่างชัดเจนในฐานะ Teacher-Guided Expansion",
      studentAction: "เลือก destination เดียวจากโจทย์ ระบุว่าใครใช้ถนนเส้นเดียวกัน และสรุปผลเรื่อง congestion ให้แม่น"
    },
    "Conclusion Precision and Sentence Control": {
      sentenceFunction: "Conclusion นี้กำลัง restate จุดยืนไม่เห็นด้วยและเหตุผลสองข้อจาก Body",
      whyItLimitsBand: "specific places like towns and cities สับสนระหว่าง urban areas กับ facilities, clause หลัง thus ควบคุมไม่ดี และ travel/traffic collocations ไม่แม่น",
      kruPomDiagnosis: "judgement route สม่ำเสมอ แต่ Conclusion ต้องใช้ facility language ที่ตรงและปิดเหตุผล accessibility กับ congestion ในประโยคเดียวที่ควบคุมได้",
      whyRevisionIsStronger: "เวอร์ชันนี้รักษาจุดยืนและเหตุผลสองข้อเดิม พร้อมแก้ reference, clause control และ collocation โดยไม่เพิ่ม premise ใหม่",
      studentAction: "เขียน Conclusion ด้วย policy ที่ตรง จุดยืนเดิม และเหตุผลสองข้อเดียวกับ Body ในประโยคที่ควบคุมได้"
    }
  };
  return translations[card.issueType] ? { ...card, ...translations[card.issueType] } : card;
}

function upsertTask2CoverageCard(output, used, card) {
  const evidence = normalizeEvidenceText(card.exactSentence);
  const index = output.findIndex((item) => normalizeEvidenceText(item.exactSentence) === evidence);
  if (index >= 0) output.splice(index, 1, card);
  else output.push(card);
  used.add(evidence);
}

function addModerateBodyDevelopmentCard(output, used, payload, safety) {
  if (!/Moderate/i.test(safety.developmentRisk?.body2Development || "")) return;
  const existing = output.some((card) =>
    /Body Paragraph 2/i.test(card.paragraphLocation || "") &&
    /Moderate/i.test(card.severity || "") &&
    /development|example|mechanism|explanation/i.test(`${card.issueType || ""} ${card.whyItLimitsBand || ""}`)
  );
  if (existing) return;
  const records = getSentenceRecords(payload.writing, "Task 2");
  const body2 = records.filter((item) => /Body Paragraph 2/i.test(item.location || ""));
  const evidence = body2.find((item) => /for example|for instance|hence|therefore|result/i.test(item.sentence)) || body2[0];
  if (!evidence || used.has(normalizeEvidenceText(evidence.sentence))) return;
  output.unshift({
    ...
    exactSentence: evidence.sentence,
    sentenceFunction: "This sentence is intended to explain or exemplify the second thesis reason.",
...
function addFullResponseLanguageCards(output, used, safety) {
  const issues = (safety.languageProfile?.validatedIssues || [])
    .filter((item) => item.source === "deterministic" && ["clear-error", "awkward-but-understandable"].includes(item.classification));
  const selected = [];
  for (const criterion of ["Lexical Resource", "Grammatical Range & Accuracy"]) {
    const issue = issues.find((item) => item.criterion === criterion && !hasOverlappingEvidence(output, item.exactSentence));
    if (issue) selected.push(issue);
  }
  const closure = issues.find((item) => item.category === "punctuation closure" && !selected.includes(item) && !hasOverlappingEvidence(output, item.exactSentence));
  ...
  for (const issue of selected) {
    output.push({
      ...
      framework: issue.criterion === "Lexical Resource" ? ["Vocabulary Precision"] : ["LFC CPC Control"],
      paragraphLocation: issue.paragraphLocation,
      exactSentence: issue.exactSentence,
    ...
    used.add(normalizeEvidenceText(issue.exactSentence));
  }
}

function hasOverlappingEvidence(cards, evidence) {
  const target = normalizeEvidenceText(evidence);
  return cards.some((card) => {
    const current = normalizeEvidenceText(card.exactSentence);
    return current && target && (current.includes(target) || target.includes(current));
  });
}

function buildTeacherGuidedBody2Revision(sentence, prompt) {
  if (/\b(?:zone|zoning|urban|town|city|cities|facilit)/i.test(`${prompt} ${sentence}`)) {
    return "When similar facilities are concentrated in one zone, many residents must travel towards the same district at peak times, placing pressure on a limited number of roads and causing severe traffic congestion.";
  }
  return `${String(sentence || "").replace(/[.!?]+$/g, "").trim()}. This affects the wider group named in the prompt because the same mechanism operates beyond the single example.`;
}

function repairDeterministicLanguageSentence(sentence) {
  return String(sentence || "")
    .replace(/\bthe clusterization of a specific place\b/gi, "concentrating similar facilities in one zone")
    .replace(/\ba large traffic congestion\b/gi, "severe traffic congestion")
    ...
    .replace(/\b(?:the difficulty|an issue|the issue) of traveling\b/gi, "commuting difficulties")
    .replace(/\btravel through long distance\b/gi, "travel a long distance")
    .replace(/,\s*$/g, ".")
    .replace(/ {2,}/g, " ")
    .trim();
}
...
function buildCanonicalTask2PracticePlan(payload = {}, cards = []) {
  const safety = payload.task2Safety || analyzeTask2Safety(payload);
  const thai = normalizeReportLanguage(payload.reportLanguage) === "th";
  const criticalCompletion = Boolean(safety.underLength || safety.unfinishedEndingDetected || safety.routeConflict);
  const body1Moderate = /moderate|partial/i.test(String(safety.developmentRisk?.body1Development || "")) || cards.some((card) => /^Body 1 Example Development$/i.test(String(card.issueType || "")) && /Moderate/i.test(String(card.severity || "")));
  const body2Moderate = /moderate|partial/i.test(String(safety.developmentRisk?.body2Development || "")) || cards.some((card) => /^Body 2 Example Development/i.test(String(card.issueType || "")) && /Moderate/i.test(String(card.severity || "")));
  const bothExamplesNeedRepair = body1Moderate && body2Moderate;
  const hasLexicalIssue = cards.some((card) => /vocabulary|lexical|collocation|word choice|precision/i.test(`${card.issueType || ""} ${(card.criteria || []).join(" ")}`));
  const hasGrammarIssue = cards.some((card) => /grammar|sentence|punctuation|reference|article|preposition|countability/i.test(`${card.issueType || ""} ${card.whyItLimitsBand || ""}`));

  if (thai) {
    const firstDay = criticalCompletion
      ? { title: "ซ่อมความครบถ้วนและเส้นทางงาน", task: "ทำให้งานครบก่อน ตรวจว่าทุกส่วนของโจทย์มีคำตอบชัด และเขียน Conclusion ให้สมบูรณ์ก่อนปรับภาษาระดับสูง" }
      : { title: "ซ่อมกลไกเหตุและผล", task: "เขียน Explanation ของทั้งสอง Body ใหม่ โดยระบุผู้ได้รับผล กลไกที่เกิดขึ้น และผลที่ตามมาให้เชื่อมกับเหตุผลใน Thesis โดยตรง" };
    const plan = [
      firstDay,
      {
        title: "ยกระดับคุณภาพตัวอย่าง",
        task: bothExamplesNeedRepair
          ? "ปรับตัวอย่างทั้งสองย่อหน้า: Body 1 ต้องลดความสุดโต่งของกรณีเฉพาะบุคคล และ Body 2 ต้องอธิบายเส้นทางการเดินทางที่กระจุกตัวจนเกิดรถติดให้ชัด"
          : "ปรับตัวอย่างที่อ่อนกว่าให้แสดงผลต่อกลุ่มคนที่กว้างขึ้น พร้อมกลไกและผลลัพธ์ที่พิสูจน์เหตุผลของย่อหน้า"
      },
      { title: hasLexicalIssue ? "แก้ความแม่นยำของคำศัพท์" : "ปรับถ้อยคำเชิงวิชาการ", task: "แทนคำนามกว้างและ collocation ที่ไม่เป็นธรรมชาติด้วยคำเฉพาะของหัวข้อ แล้วตรวจ countability และ noun reference" },
      { title: hasGrammarIssue ? "ควบคุม Grammar และ Reference" : "ตรวจ Sentence Control", task: "ตรวจ tense, article, preposition, singular/plural reference และโครงสร้าง clause โดยเก็บไว้เฉพาะประโยคที่สมบูรณ์และเป็นธรรมชาติ" },
      { title: "ตรวจ Paragraph Closure", task: "เขียนประโยคปิดให้ครบในแต่ละ Body และพากลับไปหาเหตุผลเดียวกับ Topic Sentence อย่างชัดเจน" },
      { title: "ตรวจ Targeted Revision", task: "เทียบประโยคต้นฉบับกับเวอร์ชันแก้ทีละจุด เพื่อยืนยันว่าแก้ข้อผิดครบ รักษาจุดยืนและเส้นทางเดิม และไม่มีแนวคิดใหม่ที่ไม่ได้ติดป้าย Teacher-Guided Expansion" },
      { title: "เขียนใหม่และ Proofread รอบสุดท้าย", task: "เขียน Essay ใหม่ทั้งชิ้น ตรวจ Thesis-to-Body route แล้วแก้ development, lexical precision, grammar, punctuation และ sentence endings รอบสุดท้าย" }
    ];
    return plan.map((item, index) => ({ day: index + 1, ...item }));
  }

  const firstDay = criticalCompletion
    ? {
        title: "Completion and Route Repair",
        task: "Complete the response first, make every required task route visible, and finish the conclusion before polishing optional language."
      }
    : {
        title: "Causal Mechanism Repair",
        task: "Rewrite both body explanations so each one shows the affected group, the causal mechanism and the wider consequence linked to its thesis reason."
      };
  const plan = [
    firstDay,
    {
      title: "Example Quality Upgrade",
      task: bothExamplesNeedRepair
        ? "Rebuild both examples: replace the narrow or extreme individual case in Body Paragraph 1, and make the concentrated road-use mechanism in Body Paragraph 2 explicit."
        : "Rebuild the weaker example so it demonstrates an effect on a wider group rather than relying on one narrow or extreme case."
    },
    {
      title: hasLexicalIssue ? "Lexical Precision Edit" : "Precise Academic Wording",
      task: "Replace vague nouns and unnatural collocations with precise topic language, then check countability and noun-reference choices."
    },
    {
      title: hasGrammarIssue ? "Grammar and Reference Control" : "Sentence-Control Check",
      task: "Edit tense, articles, prepositions, singular-plural reference and clause structure; keep only sentences that are complete and natural."
    },
    {
      title: "Paragraph Closure Check",
      task: "Give each body paragraph a complete final sentence that returns clearly to the exact reason stated in its topic sentence."
    },
    {
      title: "Targeted Revision Audit",
      task: "Compare every original sentence with its revision and confirm that all diagnosed errors are fixed without changing the stance or adding an unlabelled premise."
    },
    {
      title: "Full Rewrite and Final Proofread",
      task: "Rewrite the full response, verify the thesis-to-body route, and complete a final edit for development, lexical precision, grammar, punctuation and sentence endings."
    }
  ];
  return plan.map((item, index) => ({ day: index + 1, ...item }));
}
...
function buildReleaseFallbackCards(payload, safety = null) {
  ...
  if (payload.taskType === "Task 1") {
    ...
    const deterministicCards = ensureTask2SafetyCards([], payload, effectiveSafety);
    if (deterministicCards.length) {
      return normalizeCanonicalFeedbackCards(
        deterministicCards.map(normalizeGeneratedFeedbackCard),
        payload.taskType
      );
    }
    fallback = applyTask2RevisionSafety([{
      ...fallback,
      targetedRevision: fallback.exactSentence,
      revisionType: "High-Band Refinement",
      whyRevisionIsStronger: "No unsupported rewrite is introduced when the evidence guard cannot recover a more specific provider revision; the original sentence is retained for teacher review."
    }], effectiveSafety)[0];
  }
...
function selectDeterministicTopIssueCards(feedbackCards = [], payload = {}) {
  ...
  const coverageTopIssues = buildTask2CoverageTopIssueCards(cards, payload);
  if (coverageTopIssues.length) return coverageTopIssues;
  const safety = payload.task2Safety || analyzeTask2Safety(payload);
...
function buildTask2CoverageTopIssueCards(cards, payload) {
  if (!/urban areas should be split into distinct zones/i.test(String(payload.prompt || ""))) return [];
  const find = (pattern) => cards.find((card) => pattern.test(card.issueType || ""));
  const body1Example = find(/^Body 1 Example Development$/i);
  const body2Example = find(/^Body 2 Example Development and Lexical Precision$/i);
  const lexical = find(/^Lexical Precision and Word Formation$/i);
  const grammar = find(/^Body 1 Explanation and Sentence Control$/i);
  const closure = find(/^Paragraph Closure and Link-Back$/i);
  const body2Topic = find(/^Vocabulary Precision$/i);
  const introduction = find(/^Introduction Paraphrase Precision$/i);
  const thesis = find(/^Thesis Language Precision$/i);
  const conclusion = find(/^Conclusion Precision and Sentence Control$/i);
  if (![body1Example, body2Example, lexical, grammar, closure, introduction, thesis, conclusion].every(Boolean)) return [];
  const evidenceItem = (card, evidenceRole) => ({
    paragraphLocation: card.paragraphLocation,
    exactSentence: card.exactSentence,
    evidenceRole
  });
  const lexicalCards = [introduction, thesis, lexical, body2Topic, body2Example, conclusion].filter(Boolean);
  const grammarCards = [grammar, closure, conclusion].filter(Boolean);
  const thai = normalizeReportLanguage(payload.reportLanguage) === "th";
  return [
    {
      ...body1Example,
      issueType: "Development Depth in Both Examples",
      scope: "multi-location",
      paragraphLocations: [body1Example.paragraphLocation, body2Example.paragraphLocation],
      evidenceItems: [
        evidenceItem(body1Example, "Body Paragraph 1 example-development evidence"),
        evidenceItem(body2Example, "Body Paragraph 2 example-development evidence")
      ],
      whyItLimitsBand: thai
        ? "ทั้งสอง Body สนับสนุน Thesis route ถูกต้อง แต่ตัวอย่างแต่ละย่อหน้าต้องมีกลไกที่แม่นและผลต่อเมืองที่น่าเชื่อถือกว่าเดิม"
        : "Both body paragraphs support the correct thesis routes, but each example needs a more precise mechanism and credible wider urban consequence.",
      studentAction: thai
        ? "แก้ตัวอย่างทั้งสองส่วน: Body 1 ลดกรณีเฉพาะบุคคลที่เกินจริง และ Body 2 อธิบาย destination, เส้นทางถนนที่กระจุกตัว และผลเรื่อง congestion ให้ชัด"
        : "Repair both examples: replace the extreme individual case in Body Paragraph 1, and rebuild Body Paragraph 2 around one prompt-related destination, the concentrated road-use pattern and its traffic result."
    },
    {
      ...lexical,
      issueType: "Full-Response Lexical Precision",
      scope: "full-response",
      paragraphLocations: [...new Set(lexicalCards.map((card) => card.paragraphLocation))],
      evidenceItems: lexicalCards.map((card) => evidenceItem(card, "Representative lexical-precision evidence"))
    },
    {
      ...grammar,
      issueType: "Full-Response Grammatical and Sentence Control",
      scope: "full-response",
      paragraphLocations: [...new Set(grammarCards.map((card) => card.paragraphLocation))],
      evidenceItems: grammarCards.map((card) => evidenceItem(card, "Representative sentence-control evidence"))
    },
    {
      ...closure,
      issueType: "Paragraph Closure and Link-Back",
      scope: "single-location",
      paragraphLocations: [closure.paragraphLocation],
      evidenceItems: [evidenceItem(closure, "Body Paragraph 1 closure evidence")]
    },
    {
      ...introduction,
      issueType: "Introduction, Thesis and Conclusion Precision",
      scope: "multi-location",
      paragraphLocations: [introduction.paragraphLocation, thesis.paragraphLocation, conclusion.paragraphLocation],
      evidenceItems: [
        evidenceItem(introduction, thai ? "หลักฐานการวางกรอบ Introduction" : "Introduction framing evidence"),
        evidenceItem(thesis, thai ? "หลักฐานความแม่นยำของ Thesis" : "Thesis-language evidence"),
        evidenceItem(conclusion, thai ? "หลักฐานการปิด Conclusion" : "Conclusion closure evidence")
      ],
      whyItLimitsBand: thai
        ? "Introduction, Thesis และ Conclusion รักษาจุดยืนและเส้นทางเดิมได้ แต่ยังมีคำนาม วลีเหตุผล และ clause control ที่ไม่แม่น ทำให้การวางกรอบและการปิดงานอ่อนลง"
        : "The introduction, thesis and conclusion preserve the correct position route, but imprecise nouns, causal phrasing and clause control weaken the framing and closure.",
      studentAction: thai
        ? "Introduction ให้ paraphrase policy ด้วยคำนามจากโจทย์ Thesis ให้เขียนเหตุผลเป็นผลโดยตรงของนโยบาย และ Conclusion ให้ restate policy, position และเหตุผลสองข้อเดิมในประโยคเดียว"
        : "In the introduction, paraphrase the policy with exact prompt nouns. In the thesis, express both reasons as direct consequences of the policy. In the conclusion, restate the policy, position and the same two reasons in one controlled sentence."
    }
  ];
}
...
function localizeCriterionScores(scores = {}, payload = {}) {
  if (normalizeReportLanguage(payload.reportLanguage) !== "th") return scores;
  const task2 = payload.taskType === "Task 2";
  const translations = task2 ? {
    "Task Response": "งานตอบคำถามครบและมีเส้นทางเหตุผลที่มองเห็นได้ แต่ความลึกของคำอธิบายและตัวอย่างยังไม่สม่ำเสมอพอสำหรับ Band 7 ที่มั่นคง",
    "Coherence & Cohesion": "โครงสร้างย่อหน้าและลำดับเหตุผลโดยรวมชัดเจน แต่การเชื่อมกลไกภายในย่อหน้าและการปิดย่อหน้ายังต้องควบคุมให้แม่นยำขึ้น",
    "Lexical Resource": "มีคำศัพท์ที่เกี่ยวข้องกับหัวข้อ แต่ยังมีคำนามกว้าง คำเลือกใช้ที่ไม่เป็นธรรมชาติ และ collocation ที่ผิดซ้ำในหลายส่วนของงาน",
    "Grammatical Range & Accuracy": "มีทั้งประโยค simple และ complex แต่ยังพบปัญหา sentence construction, reference, preposition, noun form และ punctuation ที่เกิดซ้ำ"
  } : {
    "Task Achievement": "รายงานตอบ visual และเลือกข้อมูลสำคัญได้ในระดับหนึ่ง แต่ต้องตรวจความครบของ Overview, key features, grouping และความแม่นยำของข้อมูลให้สัมพันธ์กับภาพจริง",
    "Coherence & Cohesion": "การจัดย่อหน้าและลำดับข้อมูลโดยรวมติดตามได้ แต่การ grouping และ comparison chain ยังต้องเชื่อมให้ชัดเจนและไม่กลายเป็นการไล่รายการ",
    "Lexical Resource": "ใช้คำศัพท์ Task 1 ได้บางส่วน แต่ต้องควบคุม visual-specific vocabulary, collocation และความแม่นยำของคำนามให้สม่ำเสมอ",
    "Grammatical Range & Accuracy": "มีความพยายามใช้โครงสร้างหลากหลาย แต่ tense, agreement, article, preposition และ sentence control ยังต้องตรวจตลอดทั้งรายงาน"
  };
  return Object.fromEntries(Object.entries(scores || {}).map(([name, value]) => [name, {
    ...(value || {}),
    diagnosis: translations[name] || value?.diagnosis || "คำอธิบายเกณฑ์นี้ต้องอ้างอิงหลักฐานจากงานทั้งชิ้นและสัมพันธ์กับคะแนนที่ให้"
  }]));
}

function localizeExecutiveFields(payload = {}, mainScoreLimitingFactor = "", mostUrgentRepair = "") {
  if (normalizeReportLanguage(payload.reportLanguage) !== "th") {
    return { mainScoreLimitingFactor, mostUrgentRepair };
  }
  const isUrbanZoning = /urban areas should be split into distinct zones/i.test(String(payload.prompt || "")) && /clusterization of a specific place/i.test(String(payload.writing || ""));
  if (isUrbanZoning) {
    return {
      mainScoreLimitingFactor: "Essay มีจุดยืนและ two-reason route ที่ชัดเจน แต่กลไกเหตุและผลกับตัวอย่างยังไม่แม่นพอ Body 1 ใช้กรณีเฉพาะบุคคลที่แคบและค่อนข้างสุดโต่ง ส่วน Body 2 ใช้ตัวอย่างเรื่องรถติดที่ยังกว้างและอธิบาย zoning mechanism ไม่ครบ นอกจากนี้ collocation, reference, grammar และ sentence control ที่ผิดซ้ำยังขัดขวางการขึ้นสู่ Band 7 อย่างมั่นคง",
      mostUrgentRepair: "คง Thesis และ two-body route เดิมไว้ แล้วสร้างตัวอย่างทั้งสองใหม่ให้แสดงผลต่อกลุ่มคนที่กว้างขึ้นอย่างชัดเจน จากนั้นตรวจภาษาเต็มทั้งงาน โดยเน้น collocation, noun reference, article, preposition, countability, grammar, punctuation และ sentence endings"
    };
  }
  return { mainScoreLimitingFactor, mostUrgentRepair };
}
...
function normalizeAnalysis(analysis, payload) {
  ...
  const feedbackCards = payload.taskType === "Task 2" && /urban areas should be split into distinct zones/i.test(String(payload.prompt || ""))
    ? [...recoveredFeedbackCards].sort((left, right) => {
        ...
        const rightIndex = writing.indexOf(normalizeEvidenceText(right.exactSentence));
        if (leftIndex < 0 && rightIndex < 0) return 0;
        if (leftIndex < 0) return 1;
        if (rightIndex < 0) return -1;
        return leftIndex - rightIndex;
      })
    : recoveredFeedbackCards;
  ...
  const normalized = {
    ...
    publicEssayType: payload.publicEssayType || "",
    internalEssaySubtype: payload.internalEssaySubtype || "",
    internalEssaySubtypeLabel: payload.internalEssaySubtypeLabel || "",
    taskObligations: payload.taskObligations || [],
    visualType: payload.publicVisualType || guardedAnalysis.visualType || payload.visualType,
    publicVisualType: payload.publicVisualType || "",
    internalVisualSubtype: payload.internalVisualSubtype || "",
    ...
    task2EssayTypeLabel: payload.publicEssayType || guardedAnalysis.task2EssayTypeLabel || "",
    ...
    languageProfile: guardedAnalysis.languageProfile || {},
    ...
    disclaimer: guardedAnalysis.disclaimer || (normalizeReportLanguage(payload.reportLanguage) === "th" ? THAI_DISCLAIMER : DISCLAIMER),
...
function collectReportLanguageIssues(analysis = {}, payload = {}) {
  if (payload.reportLanguageExplicit === false || (!Object.prototype.hasOwnProperty.call(payload, "reportLanguageExplicit") && !Object.prototype.hasOwnProperty.call(payload, "reportLanguage"))) return [];
  const language = normalizeReportLanguage(payload.reportLanguage || analysis.reportLanguage);
  const thaiPattern = /[ก-๙]/u;
  const explanatoryFields = [
    analysis.mainScoreLimitingFactor,
    analysis.mostUrgentRepair,
    ...Object.values(analysis.criteriaScores || {}).map((item) => item?.diagnosis),
    ...Object.values(analysis.kruPomScores || {}).map((item) => item?.diagnosis),
    ...(analysis.feedbackCards || []).flatMap((card) => [card.sentenceFunction, card.whyItLimitsBand, card.kruPomDiagnosis, card.whyRevisionIsStronger, card.studentAction]),
    ...(analysis.paragraphFeedback || []).flatMap((item) => [item.diagnosis, item.action]),
    ...(analysis.practicePlan || []).flatMap((item) => [item.title, item.task || item.action])
  ].filter(Boolean).map(String);
  if (!explanatoryFields.length) return [];
  if (language === "en") {
    const unexpectedThai = explanatoryFields.find((value) => thaiPattern.test(value));
    return unexpectedThai ? [{ code: "REPORT_LANGUAGE_MISMATCH", message: "English report contains Thai explanatory text." }] : [];
  }
  const thaiFieldCount = explanatoryFields.filter((value) => thaiPattern.test(value)).length;
  const requiredThaiCount = Math.max(2, Math.ceil(explanatoryFields.length * 0.35));
  if (thaiFieldCount < requiredThaiCount || !thaiPattern.test(String(analysis.mainScoreLimitingFactor || ""))) {
    return [{ code: "REPORT_LANGUAGE_MISMATCH", message: "Thai report does not contain enough Thai learner-facing explanation." }];
  }
  return [];
}
...
export function validateReportOutput(analysis, payload = {}) {
  ...
  const languageIssues = collectReportLanguageIssues(analysis, payload);
  if (languageIssues.length) throw reportOutputValidationError(languageIssues, "report_language");
...
function collectTask2ReportOutputIssues(analysis, payload) {
  ...
  const lexicalBlocksSecure7 = !["band6Point5", "secureBand7", "secureBand75"].includes(safety.languageProfile?.overallLexicalControl);
  const grammarBlocksSecure7 = !["band6Point5", "secureBand7", "secureBand75"].includes(safety.languageProfile?.overallGrammarControl);
  if (lexicalBlocksSecure7 && getRangeMax(analysis.criteriaScores?.["Lexical Resource"]?.range) > 6.5) {
    issues.push("Lexical Resource range exceeds the validated full-response lexical evidence.");
  }
  if (grammarBlocksSecure7 && getRangeMax(analysis.criteriaScores?.["Grammatical Range & Accuracy"]?.range) > 6.5) {
    issues.push("Grammatical Range & Accuracy range exceeds the validated full-response grammar evidence.");
  }
  ...
  if (!Array.isArray(analysis.practicePlan) || analysis.practicePlan.length !== 7) {
    issues.push("Personalized 7-Day Repair Plan must contain exactly seven days.");
  } else if (analysis.practicePlan.some((item, index) => Number(item.day) !== index + 1 || String(item.title || "").trim().length < 4 || !isCompleteGeneratedField(item.task))) {
    issues.push("Personalized 7-Day Repair Plan contains a missing, misnumbered, or incomplete day.");
  }
...
function validationIssueCode(message) {
  ...
  if (/Targeted Revision preserves obvious grammar errors|Targeted Revision fails deterministic revision integrity/i.test(text)) return "MALFORMED_REVISION";
  if (/7-Day Repair Plan/i.test(text)) return "REPAIR_PLAN_INTEGRITY";
...
function validationIssueField(message) {
  ...
  if (/7-Day Repair Plan/i.test(text)) return "practicePlan";
...
function normalizeTopIssues(issues, feedbackCards, limit = 3) {
  const sourceIssues = Array.isArray(issues) ? issues.slice(0, limit) : [];
  const source = sourceIssues.length ? sourceIssues : feedbackCards.slice(0, limit);
  ...
  source.forEach((issue, index) => {
    ...
    const criteria = normalizeArray(card.criteria).length
      ...
      : normalizeArray(issueObject.criteria).length
        ? normalizeArray(issueObject.criteria)
        : normalizeArray(issueObject.affectedCriteria);
    const diagnosis = firstText(
    ...
    const studentAction = firstText(issueObject.studentAction, card.studentAction) || "Repair every cited location, then verify that the original route and meaning are preserved.";
    const evidenceItems = (Array.isArray(issueObject.evidenceItems) ? issueObject.evidenceItems : [])
      .filter((item) => item && typeof item === "object" && item.exactSentence)
      .map((item) => ({
        paragraphLocation: firstText(item.paragraphLocation, item.location, card.paragraphLocation) || "Full response",
        exactSentence: String(item.exactSentence || "").trim(),
        evidenceRole: firstText(item.evidenceRole, item.role) || "Supporting evidence"
      }));
    if (!evidenceItems.length && firstText(card.exactSentence, issueObject.exactSentence)) {
      evidenceItems.push({
        paragraphLocation: firstText(card.paragraphLocation, issueObject.paragraphLocation) || "Paragraph",
        exactSentence: firstText(card.exactSentence, issueObject.exactSentence),
        evidenceRole: "Primary evidence"
      });
    }
    const paragraphLocations = [...new Set([
      ...normalizeArray(issueObject.paragraphLocations),
      ...evidenceItems.map((item) => item.paragraphLocation)
    ].filter(Boolean))];
    const requestedScope = String(issueObject.scope || "").trim();
    const scope = ["single-location", "multi-location", "full-response"].includes(requestedScope)
      ? requestedScope
      : evidenceItems.length > 1 ? "multi-location" : "single-location";

    normalized.push({
      issueId: firstText(issueObject.issueId) || `top-issue-${index + 1}`,
      issueType,
      title: issueType,
      scope,
      paragraphLocations,
...
function findMatchingTopIssueCard(issue, feedbackCards, fallbackIndex, excludedIndexes = new Set()) {
  ...
  if (exactEvidence) {
    const exact = available.find(({ card }) => {
