...
function updateUserPanel(user) {
  ...
  if (isUnlimitedAccount(currentUser)) {
    studentPlan.textContent = `${currentUser.plan || "Internal Use"} (${currentUser.role === "admin" ? "Admin" : "Teacher"} account)`;
    studentQuota.textContent = "Internal use. No student credit is deducted.";
...
function updateAnalysisCreditNote() {
  ...
  if (isUnlimitedAccount(currentUser)) {
    analysisCreditNote.textContent = "Teacher/internal account: no student credit is deducted.";
...
async function loadStudentProfiles(preferredToken = "") {
  ...
  try {
    ...
    studentProfiles = Array.isArray(result.profiles) ? result.profiles : [];
    const remembered = preferredToken || sessionStorage.getItem("diagnostic-selected-student") || "";
    selectedStudentProfileToken = isTeacherAccount(currentUser)
      ? (studentProfiles.some((profile) => profile.profileToken === remembered) ? remembered : "")
      : (studentProfiles[0]?.profileToken || "");
...
function renderStudentProfileOptions() {
  studentProfileSelect.innerHTML = [
  ...
  printReport.innerHTML = `
    <article class="print-page print-cover">
      ...
      <section class="print-section">
        ...
        ${taskType === "Task 2" && normalized.bodyRouteSummary ? renderPrintCallout("Position and Route", `Detected position: ${normalized.detectedPosition || "unclear"} (${normalized.positionConfidence || "low"} confidence). ${normalized.bodyRouteSummary}`) : ""}
