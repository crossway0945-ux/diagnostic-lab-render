...
const POSITION_PATTERN = /\b(?:i\s+(?:(strongly|firmly|completely|fully|generally|partly|partially)\s+)?(agree|disagree)|in my (?:view|opinion)[^.!?]{0,80}\b(agree|disagree)|i believe[^.!?]{0,80}\b(?:should|must|ought|outweigh|more (?:important|significant|beneficial)))\b/i;
...
export function analyzeTask2Safety(payload = {}) {
  ...
  const paragraphs = splitParagraphs(writing);
  const introduction = paragraphs[0] || "";
  const conclusion = paragraphs.length >= 3 ? paragraphs.at(-1) : "";
  const bodyParagraphs = paragraphs.length >= 3 ? paragraphs.slice(1, -1) : paragraphs.slice(1);
  ...
  const bodyRoutes = bodyParagraphs.map(classifyBodyRoute);
  ...
  const shortBodyParagraphs = bodyParagraphs
    .map((paragraph, index) => ({ paragraph: index + 1, wordCount: countWords(paragraph) }))
    .filter((item) => item.wordCount < 40);
  ...
  return {
    ...
    positionConfidence,
    routeConflict,
    bodyRouteSummary: bodyRoutes.length
      ? bodyRoutes.map((route, index) => `Body ${index + 1}: ${route}`).join(" | ")
    ...
    completionIntegrity: completionStatus === "unfinished"
      ...
      : completionStatus === "substantially incomplete"
        ...
        : completionStatus === "mostly complete"
          ? "partially controlled"
          : "stable",
    languageControlIntegrity: meaningErrors.some((item) => item.category === "meaning-reversing") || languageErrors.length >= 4
      ? "weak"
...
function buildCompletionEvidence({ wordMetadata, paragraphs, conclusion, ending, unfinishedEndingDetected, shortBodyParagraphs }) {
  const evidence = [
    `Verified count: ${wordMetadata.wordCount}/${wordMetadata.minimumWordCount} words`,
    `${paragraphs.length} detected paragraphs`,
    `conclusion length: ${countWords(conclusion)} words`
  ];
...
function classifyBodyRoute(paragraph) {
