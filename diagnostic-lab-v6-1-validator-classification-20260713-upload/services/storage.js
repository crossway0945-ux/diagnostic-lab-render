...
class JsonFileStorage {
  ...
  async listStudentProfiles(ownerAccountId) {
    const profiles = await this.readStudentProfiles();
    return profiles.filter((profile) => profile.ownerAccountId === ownerAccountId && profile.active);
  }

  async getStudentProfile(ownerAccountId, profileId) {
    const profiles = await this.readStudentProfiles();
    return profiles.find((profile) => profile.ownerAccountId === ownerAccountId && profile.id === profileId && profile.active) || null;
  }
