  ...
  return async function handleApiRequest(request) {
    ...
    try {
      ...
      if (method === "GET" && apiPath === "/api/student-profiles") {
        ...
        return jsonResponse(200, { ok: true, profiles: profiles.map((profile) => sanitizeStudentProfileForClient(session.user, profile)) });
      }
...
async function listProfilesForUser(storage, user) {
  if (isTeacherOrAdmin(user)) return storage.listStudentProfiles(user.username);
  const profile = await storage.ensureStudentProfile(user.username, user.displayName || user.username);
  return [profile];
}
...
function sanitizeStudentProfileForClient(user, profile) {
  return {
...
async function resolveStudentProfileToken(storage, user, token) {
  if (!token) return null;
  const profiles = await storage.listStudentProfiles(user.username);
