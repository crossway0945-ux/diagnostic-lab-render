...
function normalizeTopIssues(issues, feedbackCards) {
  ...
  return source.map((issue, index) => {
    ...
    const matched = findMatchingTopIssueCard(issueObject, feedbackCards, index);
...
function findMatchingTopIssueCard(issue, feedbackCards, fallbackIndex) {
  ...
  const categoryPattern = /thesis/.test(label)
    ? /thesis|position route/i
    : /unfinished|underlength|completion/.test(label)
      ? /unfinished|underlength|completion integrity/i
      : /meaning|vocabulary|lexical/.test(label)
        ? /meaning-changing|meaning-reversing/i
        : /counterargument|body 2/.test(label)
          ? /counterargument|body paragraph 2/i
          : null;
  const candidateIndexes = cards
    .map((card, index) => ({ card, index }))
    .filter(({ card }) => !categoryPattern || categoryPattern.test(card.issueType || ""));
  const pool = candidateIndexes.length ? candidateIndexes : cards.map((card, index) => ({ card, index }));
  ...
  if (best && (best.score > 0 || candidateIndexes.length)) return { card: best.card, index: best.index };
  const index = Math.min(fallbackIndex, cards.length - 1);
