...
import { analyzeTask2Safety, parseTask2Structure } from "./task2Safety.js";
...
function applyTask2StrictGuardrails(analysis, payload) {
  ...
  let next = {
    ...
    revisedThesisRevisionType: safety.positionConfidence === "low" ? "Teacher-Guided Recommended Route" : "Route-Preserving Revision"
  };

  next = applyTask2LowBandSafety(next, payload, safety);
  if (safety.criticalInteraction || safety.seriousInteraction) return next;

  if (!strictModeApplied && safety.languageAccuracyRisk?.blocksSecureBand7) {
    return applyTask2LanguageAccuracyCalibration(next, safety);
  }

  if (!strictModeApplied) {
    if (isTask2Band7Candidate(payload, next, guardrail, safety)) {
      if (isSecureTask2Band75Candidate(next, safety)) {
        return applyTask2Band7Calibration(next, guardrail, safety);
      }
      return applyTask2Band7OnlyCalibration(next, safety);
    }
    return next;
  }
...
function applyTask2LanguageAccuracyCalibration(analysis, safety) {
  const routeIsControlled = safety.positionConfidence === "high" && !safety.routeConflict;
  const mainScoreLimitingFactor = routeIsControlled
    ? "The main limitation is uneven development combined with frequent grammar and collocation errors. The position and paragraph route are clear, but the weaker side is repetitive or speculative while the stronger side contains unsupported causal claims."
    : "Frequent grammar, spelling, and collocation errors reduce meaning control, while development remains uneven.";
  const mostUrgentRepair = "Strengthen one controlled causal chain in each body paragraph, add a specific or plausible example, and remove recurring article, agreement, spelling and collocation errors.";
  const criteriaScores = {
    ...(analysis.criteriaScores || {}),
    "Task Response": calibrateTask2CriterionAtMost(
      analysis.criteriaScores?.["Task Response"],
      "6.5-7.0",
      "The position and outweigh route are clear, but development is uneven: one side is repetitive or speculative and the stronger side contains unsupported causal claims."
    ),
    "Coherence & Cohesion": calibrateTask2CriterionAtMost(
      analysis.criteriaScores?.["Coherence & Cohesion"],
      "6.5-7.0",
      "The paragraph route is controlled, but repetition and several compressed outcomes weaken progression within the body paragraphs."
    ),
    "Lexical Resource": calibrateTask2CriterionAtMost(
      analysis.criteriaScores?.["Lexical Resource"],
      "6.0-6.5",
      "Several spelling, word-choice, and collocation errors remain visible, so vocabulary is not securely controlled at Band 7."
    ),
    "Grammatical Range & Accuracy": calibrateTask2CriterionAtMost(
      analysis.criteriaScores?.["Grammatical Range & Accuracy"],
      "6.0-6.5",
      "Recurring article, agreement, reference, and sentence-structure errors prevent a secure Band 7 grammar profile."
    )
  };

  return {
    ...analysis,
    estimatedBandRange: chooseConservativeRange(analysis.estimatedBandRange, "6.5"),
    mainScoreLimitingFactor,
    mainScoreLimitingIssue: mainScoreLimitingFactor,
    mostUrgentRepair,
    topRepairPriority: mostUrgentRepair,
    criteriaScores,
    taskResponseCapReason: "",
    overallBandCap: "6.5",
    strictModeApplied: false,
    capsApplied: [{
      criterion: "Overall / Language accuracy",
      reason: "Frequent evidence-based grammar, spelling, and collocation errors block automatic promotion to a secure Band 7 range.",
      cap: "6.5"
    }],
    warnings: [
      ...(Array.isArray(analysis.warnings) ? analysis.warnings : []),
      "Task 2 language-accuracy calibration applied: clear route preserved, but automatic Band 7 promotion was blocked by frequent evidence-based errors."
    ].filter((warning, index, list) => list.indexOf(warning) === index)
  };
}

function calibrateTask2CriterionAtMost(value, maximumRange, diagnosis) {
  const score = typeof value === "object" && value
    ? { ...value }
    : { range: typeof value === "string" ? value : "" };
  score.range = chooseConservativeRange(score.range, maximumRange);
  score.diagnosis = diagnosis;
  return score;
}

function applyTask2LowBandSafety(analysis, payload, safety) {
  const next = {
    ...analysis,
    feedbackCards: ensureTask2SafetyCards(analysis.feedbackCards || [], payload, safety)
  };
  if (!safety.criticalInteraction && !safety.seriousInteraction) return next;

  const critical = safety.criticalInteraction;
  const overallRange = critical ? "4.0-4.5" : "4.5-5.0";
  const taskResponseRange = critical ? "4.0" : "4.0-4.5";
  const otherRange = critical ? "4.0-4.5" : "4.5-5.0";
  const countEvidence = `Verified word count: ${safety.wordCount}/${safety.minimumRequiredWords}; completion: ${safety.completionStatus}.`;
  const routeEvidence = `${safety.bodyRouteSummary} Detected position: ${safety.detectedPosition} (${safety.positionConfidence} confidence).`;
  const meaningEvidence = safety.meaningReversingErrors[0]?.exactEvidence || safety.meaningChangingErrors[0]?.exactEvidence || safety.evidence.ending;
  const completionDiagnosis = critical
    ? `The response is ${safety.underLengthBy} words below the Task 2 minimum, ends with an unfinished sentence, and does not complete a controlled position-and-body route. These failures interact rather than operating as an isolated word-count penalty.`
    : safety.directQuestionMissingPart
      ? "The response does not develop an answer route for both explicit questions, so an important part of the task is materially under-addressed."
      : `The response is ${safety.underLengthBy} words below the Task 2 minimum and remains structurally incomplete; the unresolved position and body route prevent a secure Band 5.5 Task Response profile.`;

  next.estimatedBandRange = chooseConservativeRange(next.estimatedBandRange, overallRange);
  next.criteriaScores = {
    ...(next.criteriaScores || {}),
    "Task Response": buildTask2LowBandScore(next.criteriaScores?.["Task Response"], taskResponseRange, completionDiagnosis, `${countEvidence} ${routeEvidence}`),
    "Coherence & Cohesion": buildTask2LowBandScore(next.criteriaScores?.["Coherence & Cohesion"], otherRange, "Paragraph breaks and linkers do not create a complete progression when the thesis, body routes, and ending do not form one Golden Thread.", routeEvidence),
    "Lexical Resource": buildTask2LowBandScore(next.criteriaScores?.["Lexical Resource"], otherRange, "Meaning-changing and meaning-reversing word choices affect the argument itself, so topic vocabulary cannot compensate for weak meaning control.", meaningEvidence),
    "Grammatical Range & Accuracy": buildTask2LowBandScore(next.criteriaScores?.["Grammatical Range & Accuracy"], otherRange, "Frequent form errors occur in controlling sentences and the response ends in a sentence fragment, which materially affects clarity and completion.", safety.languageControlErrors?.[0]?.exactEvidence || safety.evidence.ending)
  };
  next.mainScoreLimitingFactor = critical
    ? `The essay is ${safety.underLengthBy} words below the minimum, ends with an unfinished conclusion, and lacks a controlled position-and-body route. Together with limited development and meaning-affecting language errors, these issues prevent it from functioning as a complete Task 2 response.`
    : completionDiagnosis;
  next.mainScoreLimitingIssue = next.mainScoreLimitingFactor;
  next.mostUrgentRepair = `Submit a complete essay of at least ${safety.minimumRequiredWords} words, including a fully finished conclusion. Before sentence polishing, state one explicit position in the introduction and make both body paragraphs prove that route.`;
  next.topRepairPriority = next.mostUrgentRepair;
  next.taskResponseCapReason = next.mainScoreLimitingFactor;
  next.overallBandCap = critical ? "4.5" : "5.0";
  next.strictModeApplied = true;
  next.capsApplied = [
    ...(Array.isArray(next.capsApplied) ? next.capsApplied : []),
    { criterion: "Overall / Task Response", reason: next.taskResponseCapReason, cap: next.overallBandCap }
  ];
  next.criticalFlags = Array.from(new Set([
    ...(Array.isArray(next.criticalFlags) ? next.criticalFlags : []),
    safety.unfinishedEndingDetected ? "Unfinished response" : "Substantially incomplete response",
    safety.routeConflict ? "Conflicting body routes" : "Unclear position",
    safety.meaningReversingErrors.length ? "Meaning-reversing language" : "Meaning-affecting language"
  ]));
  next.severitySummary = safety.criticalInteractionSummary;
  next.promptCoverageStatus = "Partially covered";
  next.thesisRouteStatus = safety.detectedPosition === "unclear" || safety.detectedPosition === "contradictory" ? "Missing or contradictory position" : next.thesisRouteStatus;
  next.bodyRouteAlignmentStatus = safety.routeConflict ? "Misaligned" : next.bodyRouteAlignmentStatus;
  next.conclusionClosureStatus = safety.unfinishedEndingDetected ? "Unfinished" : next.conclusionClosureStatus;
  next.warnings = Array.from(new Set([
    ...(Array.isArray(next.warnings) ? next.warnings : []),
    `Task 2 completion check: ${safety.wordCount}/${safety.minimumRequiredWords} words; status: ${safety.completionStatus}.`,
    "Task 2 compound-severity guardrail applied from combined completion, route, development, and language evidence."
  ]));
  return next;
}

function buildTask2LowBandScore(value, capRange, diagnosis, evidence) {
  const score = typeof value === "object" && value
    ? { ...value }
    : { range: typeof value === "string" ? value : "" };
  score.range = chooseConservativeRange(score.range, capRange);
  score.diagnosis = diagnosis;
  score.evidence = firstText(evidence, score.evidence);
  return score;
}

function applyTask2RevisionSafety(cards, safety) {
  return cards.map((card) => {
    const cardText = `${card.issueType || ""} ${card.kruPomDiagnosis || ""} ${card.targetedRevision || ""}`;
    let revisionType = card.revisionType;
    if (!revisionType || !["Minimal Correction", "Route-Preserving Revision", "Teacher-Guided Recommended Route", "Model Paragraph"].includes(revisionType)) {
      if (safety.positionConfidence === "low" && /thesis|position|route|conclusion|\bi (?:strongly|generally|partly)?\s*(?:agree|disagree)\b/i.test(cardText)) {
        revisionType = "Teacher-Guided Recommended Route";
      } else if (/meaning|lexical|vocabulary|word form|grammar/i.test(cardText) && countWords(card.targetedRevision) < 35) {
        revisionType = "Minimal Correction";
      } else if (countWords(card.targetedRevision) >= 55) {
        revisionType = "Model Paragraph";
      } else {
        revisionType = "Route-Preserving Revision";
      }
    }
    const teacherGuidance = revisionType === "Teacher-Guided Recommended Route"
      ? safety.recommendedRouteRationale
      : "";
    return {
      ...card,
      revisionType,
...
function ensureTask2SafetyCards(cards, payload, safety) {
  ...
  if (safety.unfinishedEndingDetected && safety.evidence.ending && !used.has(normalizeEvidenceText(safety.evidence.ending))) {
    output.unshift({
      ...
      kruPomDiagnosis: "Completion is a route-control issue as well as a grammar issue: the final position and its justification are not delivered.",
      targetedRevision: "Complete the conclusion only after selecting one explicit position and making both body paragraphs prove it.",
      revisionType: "Teacher-Guided Recommended Route",
...
function detectTask2StrictGuardrail(payload, cards = []) {
  const records = getSentenceRecords(payload.writing, "Task 2");
  const paragraphs = getParagraphs(payload.writing, "Task 2");
...
function isTask2Band7Candidate(payload, analysis, guardrail, safety) {
  ...
  const writing = String(payload.writing || "");
  const paragraphs = getParagraphs(writing, "Task 2");
  const wordCount = Number(payload.wordCount ?? getWordCountMetadata("Task 2", writing).wordCount);
  const promptAndType = `${payload.prompt || ""} ${payload.essayType || ""}`;
  const isOutweigh = /advantage|disadvantage|outweigh/i.test(promptAndType);
  const hasClearPosition = safety.positionConfidence !== "low" && !["unclear", "contradictory"].includes(safety.detectedPosition);
  ...
  const hasBodyRoute = isOutweigh
    ? !/route unclear from the controlling sentences|mixes advantage and disadvantage claims/i.test(safety.bodyRouteSummary || "")
    : paragraphs.length >= 4;
...
function isSecureTask2Band75Candidate(analysis, safety) {
  if (safety.languageAccuracyRisk?.blocksSecureBand75) return false;
  if (safety.developmentRisk?.unevenDevelopment) return false;
  if (/does not state a clear final position|partially controlled|conflicting|uncontrolled/i.test(safety.bodyRouteSummary || "")) return false;
  const moderateLimiters = (analysis.feedbackCards || []).filter((card) => {
    const severity = String(card.severity || "").toLowerCase();
    return ["moderate", "needs work"].includes(severity) && isModerateTask2Card(card);
  });
  return moderateLimiters.length <= 1;
}

function applyTask2Band7Calibration(analysis, guardrail, safety) {
  ...
  return {
    ...
    mainScoreLimitingFactor: buildTask2HighBandSummary(analysis, safety, true),
    mostUrgentRepair: buildTask2HighBandUrgentRepair(analysis, safety, true),
    criteriaScores,
    feedbackCards,
    top3Issues: [],
    taskResponseCapReason: "",
...
function applyTask2Band7OnlyCalibration(analysis, safety) {
  const criteriaScores = {
    ...(analysis.criteriaScores || {})
  };
  for (const criterion of ["Task Response", "Coherence & Cohesion", "Lexical Resource", "Grammatical Range & Accuracy"]) {
    criteriaScores[criterion] = calibrateTask2CriterionAtMost(
      criteriaScores[criterion],
      "7.0",
      buildBand7CriterionDiagnosis(criterion, safety)
    );
    criteriaScores[criterion].range = "7.0";
  }
  const mainScoreLimitingFactor = buildTask2HighBandSummary(analysis, safety, false);
  const mostUrgentRepair = buildTask2HighBandUrgentRepair(analysis, safety, false);
  return {
    ...analysis,
    ...
    warnings: [
      ...(Array.isArray(analysis.warnings) ? analysis.warnings : []),
      "Task 2 Band 7 calibration applied: the essay meets Band 7 conditions but does not pass the secure 7.5 eligibility gate."
    ].filter((warning, index, list) => list.indexOf(warning) === index)
  };
}

function buildTask2HighBandSummary(analysis, safety, secure75) {
  if (safety.developmentRisk?.missingNationalBridge || safety.developmentRisk?.overloadedAdvantageRoute) {
    return "The main limitation is analytical depth after examples: individual or company-level cases are not consistently connected to wider national consequences, and the stronger body paragraph compresses several relevant claims into one route. Precision and mechanical slips also prevent a secure Band 7.5 profile.";
  }
  const developmentCard = (analysis.feedbackCards || []).find((card) => /development|explanation|sar example|evidence/i.test([
    card.issueType,
    ...(Array.isArray(card.framework) ? card.framework : [card.framework]),
    card.whyItLimitsBand
  ].filter(Boolean).join(" ")));
  if (developmentCard) return firstText(developmentCard.whyItLimitsBand, developmentCard.kruPomDiagnosis);
  if (secure75) return "The response reaches the 7.0-7.5 range because its position, paragraph route, development, and language control are consistently strong; the remaining limitation is an isolated high-band repair point identified in the detailed feedback.";
  return "The response has a clear position and controlled route, but moderate development or precision limitations prevent a secure Band 7.5 profile.";
}

function buildTask2HighBandUrgentRepair(analysis, safety, secure75) {
  if (safety.developmentRisk?.missingNationalBridge || safety.developmentRisk?.overloadedAdvantageRoute) {
    return "After each example, explain its wider significance before introducing another claim. Keep the stronger body paragraph to two connected mechanisms and complete a final precision scan.";
  }
  const priorityCard = (analysis.feedbackCards || []).find((card) => /development|explanation|sar example|thesis|vocabulary|grammar/i.test([
    card.issueType,
    ...(Array.isArray(card.framework) ? card.framework : [card.framework])
  ].filter(Boolean).join(" ")));
  return firstText(priorityCard?.studentAction, secure75 ? "Repair the highest-impact evidence-based issue while preserving the existing route." : "Deepen the weakest explanation and complete a final precision scan before submission.");
}

function buildBand7CriterionDiagnosis(criterion, safety) {
  if (criterion === "Task Response") return "The task is fully addressed with a clear outweigh position, but examples need more consistently extended analysis before the response can be considered securely above Band 7.";
  if (criterion === "Coherence & Cohesion") return "The overall route is clear, but one dense body paragraph compresses several claims and examples, limiting internal progression.";
  if (criterion === "Lexical Resource") return "Vocabulary is varied and appropriate at Band 7, but several intensity or precision choices prevent consistently polished Band 7.5 control.";
  return safety.languageAccuracyRisk?.blocksSecureBand75
    ? "Complex sentences are generally controlled, but several mechanical or reference-precision slips prevent a secure Band 7.5 profile."
    : "Grammar is controlled at Band 7, with remaining errors treated as high-band repair points.";
}
...
function normalizeAnalysis(analysis, payload) {
  ...
  const feedbackCards = enrichFeedbackStrategies(locationAlignedCards, payload);
...
function collectTask2ReportOutputIssues(analysis, payload) {
  ...
  if (safety.languageAccuracyRisk?.blocksSecureBand7) {
    if (getRangeMax(analysis.estimatedBandRange) > 6.5) issues.push("Overall band range exceeds the frequent language-accuracy evidence.");
  ...
  for (const [index, card] of analysis.feedbackCards.entries()) {
    ...
    if (safety.positionConfidence === "low" && /\bi\s+(?:strongly|firmly|generally|partly|partially)?\s*(?:agree|disagree)\b/i.test(card.targetedRevision || "") && card.revisionType !== "Teacher-Guided Recommended Route" && card.revisionType !== "Model Paragraph") {
  ...
  if (safety.criticalInteraction || safety.seriousInteraction) {
    const overallMax = getRangeMax(analysis.estimatedBandRange);
    const allowedMax = safety.criticalInteraction ? 4.5 : 5.0;
    if (!Number.isFinite(overallMax) || overallMax > allowedMax) issues.push("Overall band range conflicts with the compound-severity evidence.");
    const taskResponseMax = getRangeMax(analysis.criteriaScores?.["Task Response"]?.range);
    if (!Number.isFinite(taskResponseMax) || taskResponseMax > (safety.criticalInteraction ? 4.0 : 4.5)) issues.push("Task Response range conflicts with the completion and route evidence.");
