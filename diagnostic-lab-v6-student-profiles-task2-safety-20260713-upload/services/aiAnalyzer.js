...
import { analyzeTask2Safety } from "./task2Safety.js";
...
function applyTask2LowBandSafety(analysis, payload, safety) {
  ...
  next.mainScoreLimitingFactor = `${completionDiagnosis} ${safety.criticalInteractionSummary}`;
  next.mainScoreLimitingIssue = next.mainScoreLimitingFactor;
  next.mostUrgentRepair = `Rebuild the essay route before sentence polishing: state one explicit position in the introduction, make both body paragraphs prove that route, and submit a complete conclusion of at least ${safety.minimumRequiredWords} words overall.`;
...
function applyTask2RevisionSafety(cards, safety) {
  return cards.map((card) => {
    ...
    if (!revisionType || !["Minimal Correction", "Route-Preserving Revision", "Teacher-Guided Recommended Route", "Model Paragraph"].includes(revisionType)) {
      if (/meaning|lexical|vocabulary|word form|grammar/i.test(cardText) && countWords(card.targetedRevision) < 35) {
        revisionType = "Minimal Correction";
      } else if (safety.positionConfidence === "low" && /thesis|position|route|conclusion|\bi (?:strongly|generally|partly)?\s*(?:agree|disagree)\b/i.test(cardText)) {
...
function ensureTask2SafetyCards(cards, payload, safety) {
  ...
  for (const meaningError of meaningErrors) {
    ...
    const revision = String(meaningError.exactEvidence || "")
    ...
    output.push({
      ...
      revisionType: "Minimal Correction",
      whyRevisionIsStronger: `The minimal correction in "${revision}" restores the intended semantic direction without inventing a new argument.`,
...
function locateExactEvidence(writing, evidence) {
  const record = getSentenceRecords(writing).find((item) => normalizeEvidenceText(item.sentence).includes(normalizeEvidenceText(evidence)));
...
export function validateReportOutput(analysis, payload = {}) {
  ...
  if (metadataIssues.length) throw reportOutputValidationError(metadataIssues);
  ...
  if (payload.taskType === "Task 2") {
    ...
    if (task2Issues.length) throw reportOutputValidationError(task2Issues);
    return normalizedTask2;
  }
  if (payload.taskType !== "Task 1") return metadataValidated;
...
function collectTask2ReportOutputIssues(analysis, payload) {
  ...
  if (safety.criticalInteraction || safety.seriousInteraction) {
    ...
    const requiredLocations = getParagraphs(payload.writing).map((_, index, list) => paragraphName(index, list.length).toLowerCase());
...
function hasConflictingProviderWordCount(analysis, payload) {
  ...
  const countClaims = Array.from(generatedText.matchAll(/word\s*count\s*(?:is|of|:)?\s*(\d+)|\b(?:response|report|essay|writing|answer)\s+(?:contains|has|is)\s+(\d+)\s+words?\b|\b(\d+)\s+words?\s+(?:long|in\s+total)\b/gi))
    .map((match) => Number(match[1] || match[2] || match[3]))
    .filter(Number.isFinite);
  const shortfallClaims = Array.from(generatedText.matchAll(/shortfall\s*(?:is|of|:)?\s*(\d+)/gi))
