...
async function testEvinProblemSolutionRegression() {
  ...
  assert.equal(safety.detectedPosition, "");
  assert.equal(safety.routeAssessment.status, "partial");
  ...
  assert.equal(canonical.frameworkAssessment.sar.scoringRole, "diagnostic-only");
}
...
async function testCriterionArithmeticAndExplicitCaps() {
  ...
  assert.equal(deriveTask2OverallBandRange(criteria, { overallCap: 6.5 }).label, "6.5");
}
