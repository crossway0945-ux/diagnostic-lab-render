...
ZIP นี้จัดแบบ flat เหมือนเวอร์ชันเก่า เมื่อใช้ Extract All จะได้โฟลเดอร์พร้อมอัปโหลดชื่อ `diagnostic-lab-v7-production-calibration-fixed-upload` และภายในต้องเห็น `package.json`, `script.js`, `server.js` และโฟลเดอร์ `services` ทันที ห้ามมีโฟลเดอร์ชื่อเดิมซ้อนอีกชั้น

ให้อัปโหลดโฟลเดอร์ `diagnostic-lab-v7-production-calibration-fixed-upload` ทั้งโฟลเดอร์ไปไว้ที่ repository root ของ `diagnostic-lab-render`

ตรวจใน GitHub ก่อน deploy ว่า path ต่อไปนี้เปิดได้จริง:

```text
diagnostic-lab-v7-production-calibration-fixed-upload/package.json
diagnostic-lab-v7-production-calibration-fixed-upload/server.js
diagnostic-lab-v7-production-calibration-fixed-upload/script.js
diagnostic-lab-v7-production-calibration-fixed-upload/services/task2Safety.js
diagnostic-lab-v7-production-calibration-fixed-upload/tests/v7-production-calibration.test.mjs
```

## ค่า Render ที่ถูกต้อง

```text
Root Directory: diagnostic-lab-v7-production-calibration-fixed-upload
