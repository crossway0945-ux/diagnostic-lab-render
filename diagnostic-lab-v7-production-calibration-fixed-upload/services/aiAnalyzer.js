...
function applyTask2StrictGuardrails(analysis, payload) {
  ...
  const lowBandProtected = applyTask2LowBandSafety({ ...analysis, feedbackCards }, payload, safety);
  ...
  const promptCoverageStatus = route.missingRequirements.length
    ...
    : route.status === "partial" ? "All required routes are present; development is partial" : "All required routes are covered";
  ...
  return {
    ...
    taskRequirements: canonical.taskRequirements,
    routeAssessment: route,
    promptCoverageStatus,
    thesisRouteStatus: canonical.stanceRequired
      ? (route.missingRequirements.includes("position") || route.missingRequirements.includes("comparative judgement") ? "Required judgement missing" : "Required judgement present")
      : "Not applicable — this task type does not require a position",
    brokenPromiseDetected: route.status === "failed" && route.missingRequirements.length > 0,
    bodyRouteAlignmentStatus: route.status === "controlled" ? "Controlled" : route.status === "partial" ? "Partially developed" : route.status,
    SARExampleStatus: canonical.frameworkAssessment.sar.status,
    conclusionClosureStatus: conclusionRequirement?.status === "present" ? "Strong" : "Needs Work",
    ...
    severitySummary: route.status === "partial"
    ...
    routeConflict: ["failed", "conflicting"].includes(route.status),
    ...
    warnings: [
      ...
      "Task 2 V7 canonical route and criterion arithmetic applied."
    ].filter((warning, index, list) => list.indexOf(warning) === index)
  };
}
...
function reconcileTask2ExecutiveSummary(providerSummary, canonical, safety) {
  ...
  if (route.status === "partial") {
    const partialRoutes = route.bodyRoutes.filter((item) => item.status === "partial").map((item) => `Body ${item.index} (${item.label})`);
  ...
  if (["failed", "conflicting"].includes(route.status)) {
    return `The ${canonical.essayTypeLabel} route is ${route.status}: ${route.missingRequirements.join(", ") || "the required routes do not remain consistent"}.`;
  }
...
function reconcileTask2UrgentRepair(providerRepair, canonical, safety) {
  ...
  if (!canonical.stanceRequired && /position|agree|disagree|thesis route/i.test(text)) {
    return canonical.routeAssessment.status === "partial"
...
function reconcileCanonicalTask2FrameworkScores(input = {}, canonical) {
  const output = { ...(input || {}) };
  const route = canonical.routeAssessment;
  const strong = route.status === "controlled";
  const moderate = route.status === "partial";
  const routeStatus = strong ? "Strong" : moderate ? "Moderate" : "Needs Work";
  output["Essay Type Recognition"] = {
    status: "Strong",
    diagnosis: `The response is assessed with the ${canonical.essayTypeLabel} requirement schema.`
  };
  ...
  output["Thesis Route Clarity"] = canonical.stanceRequired
    ? {
        status: route.missingRequirements.some((item) => /position|judgement|opinion/.test(item)) ? "Needs Work" : "Strong",
        diagnosis: route.position ? `Required judgement detected: ${route.position}.` : "The required judgement is not clear."
      }
    : {
        status: "Not Applicable",
        diagnosis: `${canonical.essayTypeLabel} does not require an opinion for this prompt; route clarity is assessed through its task requirements instead.`
      };
  ...
  output["Explanation Depth"] = {
    status: canonical.frameworkAssessment.bodyDevelopment.status,
  ...
  output["SAR Example Quality"] = {
    status: canonical.frameworkAssessment.sar.status,
    diagnosis: canonical.frameworkAssessment.sar.note
  };
  output["Conclusion Closure"] = {
    status: canonical.frameworkAssessment.conclusionClosure.status,
    diagnosis: route.requirements.find((item) => item.id === "conclusion")?.status === "present"
...
function applyTask2LowBandSafety(analysis, payload, safety) {
  ...
  const routeTerm = safety.stanceRequired ? "position-and-body route" : `${safety.essayRoute} task route`;
  const countEvidence = `Verified word count: ${safety.wordCount}/${safety.minimumRequiredWords}; completion: ${safety.completionStatus}.`;
  const routeEvidence = safety.stanceRequired
    ? `${safety.bodyRouteSummary} Detected position: ${safety.detectedPosition} (${safety.positionConfidence} confidence).`
    : safety.bodyRouteSummary;
  ...
  const completionDiagnosis = critical
    ? `The response is ${safety.underLengthBy} words below the Task 2 minimum, ends with an unfinished sentence, and does not complete a controlled ${routeTerm}. These failures interact rather than operating as an isolated word-count penalty.`
    : safety.directQuestionMissingPart
      ...
      : `The response is ${safety.underLengthBy} words below the Task 2 minimum and remains structurally incomplete; the unresolved ${routeTerm} prevents a secure Band 5.5 Task Response profile.`;
  ...
  next.criteriaScores = {
    ...
    "Coherence & Cohesion": buildTask2LowBandScore(next.criteriaScores?.["Coherence & Cohesion"], otherRange, `Paragraph breaks and linkers do not create a complete progression when the required ${routeTerm} and ending do not form one Golden Thread.`, routeEvidence),
  ...
  next.mainScoreLimitingFactor = critical
    ? `The essay is ${safety.underLengthBy} words below the minimum, ends with an unfinished conclusion, and lacks a controlled ${routeTerm}. Together with limited development and meaning-affecting language errors, these issues prevent it from functioning as a complete Task 2 response.`
    : completionDiagnosis;
  next.mainScoreLimitingIssue = next.mainScoreLimitingFactor;
  next.mostUrgentRepair = safety.stanceRequired
    ? `Submit a complete essay of at least ${safety.minimumRequiredWords} words, including a fully finished conclusion. Before sentence polishing, state one explicit position in the introduction and make both body paragraphs prove that route.`
    : `Submit a complete essay of at least ${safety.minimumRequiredWords} words, including a fully finished conclusion. Before sentence polishing, map every required ${safety.essayRoute} route to a body paragraph and close the same routes in the conclusion.`;
  ...
  next.criticalFlags = Array.from(new Set([
    ...
    safety.routeConflict ? "Conflicting task routes" : safety.stanceRequired ? "Unclear position" : "Incomplete task route",
  ...
  next.thesisRouteStatus = safety.stanceRequired
    ? (safety.detectedPosition === "unclear" || safety.detectedPosition === "contradictory" ? "Missing or contradictory position" : next.thesisRouteStatus)
    : "Not applicable — this task type does not require a position";
...
function applyTask2RevisionSafety(cards, safety) {
  return cards.map((card) => {
    ...
    if (!revisionType || !["Minimal Correction", "Route-Preserving Revision", "Teacher-Guided Recommended Route", "Teacher-Guided Expansion", "Model Paragraph"].includes(revisionType)) {
...
function ensureTask2SafetyCards(cards, payload, safety) {
  ...
  if (safety.unfinishedEndingDetected && safety.evidence.ending && !used.has(normalizeEvidenceText(safety.evidence.ending))) {
    ...
    output.unshift({
      ...
      revisionType: safety.stanceRequired ? "Teacher-Guided Recommended Route" : "Teacher-Guided Expansion",
...
function collectTask2ReportOutputIssues(analysis, payload) {
  const issues = [];
  const safety = payload.task2Safety || analyzeTask2Safety(payload);
  const canonical = analysis.canonicalTask2Analysis || reconcileTask2CanonicalAnalysis(payload, analysis, safety);
  ...
  if (analysis.estimatedBandRange !== canonical.overallBandRange.label) issues.push("Overall band range does not equal the deterministic average of the four displayed criterion ranges.");
  ...
  if (!canonical.stanceRequired) {
    ...
    if (!/^not applicable/i.test(analysis.thesisRouteStatus || "")) issues.push("A non-opinion task must not report thesis-position status as an assessed requirement.");
    if (!/^not applicable/i.test(analysis.kruPomScores?.["Thesis Route Clarity"]?.status || "")) {
      issues.push("Framework breakdown contradicts the canonical non-opinion task route.");
    }
  }
