...
async function testTeacherStudentSelectionReenablesAnalyzeButton() {
  ...
  const cacheBustedScript = "script.js?v=diagnostic-v6-5-semantic-outweigh-integrity";
...
async function testEvaOutweighPositionAndCalibration() {
  ...
  assert.match(safety.bodyRouteSummary, /Overall route status: controlled, but weakened by language accuracy and uneven development/);
...
async function testEvinSemanticOutweighAndIssueMapping() {
  ...
  assert.equal(localAnalysis.estimatedBandRange, "7.0");
  assert.equal(maxBand(localAnalysis.criteriaScores["Task Response"].range), 7.0);
...
async function testTask2LowBandCompletionAndRouteSafety() {
  ...
  assert.match(safety.bodyRouteSummary, /Overall route status: conflicting \/ uncontrolled/);
