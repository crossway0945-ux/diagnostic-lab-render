...
- Exactly five public Task 2 essay families plus Not Sure / Auto-detect; cause-effect, positive/negative, outweigh and multi-question patterns remain internal obligations rather than public essay types
- Semantic Task 2 position analysis separates writer judgement, relation to the prompt claim, clarity, consistency, wording quality and concession control
...
The V8.3 flat release ZIP extracts to `diagnostic-lab-v8-3-final-ready-upload`, with `package.json` immediately inside it. Upload that whole folder to GitHub and use the same name as the Render **Root Directory**. See `UPLOAD_V8_3_TO_RENDER_THAI.md` for the exact steps.
