...
const TASK2_PUBLIC_TYPE_LABELS = Object.freeze({
...
export function classifyTask2Prompt(payload = {}) {
  ...
  add(TASK2_PUBLIC_FAMILIES.DISCUSS_BOTH_VIEWS, TASK2_INTERNAL_SUBTYPES.STANDARD, "high", /discuss both views|discuss (?:these|the) views/, "two views", ["discuss view A", "discuss view B", "give the writer's opinion when requested"]);
  add(TASK2_PUBLIC_FAMILIES.ADVANTAGES_DISADVANTAGES, TASK2_INTERNAL_SUBTYPES.OUTWEIGH, "high", /(?:do|does) the advantages? outweigh|(?:do|does) the disadvantages? outweigh|outweigh the (?:advantages?|disadvantages?|drawbacks?)/, "comparative outweigh judgement", ["develop advantages", "develop disadvantages", "make a comparative judgement"]);
  add(TASK2_PUBLIC_FAMILIES.DIRECT_QUESTION, TASK2_INTERNAL_SUBTYPES.POSITIVE_NEGATIVE, "high", /positive or negative development|negative or positive development|is this (?:a )?positive or negative/, "evaluative development judgement", ["answer the positive/negative evaluation question", "support the judgement"]);
  add(TASK2_PUBLIC_FAMILIES.DIRECT_QUESTION, TASK2_INTERNAL_SUBTYPES.CAUSES_EFFECTS, "high", /(?:what|which) (?:are )?(?:the )?causes?[^?]*(?:effects?|consequences?|impacts?)|causes?\s*(?:and|&)\s*(?:effects?|consequences?|impacts?)/, "cause-effect prompt pair", ["explain causes", "explain effects", "show the cause-effect mechanism"]);
  add(TASK2_PUBLIC_FAMILIES.PROBLEM_SOLUTION, TASK2_INTERNAL_SUBTYPES.CAUSES_SOLUTIONS, "high", /(?:what|which) (?:are )?(?:the )?causes?[^?]*(?:solutions?|measures?|steps?)|causes?\s*(?:and|&)\s*(?:solutions?|measures?)/, "cause-solution prompt pair", ["explain causes", "develop solutions", "pair solutions with the causes"]);
  add(TASK2_PUBLIC_FAMILIES.PROBLEM_SOLUTION, TASK2_INTERNAL_SUBTYPES.STANDARD, "high", /problems?[^?]*(?:solutions?|measures?|steps?)|problem\s*[&/]\s*solution/, "problem-solution prompt pair", ["explain problems", "develop solutions", "pair solutions with the problems"]);
  add(TASK2_PUBLIC_FAMILIES.ADVANTAGES_DISADVANTAGES, TASK2_INTERNAL_SUBTYPES.STANDARD, "high", /advantages?\s*(?:and|&)\s*disadvantages?|benefits?\s*(?:and|&)\s*drawbacks?|what are the advantages?[^?]*disadvantages?/, "two-sided advantages/disadvantages", ["develop advantages", "develop disadvantages"]);
  add(TASK2_PUBLIC_FAMILIES.OPINION, TASK2_INTERNAL_SUBTYPES.STANDARD, "high", /to what extent do you agree or disagree|do you agree or disagree/, "agree/disagree judgement", ["state a clear position", "support that position", "maintain it through the conclusion"]);
  ...
  if (!winner && directQuestionCount >= 2) {
    winner = {
      ...
      promptObligations: promptParts.map((part, index) => `answer question ${index + 1}: ${part}`)
    };
  }
  ...
  if (!winner && directQuestionCount === 1) {
    winner = {
      ...
      promptObligations: [promptParts[0] || "answer the explicit question"]
    };
  }
  ...
  const classificationMatch = autoDetectSelected || !selectedType || selectedType === winner.publicFamily;
  const stanceRequired = isStanceRequired({ ...payload, essayType: TASK2_PUBLIC_TYPE_LABELS[winner.publicFamily] }, winner.publicFamily, winner.internalSubtype);
  const requiredRoutes = requiredRoutesForType(winner.publicFamily, winner.internalSubtype, stanceRequired, promptParts.length);
  return {
    essayType: winner.publicFamily,
    essayTypeLabel: TASK2_PUBLIC_TYPE_LABELS[winner.publicFamily],
    publicEssayFamily: winner.publicFamily,
    publicEssayFamilyLabel: TASK2_PUBLIC_TYPE_LABELS[winner.publicFamily],
    internalSubtype: winner.internalSubtype,
    promptObligations: winner.promptObligations?.length ? winner.promptObligations : requiredRoutes,
    ...
    selectedEssayTypeLabel: selectedType ? TASK2_PUBLIC_TYPE_LABELS[selectedType] : selected,
    autoDetectSelected,
    classificationMatch,
    stanceRequired,
    requiredRoutes,
...
function detectPosition(text, prompt = "") {
  ...
  if (opinionPropositionPolarity(prompt) === "negative" && /\b(?:money|funds?|spending|investment)[^.!?]{0,100}\b(?:space|exploration|technology)[^.!?]{0,100}\b(?:is|are|remains?|seems?)\s*(?:fully\s+|largely\s+)?justified\b|\b(?:space|exploration|technology|costs?\s+of\s+space)[^.!?]{0,100}\b(?:is|are|remains?|seems?)\s*(?:fully\s+|largely\s+)?justified\b/i.test(source)) {
