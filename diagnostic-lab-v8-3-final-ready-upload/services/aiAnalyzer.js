...
function reconcileCanonicalTask2FrameworkScores(input = {}, canonical) {
  ...
  output["Position Clarity"] = canonical.stanceRequired
    ? {
        status: semanticPosition.positionClarity === "clear" ? "Strong" : "Needs Work",
        diagnosis: semanticPosition.positionClarity === "clear"
          ? `The writer's judgement is clear and shows ${semanticPosition.relationToPromptClaim || route.position}. Stance wording is ${semanticPosition.stanceWordingQuality || "understandable"}; this wording quality is not a missing-position error.`
          : "The required position is not stated clearly enough."
      }
    : { status: "Not Applicable", diagnosis: "This task does not require an agree/disagree position." };
...
function task2SafetyReportFields(safety) {
  return {
    ...
    semanticPosition: safety.semanticPosition,
    task2InternalSubtype: safety.internalSubtype,
    promptObligations: safety.taskClassification?.promptObligations || []
  };
}
...
function collectTask2ReportOutputIssues(analysis, payload) {
  ...
  if (canonical.metadata?.essayType !== safety.taskClassification?.publicEssayFamily) issues.push("Canonical Task 2 public family conflicts with deterministic prompt classification.");
  if (canonical.metadata?.essayTypeLabel !== safety.taskClassification?.publicEssayFamilyLabel) issues.push("Canonical Task 2 public label conflicts with deterministic prompt classification.");
  if (canonical.metadata?.internalSubtype !== safety.taskClassification?.internalSubtype) issues.push("Canonical Task 2 internal subtype conflicts with deterministic prompt obligations.");
  if (!sameSemanticPosition(canonical.routeAssessment?.semanticPosition, safety.semanticPosition)) issues.push("Canonical semantic position conflicts with deterministic position evidence.");
  if (analysis.semanticPosition && !sameSemanticPosition(analysis.semanticPosition, safety.semanticPosition)) issues.push("Displayed semantic position conflicts with canonical position evidence.");
  if (safety.semanticPosition?.positionClarity === "clear") {
    if (canonical.capMetadata?.caps?.some((cap) => cap.reasonCode === "REQUIRED_POSITION_ABSENT")) issues.push("A clear semantic position must not receive a missing-position cap.");
    if (canonical.frameworkAssessment?.display?.["Position Clarity"]?.status !== "Strong") issues.push("Framework Position Clarity conflicts with the clear canonical position.");
  }
...
function sameSemanticPosition(left = {}, right = {}) {
  const fields = [
    "positionRequired",
    "writerJudgement",
