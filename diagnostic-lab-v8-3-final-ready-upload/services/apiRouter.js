...
import { classifyTask2Prompt, mapLegacyTask2PublicType, task2PublicTypeLabel } from "./task2Safety.js";
...
const TASK2_ESSAY_TYPES = new Set([
  ...
  // Read-only compatibility for requests saved before V8.3. These labels are not exposed in the UI or reports.
  "Opinion",
  "Discussion Essay",
  "Problem / Solution",
  "Problem/Solution",
...
async function listProfilesForUser(storage, user) {
  ...
  return Promise.all(profiles.map(async (profile) => ({
    ...profile,
    reportCount: (await storage.getSubmissionHistory(user.username, profile.id)).length
  })));
}
...
function sanitizeAnalysisForClient(analysis = {}) {
  ...
  const taskType = sanitized.taskType || sanitized.canonicalAnalysis?.metadata?.taskType || sanitized.canonicalTask2Analysis?.metadata?.taskType;
  if (taskType === "Task 2") {
    const publicFamily = mapLegacyTask2PublicType(
      sanitized.canonicalAnalysis?.metadata?.essayType ||
      sanitized.canonicalTask2Analysis?.metadata?.essayType ||
      sanitized.task2EssayType ||
      sanitized.essayType
    );
    if (publicFamily) {
      const publicLabel = task2PublicTypeLabel(publicFamily);
      sanitized.essayType = publicLabel;
      sanitized.task2EssayType = publicFamily;
      sanitized.task2EssayTypeLabel = publicLabel;
    }
  }
  return sanitized;
}
...
function sanitizeCanonicalForClient(canonical = {}) {
  ...
  if (safeMetadata.taskType === "Task 2") {
    const publicFamily = mapLegacyTask2PublicType(safeMetadata.essayType || safeMetadata.essayTypeLabel);
    if (publicFamily) {
      safeMetadata.essayType = publicFamily;
      safeMetadata.essayTypeLabel = task2PublicTypeLabel(publicFamily);
    }
  }
...
function sanitizeProgressRecordForClient(record = {}) {
  ...
  if (safe.taskType === "Task 2") {
    const publicFamily = mapLegacyTask2PublicType(safe.canonicalEssayType || safe.canonicalEssayTypeLabel || safe.essayType || safe.taskSubtype);
    if (publicFamily) {
      const publicLabel = task2PublicTypeLabel(publicFamily);
      safe.taskSubtype = publicLabel;
      safe.essayType = publicLabel;
      safe.canonicalEssayType = publicFamily;
      safe.canonicalEssayTypeLabel = publicLabel;
    }
  }
  return safe;
}
...
function validateAnalyzePayload(body) {
  ...
  if (taskType === "Task 2" && !TASK2_ESSAY_TYPES.has(essayType)) {
    throw statusError("Please choose one of the five Task 2 essay families or use Not Sure / Auto-detect.", 400, "VALIDATION_ERROR");
  }
...
export function applyTask2ClassificationGuard(payload = {}) {
  ...
  Object.assign(payload, {
    ...
    task2InternalSubtype: classification.internalSubtype,
  ...
  if (classification.confidence === "medium") {
    ...
    if (!confirmed) {
      const error = statusError(
        `The prompt appears to be ${classification.essayTypeLabel}, but the signal is not fully decisive. Please confirm this family before analysis.`,
        409,
  ...
  if (classification.confidence === "low" && classification.autoDetectSelected) {
    const error = statusError(
      "The essay family cannot be determined safely from this prompt. Please select one of the five Task 2 essay families before analysis.",
      409,
  ...
  if (classification.confidence === "low" && !classification.autoDetectSelected) {
    payload.classificationConfirmation = {
      confirmed: true,
      confirmationMethod: "manual-public-family-selection",
      confirmedEssayType: classification.essayType,
      confirmedEssayTypeLabel: classification.essayTypeLabel,
      confirmedAt: new Date().toISOString()
    };
...
function buildSubmissionHistoryRecord(username, payload, analysis, submissionHash = "") {
  ...
  return {
    ...
    task2InternalSubtype: payload.taskType === "Task 2" ? (payload.task2InternalSubtype || "standard") : "",
    promptObligations: payload.taskType === "Task 2" ? (payload.promptObligations || []) : [],
...
function normalizeError(error) {
  ...
  for (const key of ["selectedEssayType", "detectedEssayType", "promptClassificationConfidence", "classificationMatch", "exactPromptSignals"]) {
