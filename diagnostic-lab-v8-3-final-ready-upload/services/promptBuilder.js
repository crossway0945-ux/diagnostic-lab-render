...
- If examples remain personal/company-level without a wider task-level consequence, Body 2 contains four or more separate mechanisms, or several precision/mechanical errors recur, use 7.0 rather than 7.0-7.5 unless stronger evidence clearly justifies the upper range.
...
- internalSubtype: ${safety.taskClassification?.internalSubtype || "standard"}
- promptObligations: ${Array.isArray(safety.taskClassification?.promptObligations) ? safety.taskClassification.promptObligations.join(" | ") : "not preclassified"}
...
- writerJudgement: ${safety.semanticPosition?.writerJudgement || "not preclassified"}
- relationToPromptClaim: ${safety.semanticPosition?.relationToPromptClaim || "not preclassified"}
- positionClarity: ${safety.semanticPosition?.positionClarity || "not preclassified"}
- positionConsistency: ${safety.semanticPosition?.positionConsistency || "not preclassified"}
- stanceWordingQuality: ${safety.semanticPosition?.stanceWordingQuality || "not preclassified"}
- concessionControl: ${safety.semanticPosition?.concessionControl || "not preclassified"}
...
- The only public Task 2 families are Opinion Essay, Discuss Both Views, Problem & Solution, Advantages & Disadvantages, and Direct Question. Never present an internal subtype as a separate essay family.
- Opinion Essay: assess semantic writer judgement, its relation to the prompt claim, reason alignment and conclusion consistency. For a negatively worded prompt, distinguish agreement with the negative claim from a judgement that rejects it. Treat stance clarity, stance wording quality, and concession control as separate dimensions.
- Discuss Both Views: assess both views; assess an own opinion only if the prompt asks for it.
- Advantages & Disadvantages: assess both sides without inventing a stance requirement. When internalSubtype is outweigh, also require an explicit comparative judgement.
- Problem & Solution: assess problem/cause routes, solution routes, pairing, prompt coverage and conclusion closure. Opinion Position is not applicable, but Thesis Route Clarity remains assessable. Never use supports/opposes-the-proposition wording.
- Direct Question: assess every prompt obligation separately. Internal causes-effects prompts require causes, effects and their mechanism; internal positive/negative prompts require an evaluative judgement; multiple questions require a direct answer to each question.
