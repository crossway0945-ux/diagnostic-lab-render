...
async function testTeacherStudentSelectionReenablesAnalyzeButton() {
  ...
  const cacheBustedScript = "script.js?v=diagnostic-v8-3-task2-taxonomy";
...
async function testTask2Band7Calibration() {
  ...
  assert.equal(analysis.criteriaScores["Lexical Resource"].range, "6.5-7.0");
  assert.equal(analysis.criteriaScores["Grammatical Range & Accuracy"].range, "6.5-7.0");
...
async function testEvaOutweighPositionAndCalibration() {
  ...
  assert.equal(safety.essayRoute, "advantages-disadvantages");
  assert.equal(safety.internalSubtype, "outweigh");
  ...
  assert.throws(
    ...
    (error) => error.validationDetails?.some((issue) => /frequent language-accuracy evidence|spelling and collocation evidence|recurring grammar evidence/i.test(issue.message))
  );
  ...
  try {
    ...
    assert.equal(maxBand(repaired.estimatedBandRange), 7.0, "overall must follow the displayed criterion arithmetic; language limits LR/GRA without creating a hidden overall cap");
...
async function testEvinSemanticOutweighAndIssueMapping() {
  ...
  assert.match(localAnalysis.mainScoreLimitingFactor, /analytical depth after examples/i);
  assert.match(localAnalysis.mostUrgentRepair, /wider significance/i);
  ...
  try {
    ...
    assert.equal(repaired.estimatedBandRange, "7.0");
    assert.equal(repaired.detectedPosition, "advantages outweigh the disadvantages");
    assert.equal(repaired.positionConfidence, "high");
    assert.match(repaired.bodyRouteSummary, /Body 1 route: presents the main disadvantage/);
    assert.match(repaired.bodyRouteSummary, /Conclusion route: clearly restates/);
    assert.match(repaired.mainScoreLimitingFactor, /analytical depth after examples/i);
    assert.match(repaired.mostUrgentRepair, /wider significance/i);
    assert.ok(Object.values(repaired.criteriaScores).every((criterion) => maxBand(criterion.range) <= 7.0));
    assert.equal(new Set(repaired.top3Issues.map((issue) => issue.feedbackCardId)).size, 3);
...
async function testDuplicateSubmissionHashCache() {
  ...
  try {
    ...
    assert.equal(first.json.analysis.estimatedBandRange, "6.5-7.0");
...
async function testTask2LowBandCompletionAndRouteSafety() {
  ...
  assert.match(safety.bodyRouteSummary, /Body 2 route: opposes or limits the policy because of public cost/);
  assert.match(safety.bodyRouteSummary, /Conclusion route: states agreement too late and remains unfinished/);
  assert.match(safety.bodyRouteSummary, /Overall route status: contradicted/);
  ...
  assert.equal(analysis.estimatedBandRange, "5.5-6.0");
...
async function testTask2RouteTypeAndCompletionRegressions() {
  ...
  assert.equal(outweigh.essayRoute, "advantages-disadvantages");
  assert.equal(outweigh.internalSubtype, "outweigh");
