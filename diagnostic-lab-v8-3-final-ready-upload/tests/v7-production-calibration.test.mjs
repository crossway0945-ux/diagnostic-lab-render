...
async function testTwentyFourTaskTypeFixtures() {
  ...
  for (const definition of TASK_TYPES) {
    for (const caseName of CASES) {
      ...
      if (definition.internalSubtype) assert.equal(safety.internalSubtype, definition.internalSubtype, `${definition.expected}/${caseName} internal subtype`);
...
async function testEvinProblemSolutionRegression() {
  ...
  const safety = analyzeTask2Safety(payload);
  assert.equal(safety.essayRoute, "problem-solution");
  assert.equal(safety.internalSubtype, "causes-solutions");
  ...
  assert.equal(canonical.criterionScores["Coherence & Cohesion"].range, "7.0");
  assert.equal(canonical.criterionScores["Lexical Resource"].range, "6.5-7.0");
  assert.equal(canonical.criterionScores["Grammatical Range & Accuracy"].range, "7.0");
