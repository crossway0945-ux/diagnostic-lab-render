...
async function testSunCanonicalOpinionRoute() {
  ...
  for (const score of Object.values(canonicalTask2Analysis.criterionScores)) {
    assert.equal(score.range, "6.0-6.5");
  }
...
function testLowConfidenceConfirmation() {
  const payload = {
    taskType: "Task 2",
    essayType: "Opinion Essay",
    prompt: "Do you think modern technology benefits society?",
    writing: sunWriting,
    options: {}
  };
  assert.throws(
    () => applyTask2ClassificationGuard(structuredClone(payload)),
    (error) => error.errorCode === "ESSAY_TYPE_CONFIRMATION_REQUIRED" && error.promptClassificationConfidence === "medium"
  );
  const confirmed = applyTask2ClassificationGuard({
    ...structuredClone(payload),
    options: { essayTypeConfirmed: true }
  });
  assert.equal(confirmed.classificationConfirmation.confirmed, true);
  assert.equal(confirmed.promptClassificationConfidence, "medium");
}
