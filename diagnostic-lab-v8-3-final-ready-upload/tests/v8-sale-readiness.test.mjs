...
const TASK2_TYPES = [
  ...
  },
  {
    ...
    selected: "Advantages & Disadvantages",
...
async function testThirtyTask2Fixtures() {
  ...
  for (const definition of TASK2_TYPES) {
    for (const scenario of TASK2_SCENARIOS) {
      ...
      if (definition.internalSubtype) assert.equal(safety.internalSubtype, definition.internalSubtype, `${definition.name}/${scenario}: internal subtype`);
      ...
      assert.equal(analysis.canonicalAnalysis.metadata.essayType, definition.expected);
...
async function testCanonicalRendererAndPdfSerializationPath() {
  const [script, index, server, previewScript, previewIndex, previewCanonical, previewTask2Safety] = await Promise.all([
  ...
  assert.match(script, /function renderPrintReport/);
  ...
  assert.match(index, /<option selected>Not Sure \/ Auto-detect<\/option>/);
  assert.doesNotMatch(index.match(/<select id="essay-type"[\s\S]*?<\/select>/)?.[0] || "", /Advantages Outweigh Disadvantages|Causes & Effects|Positive or Negative Development|Hybrid/);
  assert.match(index, /script\.js\?v=diagnostic-v8-3-task2-taxonomy/);
  ...
  assert.match(previewCanonical, /export function projectCanonicalAnalysis/);
  assert.match(previewTask2Safety, /export function analyzeTask2Safety/);
}
...
function assertCanonicalReport(analysis, payload, label) {
  ...
  assert.equal(canonical.version, "8.0", `${label}: canonical version`);
